...
' for streaming take a look at "test6_b.bas"

' only an test for the fbsound background thread's
' the mainloop will try to consume 90-100% of cpu time
' on slow PC's the backgroundthread's can be go out of sync.
...
if fbs_init(22050)=true then
  ...
  setmouse ,,0
  ? "please wait while decode mp3 ..."
  if fbs_Load_MP3File(data_path & "legends.mp3",@hWave)=true then
