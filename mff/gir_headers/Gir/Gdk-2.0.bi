'            FreeBasic header file, auto-generated by
'                       ### GirToBac ###
' LGPLv2.1 (C) 2013-2022 by Thomas[ dot }Freiherr[ at ]gmx[ dot }net
' Auto-translated from file /usr/share/gir-1.0/Gdk-2.0.gir
TYPE AS GdkAtom_ PTR GdkAtom
#INCLUDE ONCE "_GirToBac-0.0.bi"

' Repository version 1.2
#INCLUDE ONCE "GdkPixbuf-2.0.bi"
#INCLUDE ONCE "Gio-2.0.bi"
#INCLUDE ONCE "Pango-1.0.bi"
#INCLUDE ONCE "cairo-1.0.bi"
TYPE AS guint32 GdkNativeWindow
TYPE AS GdkAtom GdkSelection
TYPE AS GdkAtom GdkSelectionType
TYPE AS GdkAtom GdkTarget
TYPE AS guint32 GdkWChar
TYPE AS gpointer GdkXEvent
TYPE AS _GdkAppLaunchContext GdkAppLaunchContext
TYPE AS _GdkAppLaunchContextClass GdkAppLaunchContextClass
TYPE AS _GdkAppLaunchContextPrivate GdkAppLaunchContextPrivate
TYPE AS LONG GdkAxisUse
ENUM
  GDK_AXIS_IGNORE = 0
  GDK_AXIS_X = 1
  GDK_AXIS_Y = 2
  GDK_AXIS_PRESSURE = 3
  GDK_AXIS_XTILT = 4
  GDK_AXIS_YTILT = 5
  GDK_AXIS_WHEEL = 6
  GDK_AXIS_LAST = 7
END ENUM
TYPE AS _GdkBitmap GdkBitmap
TYPE AS LONG GdkByteOrder
ENUM
  GDK_LSB_FIRST = 0
  GDK_MSB_FIRST = 1
END ENUM
#DEFINE GDK_CURRENT_TIME 0
TYPE AS LONG GdkCapStyle
ENUM
  GDK_CAP_NOT_LAST = 0
  GDK_CAP_BUTT = 1
  GDK_CAP_ROUND = 2
  GDK_CAP_PROJECTING = 3
END ENUM
TYPE AS _GdkColor GdkColor
TYPE AS _GdkColormap GdkColormap
TYPE AS _GdkColormapClass GdkColormapClass
TYPE AS LONG GdkCrossingMode
ENUM
  GDK_CROSSING_NORMAL = 0
  GDK_CROSSING_GRAB = 1
  GDK_CROSSING_UNGRAB = 2
  GDK_CROSSING_GTK_GRAB = 3
  GDK_CROSSING_GTK_UNGRAB = 4
  GDK_CROSSING_STATE_CHANGED = 5
END ENUM
TYPE AS _GdkCursor GdkCursor
TYPE AS LONG GdkCursorType
ENUM
  GDK_X_CURSOR = 0
  GDK_ARROW = 2
  GDK_BASED_ARROW_DOWN = 4
  GDK_BASED_ARROW_UP = 6
  GDK_BOAT = 8
  GDK_BOGOSITY = 10
  GDK_BOTTOM_LEFT_CORNER = 12
  GDK_BOTTOM_RIGHT_CORNER = 14
  GDK_BOTTOM_SIDE = 16
  GDK_BOTTOM_TEE = 18
  GDK_BOX_SPIRAL = 20
  GDK_CENTER_PTR = 22
  GDK_CIRCLE = 24
  GDK_CLOCK = 26
  GDK_COFFEE_MUG = 28
  GDK_CROSS = 30
  GDK_CROSS_REVERSE = 32
  GDK_CROSSHAIR = 34
  GDK_DIAMOND_CROSS = 36
  GDK_DOT = 38
  GDK_DOTBOX = 40
  GDK_DOUBLE_ARROW = 42
  GDK_DRAFT_LARGE = 44
  GDK_DRAFT_SMALL = 46
  GDK_DRAPED_BOX = 48
  GDK_EXCHANGE = 50
  GDK_FLEUR = 52
  GDK_GOBBLER = 54
  GDK_GUMBY = 56
  GDK_HAND1 = 58
  GDK_HAND2 = 60
  GDK_HEART = 62
  GDK_ICON = 64
  GDK_IRON_CROSS = 66
  GDK_LEFT_PTR = 68
  GDK_LEFT_SIDE = 70
  GDK_LEFT_TEE = 72
  GDK_LEFTBUTTON = 74
  GDK_LL_ANGLE = 76
  GDK_LR_ANGLE = 78
  GDK_MAN = 80
  GDK_MIDDLEBUTTON = 82
  GDK_MOUSE = 84
  GDK_PENCIL = 86
  GDK_PIRATE = 88
  GDK_PLUS = 90
  GDK_QUESTION_ARROW = 92
  GDK_RIGHT_PTR = 94
  GDK_RIGHT_SIDE = 96
  GDK_RIGHT_TEE = 98
  GDK_RIGHTBUTTON = 100
  GDK_RTL_LOGO = 102
  GDK_SAILBOAT = 104
  GDK_SB_DOWN_ARROW = 106
  GDK_SB_H_DOUBLE_ARROW = 108
  GDK_SB_LEFT_ARROW = 110
  GDK_SB_RIGHT_ARROW = 112
  GDK_SB_UP_ARROW = 114
  GDK_SB_V_DOUBLE_ARROW = 116
  GDK_SHUTTLE = 118
  GDK_SIZING = 120
  GDK_SPIDER = 122
  GDK_SPRAYCAN = 124
  GDK_STAR = 126
  GDK_TARGET = 128
  GDK_TCROSS = 130
  GDK_TOP_LEFT_ARROW = 132
  GDK_TOP_LEFT_CORNER = 134
  GDK_TOP_RIGHT_CORNER = 136
  GDK_TOP_SIDE = 138
  GDK_TOP_TEE = 140
  GDK_TREK = 142
  GDK_UL_ANGLE = 144
  GDK_UMBRELLA = 146
  GDK_UR_ANGLE = 148
  GDK_WATCH = 150
  GDK_XTERM = 152
  GDK_LAST_CURSOR = 153
  GDK_BLANK_CURSOR = -2
  GDK_CURSOR_IS_PIXMAP = -1
END ENUM
TYPE AS _GdkDevice GdkDevice
TYPE AS _GdkDeviceAxis GdkDeviceAxis
TYPE AS _GdkDeviceClass GdkDeviceClass
TYPE AS _GdkDeviceKey GdkDeviceKey
TYPE AS _GdkDisplay GdkDisplay
TYPE AS _GdkDisplayClass GdkDisplayClass
TYPE AS _GdkDisplayManager GdkDisplayManager
TYPE AS _GdkDisplayManagerClass GdkDisplayManagerClass
TYPE AS _GdkDisplayPointerHooks GdkDisplayPointerHooks
TYPE AS LONG GdkDragAction
ENUM
  GDK_ACTION_DEFAULT = 1
  GDK_ACTION_COPY = 2
  GDK_ACTION_MOVE = 4
  GDK_ACTION_LINK = 8
  GDK_ACTION_PRIVATE = 16
  GDK_ACTION_ASK = 32
END ENUM
TYPE AS _GdkDragContext GdkDragContext
TYPE AS _GdkDragContextClass GdkDragContextClass
TYPE AS LONG GdkDragProtocol
ENUM
  GDK_DRAG_PROTO_MOTIF = 0
  GDK_DRAG_PROTO_XDND = 1
  GDK_DRAG_PROTO_ROOTWIN = 2
  GDK_DRAG_PROTO_NONE = 3
  GDK_DRAG_PROTO_WIN32_DROPFILES = 4
  GDK_DRAG_PROTO_OLE2 = 5
  GDK_DRAG_PROTO_LOCAL = 6
END ENUM
TYPE AS _GdkDrawable GdkDrawable
TYPE AS _GdkDrawableClass GdkDrawableClass
TYPE AS _GdkEventAny GdkEventAny
TYPE AS _GdkEventButton GdkEventButton
TYPE AS _GdkEventClient GdkEventClient
TYPE AS _GdkEventConfigure GdkEventConfigure
TYPE AS _GdkEventCrossing GdkEventCrossing
TYPE AS _GdkEventDND GdkEventDND
TYPE AS _GdkEventExpose GdkEventExpose
TYPE AS _GdkEventFocus GdkEventFocus
TYPE AS _GdkEventGrabBroken GdkEventGrabBroken
TYPE AS _GdkEventKey GdkEventKey
TYPE AS LONG GdkEventMask
ENUM
  GDK_EXPOSURE_MASK = 2
  GDK_POINTER_MOTION_MASK = 4
  GDK_POINTER_MOTION_HINT_MASK = 8
  GDK_BUTTON_MOTION_MASK = 16
  GDK_BUTTON1_MOTION_MASK = 32
  GDK_BUTTON2_MOTION_MASK = 64
  GDK_BUTTON3_MOTION_MASK = 128
  GDK_BUTTON_PRESS_MASK = 256
  GDK_BUTTON_RELEASE_MASK = 512
  GDK_KEY_PRESS_MASK = 1024
  GDK_KEY_RELEASE_MASK = 2048
  GDK_ENTER_NOTIFY_MASK = 4096
  GDK_LEAVE_NOTIFY_MASK = 8192
  GDK_FOCUS_CHANGE_MASK = 16384
  GDK_STRUCTURE_MASK = 32768
  GDK_PROPERTY_CHANGE_MASK = 65536
  GDK_VISIBILITY_NOTIFY_MASK = 131072
  GDK_PROXIMITY_IN_MASK = 262144
  GDK_PROXIMITY_OUT_MASK = 524288
  GDK_SUBSTRUCTURE_MASK = 1048576
  GDK_SCROLL_MASK = 2097152
  GDK_ALL_EVENTS_MASK = 4194302
END ENUM
TYPE AS _GdkEventMotion GdkEventMotion
TYPE AS _GdkEventNoExpose GdkEventNoExpose
TYPE AS _GdkEventOwnerChange GdkEventOwnerChange
TYPE AS _GdkEventProperty GdkEventProperty
TYPE AS _GdkEventProximity GdkEventProximity
TYPE AS _GdkEventScroll GdkEventScroll
TYPE AS _GdkEventSelection GdkEventSelection
TYPE AS _GdkEventSetting GdkEventSetting
TYPE AS LONG GdkEventType
ENUM
  GDK_NOTHING = -1
  GDK_DELETE = 0
  GDK_DESTROY = 1
  GDK_EXPOSE = 2
  GDK_MOTION_NOTIFY = 3
  GDK_BUTTON_PRESS = 4
  GDK_2BUTTON_PRESS = 5
  GDK_3BUTTON_PRESS = 6
  GDK_BUTTON_RELEASE = 7
  GDK_KEY_PRESS = 8
  GDK_KEY_RELEASE = 9
  GDK_ENTER_NOTIFY = 10
  GDK_LEAVE_NOTIFY = 11
  GDK_FOCUS_CHANGE = 12
  GDK_CONFIGURE = 13
  GDK_MAP = 14
  GDK_UNMAP = 15
  GDK_PROPERTY_NOTIFY = 16
  GDK_SELECTION_CLEAR = 17
  GDK_SELECTION_REQUEST = 18
  GDK_SELECTION_NOTIFY = 19
  GDK_PROXIMITY_IN = 20
  GDK_PROXIMITY_OUT = 21
  GDK_DRAG_ENTER = 22
  GDK_DRAG_LEAVE = 23
  GDK_DRAG_MOTION = 24
  GDK_DRAG_STATUS = 25
  GDK_DROP_START = 26
  GDK_DROP_FINISHED = 27
  GDK_CLIENT_EVENT = 28
  GDK_VISIBILITY_NOTIFY = 29
  GDK_NO_EXPOSE = 30
  GDK_SCROLL = 31
  GDK_WINDOW_STATE = 32
  GDK_SETTING = 33
  GDK_OWNER_CHANGE = 34
  GDK_GRAB_BROKEN = 35
  GDK_DAMAGE = 36
  GDK_EVENT_LAST = 37
END ENUM
TYPE AS _GdkEventVisibility GdkEventVisibility
TYPE AS _GdkEventWindowState GdkEventWindowState
TYPE AS LONG GdkExtensionMode
ENUM
  GDK_EXTENSION_EVENTS_NONE = 0
  GDK_EXTENSION_EVENTS_ALL = 1
  GDK_EXTENSION_EVENTS_CURSOR = 2
END ENUM
TYPE AS LONG GdkFill
ENUM
  GDK_SOLID = 0
  GDK_TILED = 1
  GDK_STIPPLED = 2
  GDK_OPAQUE_STIPPLED = 3
END ENUM
TYPE AS LONG GdkFillRule
ENUM
  GDK_EVEN_ODD_RULE = 0
  GDK_WINDING_RULE = 1
END ENUM
TYPE AS LONG GdkFilterReturn
ENUM
  GDK_FILTER_CONTINUE = 0
  GDK_FILTER_TRANSLATE = 1
  GDK_FILTER_REMOVE = 2
END ENUM
TYPE AS _GdkFont GdkFont
TYPE AS LONG GdkFontType
ENUM
  GDK_FONT_FONT = 0
  GDK_FONT_FONTSET = 1
END ENUM
TYPE AS LONG GdkFunction
ENUM
  GDK_COPY = 0
  GDK_INVERT = 1
  GDK_XOR = 2
  GDK_CLEAR = 3
  GDK_AND = 4
  GDK_AND_REVERSE = 5
  GDK_AND_INVERT = 6
  GDK_NOOP = 7
  GDK_OR = 8
  GDK_EQUIV = 9
  GDK_OR_REVERSE = 10
  GDK_COPY_INVERT = 11
  GDK_OR_INVERT = 12
  GDK_NAND = 13
  GDK_NOR = 14
  GDK_SET = 15
END ENUM
TYPE AS _GdkGC GdkGC
TYPE AS _GdkGCClass GdkGCClass
TYPE AS _GdkGCValues GdkGCValues
TYPE AS LONG GdkGCValuesMask
ENUM
  GDK_GC_FOREGROUND = 1
  GDK_GC_BACKGROUND = 2
  GDK_GC_FONT = 4
  GDK_GC_FUNCTION = 8
  GDK_GC_FILL = 16
  GDK_GC_TILE = 32
  GDK_GC_STIPPLE = 64
  GDK_GC_CLIP_MASK = 128
  GDK_GC_SUBWINDOW = 256
  GDK_GC_TS_X_ORIGIN = 512
  GDK_GC_TS_Y_ORIGIN = 1024
  GDK_GC_CLIP_X_ORIGIN = 2048
  GDK_GC_CLIP_Y_ORIGIN = 4096
  GDK_GC_EXPOSURES = 8192
  GDK_GC_LINE_WIDTH = 16384
  GDK_GC_LINE_STYLE = 32768
  GDK_GC_CAP_STYLE = 65536
  GDK_GC_JOIN_STYLE = 131072
END ENUM
TYPE AS _GdkGeometry GdkGeometry
TYPE AS LONG GdkGrabStatus
ENUM
  GDK_GRAB_SUCCESS = 0
  GDK_GRAB_ALREADY_GRABBED = 1
  GDK_GRAB_INVALID_TIME = 2
  GDK_GRAB_NOT_VIEWABLE = 3
  GDK_GRAB_FROZEN = 4
END ENUM
TYPE AS LONG GdkGravity
ENUM
  GDK_GRAVITY_NORTH_WEST = 1
  GDK_GRAVITY_NORTH = 2
  GDK_GRAVITY_NORTH_EAST = 3
  GDK_GRAVITY_WEST = 4
  GDK_GRAVITY_CENTER = 5
  GDK_GRAVITY_EAST = 6
  GDK_GRAVITY_SOUTH_WEST = 7
  GDK_GRAVITY_SOUTH = 8
  GDK_GRAVITY_SOUTH_EAST = 9
  GDK_GRAVITY_STATIC = 10
END ENUM
TYPE AS _GdkImage GdkImage
TYPE AS _GdkImageClass GdkImageClass
TYPE AS LONG GdkImageType
ENUM
  GDK_IMAGE_NORMAL = 0
  GDK_IMAGE_SHARED = 1
  GDK_IMAGE_FASTEST = 2
END ENUM
TYPE AS LONG GdkInputCondition
ENUM
  GDK_INPUT_READ = 1
  GDK_INPUT_WRITE = 2
  GDK_INPUT_EXCEPTION = 4
END ENUM
TYPE AS LONG GdkInputMode
ENUM
  GDK_MODE_DISABLED = 0
  GDK_MODE_SCREEN = 1
  GDK_MODE_WINDOW = 2
END ENUM
TYPE AS LONG GdkInputSource
ENUM
  GDK_SOURCE_MOUSE = 0
  GDK_SOURCE_PEN = 1
  GDK_SOURCE_ERASER = 2
  GDK_SOURCE_CURSOR = 3
END ENUM
TYPE AS LONG GdkJoinStyle
ENUM
  GDK_JOIN_MITER = 0
  GDK_JOIN_ROUND = 1
  GDK_JOIN_BEVEL = 2
END ENUM
#DEFINE GDK_KEY_0 48
#DEFINE GDK_KEY_1 49
#DEFINE GDK_KEY_2 50
#DEFINE GDK_KEY_3 51
#DEFINE GDK_KEY_3270_AltCursor 64784
#DEFINE GDK_KEY_3270_Attn 64782
#DEFINE GDK_KEY_3270_BackTab 64773
#DEFINE GDK_KEY_3270_ChangeScreen 64793
#DEFINE GDK_KEY_3270_Copy 64789
#DEFINE GDK_KEY_3270_CursorBlink 64783
#DEFINE GDK_KEY_3270_CursorSelect 64796
#DEFINE GDK_KEY_3270_DeleteWord 64794
#DEFINE GDK_KEY_3270_Duplicate 64769
#DEFINE GDK_KEY_3270_Enter 64798
#DEFINE GDK_KEY_3270_EraseEOF 64774
#DEFINE GDK_KEY_3270_EraseInput 64775
#DEFINE GDK_KEY_3270_ExSelect 64795
#DEFINE GDK_KEY_3270_FieldMark 64770
#DEFINE GDK_KEY_3270_Ident 64787
#DEFINE GDK_KEY_3270_Jump 64786
#DEFINE GDK_KEY_3270_KeyClick 64785
#DEFINE GDK_KEY_3270_Left2 64772
#DEFINE GDK_KEY_3270_PA1 64778
#DEFINE GDK_KEY_3270_PA2 64779
#DEFINE GDK_KEY_3270_PA3 64780
#DEFINE GDK_KEY_3270_Play 64790
#DEFINE GDK_KEY_3270_PrintScreen 64797
#DEFINE GDK_KEY_3270_Quit 64777
#DEFINE GDK_KEY_3270_Record 64792
#DEFINE GDK_KEY_3270_Reset 64776
#DEFINE GDK_KEY_3270_Right2 64771
#DEFINE GDK_KEY_3270_Rule 64788
#DEFINE GDK_KEY_3270_Setup 64791
#DEFINE GDK_KEY_3270_Test 64781
#DEFINE GDK_KEY_4 52
#DEFINE GDK_KEY_5 53
#DEFINE GDK_KEY_6 54
#DEFINE GDK_KEY_7 55
#DEFINE GDK_KEY_8 56
#DEFINE GDK_KEY_9 57
#DEFINE GDK_KEY_A_ 65
#DEFINE GDK_KEY_AE_ 198
#DEFINE GDK_KEY_Aacute_ 193
#DEFINE GDK_KEY_Abelowdot_ 16785056
#DEFINE GDK_KEY_Abreve_ 451
#DEFINE GDK_KEY_Abreveacute_ 16785070
#DEFINE GDK_KEY_Abrevebelowdot_ 16785078
#DEFINE GDK_KEY_Abrevegrave_ 16785072
#DEFINE GDK_KEY_Abrevehook_ 16785074
#DEFINE GDK_KEY_Abrevetilde_ 16785076
#DEFINE GDK_KEY_AccessX_Enable 65136
#DEFINE GDK_KEY_AccessX_Feedback_Enable 65137
#DEFINE GDK_KEY_Acircumflex_ 194
#DEFINE GDK_KEY_Acircumflexacute_ 16785060
#DEFINE GDK_KEY_Acircumflexbelowdot_ 16785068
#DEFINE GDK_KEY_Acircumflexgrave_ 16785062
#DEFINE GDK_KEY_Acircumflexhook_ 16785064
#DEFINE GDK_KEY_Acircumflextilde_ 16785066
#DEFINE GDK_KEY_AddFavorite 269025081
#DEFINE GDK_KEY_Adiaeresis_ 196
#DEFINE GDK_KEY_Agrave_ 192
#DEFINE GDK_KEY_Ahook_ 16785058
#DEFINE GDK_KEY_Alt_L 65513
#DEFINE GDK_KEY_Alt_R 65514
#DEFINE GDK_KEY_Amacron_ 960
#DEFINE GDK_KEY_Aogonek_ 417
#DEFINE GDK_KEY_ApplicationLeft 269025104
#DEFINE GDK_KEY_ApplicationRight 269025105
#DEFINE GDK_KEY_Arabic_0 16778848
#DEFINE GDK_KEY_Arabic_1 16778849
#DEFINE GDK_KEY_Arabic_2 16778850
#DEFINE GDK_KEY_Arabic_3 16778851
#DEFINE GDK_KEY_Arabic_4 16778852
#DEFINE GDK_KEY_Arabic_5 16778853
#DEFINE GDK_KEY_Arabic_6 16778854
#DEFINE GDK_KEY_Arabic_7 16778855
#DEFINE GDK_KEY_Arabic_8 16778856
#DEFINE GDK_KEY_Arabic_9 16778857
#DEFINE GDK_KEY_Arabic_ain 1497
#DEFINE GDK_KEY_Arabic_alef 1479
#DEFINE GDK_KEY_Arabic_alefmaksura 1513
#DEFINE GDK_KEY_Arabic_beh 1480
#DEFINE GDK_KEY_Arabic_comma 1452
#DEFINE GDK_KEY_Arabic_dad 1494
#DEFINE GDK_KEY_Arabic_dal 1487
#DEFINE GDK_KEY_Arabic_damma 1519
#DEFINE GDK_KEY_Arabic_dammatan 1516
#DEFINE GDK_KEY_Arabic_ddal 16778888
#DEFINE GDK_KEY_Arabic_farsi_yeh 16778956
#DEFINE GDK_KEY_Arabic_fatha 1518
#DEFINE GDK_KEY_Arabic_fathatan 1515
#DEFINE GDK_KEY_Arabic_feh 1505
#DEFINE GDK_KEY_Arabic_fullstop 16778964
#DEFINE GDK_KEY_Arabic_gaf 16778927
#DEFINE GDK_KEY_Arabic_ghain 1498
#DEFINE GDK_KEY_Arabic_ha 1511
#DEFINE GDK_KEY_Arabic_hah 1485
#DEFINE GDK_KEY_Arabic_hamza 1473
#DEFINE GDK_KEY_Arabic_hamza_above 16778836
#DEFINE GDK_KEY_Arabic_hamza_below 16778837
#DEFINE GDK_KEY_Arabic_hamzaonalef 1475
#DEFINE GDK_KEY_Arabic_hamzaonwaw 1476
#DEFINE GDK_KEY_Arabic_hamzaonyeh 1478
#DEFINE GDK_KEY_Arabic_hamzaunderalef 1477
#DEFINE GDK_KEY_Arabic_heh 1511
#DEFINE GDK_KEY_Arabic_heh_doachashmee 16778942
#DEFINE GDK_KEY_Arabic_heh_goal 16778945
#DEFINE GDK_KEY_Arabic_jeem 1484
#DEFINE GDK_KEY_Arabic_jeh 16778904
#DEFINE GDK_KEY_Arabic_kaf 1507
#DEFINE GDK_KEY_Arabic_kasra 1520
#DEFINE GDK_KEY_Arabic_kasratan 1517
#DEFINE GDK_KEY_Arabic_keheh 16778921
#DEFINE GDK_KEY_Arabic_khah 1486
#DEFINE GDK_KEY_Arabic_lam 1508
#DEFINE GDK_KEY_Arabic_madda_above 16778835
#DEFINE GDK_KEY_Arabic_maddaonalef 1474
#DEFINE GDK_KEY_Arabic_meem 1509
#DEFINE GDK_KEY_Arabic_noon 1510
#DEFINE GDK_KEY_Arabic_noon_ghunna 16778938
#DEFINE GDK_KEY_Arabic_peh 16778878
#DEFINE GDK_KEY_Arabic_percent 16778858
#DEFINE GDK_KEY_Arabic_qaf 1506
#DEFINE GDK_KEY_Arabic_question_mark 1471
#DEFINE GDK_KEY_Arabic_ra 1489
#DEFINE GDK_KEY_Arabic_rreh 16778897
#DEFINE GDK_KEY_Arabic_sad 1493
#DEFINE GDK_KEY_Arabic_seen 1491
#DEFINE GDK_KEY_Arabic_semicolon 1467
#DEFINE GDK_KEY_Arabic_shadda 1521
#DEFINE GDK_KEY_Arabic_sheen 1492
#DEFINE GDK_KEY_Arabic_sukun 1522
#DEFINE GDK_KEY_Arabic_superscript_alef 16778864
#DEFINE GDK_KEY_Arabic_switch 65406
#DEFINE GDK_KEY_Arabic_tah 1495
#DEFINE GDK_KEY_Arabic_tatweel 1504
#DEFINE GDK_KEY_Arabic_tcheh 16778886
#DEFINE GDK_KEY_Arabic_teh 1482
#DEFINE GDK_KEY_Arabic_tehmarbuta 1481
#DEFINE GDK_KEY_Arabic_thal 1488
#DEFINE GDK_KEY_Arabic_theh 1483
#DEFINE GDK_KEY_Arabic_tteh 16778873
#DEFINE GDK_KEY_Arabic_veh 16778916
#DEFINE GDK_KEY_Arabic_waw 1512
#DEFINE GDK_KEY_Arabic_yeh 1514
#DEFINE GDK_KEY_Arabic_yeh_baree 16778962
#DEFINE GDK_KEY_Arabic_zah 1496
#DEFINE GDK_KEY_Arabic_zain 1490
#DEFINE GDK_KEY_Aring_ 197
#DEFINE GDK_KEY_Armenian_AT_ 16778552
#DEFINE GDK_KEY_Armenian_AYB_ 16778545
#DEFINE GDK_KEY_Armenian_BEN_ 16778546
#DEFINE GDK_KEY_Armenian_CHA_ 16778569
#DEFINE GDK_KEY_Armenian_DA_ 16778548
#DEFINE GDK_KEY_Armenian_DZA_ 16778561
#DEFINE GDK_KEY_Armenian_E_ 16778551
#DEFINE GDK_KEY_Armenian_FE_ 16778582
#DEFINE GDK_KEY_Armenian_GHAT_ 16778562
#DEFINE GDK_KEY_Armenian_GIM_ 16778547
#DEFINE GDK_KEY_Armenian_HI_ 16778565
#DEFINE GDK_KEY_Armenian_HO_ 16778560
#DEFINE GDK_KEY_Armenian_INI_ 16778555
#DEFINE GDK_KEY_Armenian_JE_ 16778571
#DEFINE GDK_KEY_Armenian_KE_ 16778580
#DEFINE GDK_KEY_Armenian_KEN_ 16778559
#DEFINE GDK_KEY_Armenian_KHE_ 16778557
#DEFINE GDK_KEY_Armenian_LYUN_ 16778556
#DEFINE GDK_KEY_Armenian_MEN_ 16778564
#DEFINE GDK_KEY_Armenian_NU_ 16778566
#DEFINE GDK_KEY_Armenian_O_ 16778581
#DEFINE GDK_KEY_Armenian_PE_ 16778570
#DEFINE GDK_KEY_Armenian_PYUR_ 16778579
#DEFINE GDK_KEY_Armenian_RA_ 16778572
#DEFINE GDK_KEY_Armenian_RE_ 16778576
#DEFINE GDK_KEY_Armenian_SE_ 16778573
#DEFINE GDK_KEY_Armenian_SHA_ 16778567
#DEFINE GDK_KEY_Armenian_TCHE_ 16778563
#DEFINE GDK_KEY_Armenian_TO_ 16778553
#DEFINE GDK_KEY_Armenian_TSA_ 16778558
#DEFINE GDK_KEY_Armenian_TSO_ 16778577
#DEFINE GDK_KEY_Armenian_TYUN_ 16778575
#DEFINE GDK_KEY_Armenian_VEV_ 16778574
#DEFINE GDK_KEY_Armenian_VO_ 16778568
#DEFINE GDK_KEY_Armenian_VYUN_ 16778578
#DEFINE GDK_KEY_Armenian_YECH_ 16778549
#DEFINE GDK_KEY_Armenian_ZA_ 16778550
#DEFINE GDK_KEY_Armenian_ZHE_ 16778554
#DEFINE GDK_KEY_Armenian_accent 16778587
#DEFINE GDK_KEY_Armenian_amanak 16778588
#DEFINE GDK_KEY_Armenian_apostrophe 16778586
#DEFINE GDK_KEY_Armenian_at 16778600
#DEFINE GDK_KEY_Armenian_ayb 16778593
#DEFINE GDK_KEY_Armenian_ben 16778594
#DEFINE GDK_KEY_Armenian_but 16778589
#DEFINE GDK_KEY_Armenian_cha 16778617
#DEFINE GDK_KEY_Armenian_da 16778596
#DEFINE GDK_KEY_Armenian_dza 16778609
#DEFINE GDK_KEY_Armenian_e 16778599
#DEFINE GDK_KEY_Armenian_exclam 16778588
#DEFINE GDK_KEY_Armenian_fe 16778630
#DEFINE GDK_KEY_Armenian_full_stop 16778633
#DEFINE GDK_KEY_Armenian_ghat 16778610
#DEFINE GDK_KEY_Armenian_gim 16778595
#DEFINE GDK_KEY_Armenian_hi 16778613
#DEFINE GDK_KEY_Armenian_ho 16778608
#DEFINE GDK_KEY_Armenian_hyphen 16778634
#DEFINE GDK_KEY_Armenian_ini 16778603
#DEFINE GDK_KEY_Armenian_je 16778619
#DEFINE GDK_KEY_Armenian_ke 16778628
#DEFINE GDK_KEY_Armenian_ken 16778607
#DEFINE GDK_KEY_Armenian_khe 16778605
#DEFINE GDK_KEY_Armenian_ligature_ew 16778631
#DEFINE GDK_KEY_Armenian_lyun 16778604
#DEFINE GDK_KEY_Armenian_men 16778612
#DEFINE GDK_KEY_Armenian_nu 16778614
#DEFINE GDK_KEY_Armenian_o 16778629
#DEFINE GDK_KEY_Armenian_paruyk 16778590
#DEFINE GDK_KEY_Armenian_pe 16778618
#DEFINE GDK_KEY_Armenian_pyur 16778627
#DEFINE GDK_KEY_Armenian_question 16778590
#DEFINE GDK_KEY_Armenian_ra 16778620
#DEFINE GDK_KEY_Armenian_re 16778624
#DEFINE GDK_KEY_Armenian_se 16778621
#DEFINE GDK_KEY_Armenian_separation_mark 16778589
#DEFINE GDK_KEY_Armenian_sha 16778615
#DEFINE GDK_KEY_Armenian_shesht 16778587
#DEFINE GDK_KEY_Armenian_tche 16778611
#DEFINE GDK_KEY_Armenian_to 16778601
#DEFINE GDK_KEY_Armenian_tsa 16778606
#DEFINE GDK_KEY_Armenian_tso 16778625
#DEFINE GDK_KEY_Armenian_tyun 16778623
#DEFINE GDK_KEY_Armenian_verjaket 16778633
#DEFINE GDK_KEY_Armenian_vev 16778622
#DEFINE GDK_KEY_Armenian_vo 16778616
#DEFINE GDK_KEY_Armenian_vyun 16778626
#DEFINE GDK_KEY_Armenian_yech 16778597
#DEFINE GDK_KEY_Armenian_yentamna 16778634
#DEFINE GDK_KEY_Armenian_za 16778598
#DEFINE GDK_KEY_Armenian_zhe 16778602
#DEFINE GDK_KEY_Atilde_ 195
#DEFINE GDK_KEY_AudibleBell_Enable 65146
#DEFINE GDK_KEY_AudioCycleTrack 269025179
#DEFINE GDK_KEY_AudioForward 269025175
#DEFINE GDK_KEY_AudioLowerVolume 269025041
#DEFINE GDK_KEY_AudioMedia 269025074
#DEFINE GDK_KEY_AudioMute 269025042
#DEFINE GDK_KEY_AudioNext 269025047
#DEFINE GDK_KEY_AudioPause 269025073
#DEFINE GDK_KEY_AudioPlay 269025044
#DEFINE GDK_KEY_AudioPrev 269025046
#DEFINE GDK_KEY_AudioRaiseVolume 269025043
#DEFINE GDK_KEY_AudioRandomPlay 269025177
#DEFINE GDK_KEY_AudioRecord 269025052
#DEFINE GDK_KEY_AudioRepeat 269025176
#DEFINE GDK_KEY_AudioRewind 269025086
#DEFINE GDK_KEY_AudioStop 269025045
#DEFINE GDK_KEY_Away 269025165
#DEFINE GDK_KEY_B_ 66
#DEFINE GDK_KEY_Babovedot_ 16784898
#DEFINE GDK_KEY_Back 269025062
#DEFINE GDK_KEY_BackForward 269025087
#DEFINE GDK_KEY_BackSpace 65288
#DEFINE GDK_KEY_Battery 269025171
#DEFINE GDK_KEY_Begin 65368
#DEFINE GDK_KEY_Blue 269025190
#DEFINE GDK_KEY_Bluetooth 269025172
#DEFINE GDK_KEY_Book 269025106
#DEFINE GDK_KEY_BounceKeys_Enable 65140
#DEFINE GDK_KEY_Break 65387
#DEFINE GDK_KEY_BrightnessAdjust 269025083
#DEFINE GDK_KEY_Byelorussian_SHORTU_ 1726
#DEFINE GDK_KEY_Byelorussian_shortu 1710
#DEFINE GDK_KEY_C_ 67
#DEFINE GDK_KEY_CD 269025107
#DEFINE GDK_KEY_Cabovedot_ 709
#DEFINE GDK_KEY_Cacute_ 454
#DEFINE GDK_KEY_Calculator 269025053
#DEFINE GDK_KEY_Calendar 269025056
#DEFINE GDK_KEY_Cancel 65385
#DEFINE GDK_KEY_Caps_Lock 65509
#DEFINE GDK_KEY_Ccaron_ 456
#DEFINE GDK_KEY_Ccedilla_ 199
#DEFINE GDK_KEY_Ccircumflex_ 710
#DEFINE GDK_KEY_Clear 65291
#DEFINE GDK_KEY_ClearGrab 269024801
#DEFINE GDK_KEY_Close 269025110
#DEFINE GDK_KEY_Codeinput 65335
#DEFINE GDK_KEY_ColonSign 16785569
#DEFINE GDK_KEY_Community 269025085
#DEFINE GDK_KEY_ContrastAdjust 269025058
#DEFINE GDK_KEY_Control_L 65507
#DEFINE GDK_KEY_Control_R 65508
#DEFINE GDK_KEY_Copy 269025111
#DEFINE GDK_KEY_CruzeiroSign 16785570
#DEFINE GDK_KEY_Cut 269025112
#DEFINE GDK_KEY_CycleAngle 269025180
#DEFINE GDK_KEY_Cyrillic_A_ 1761
#DEFINE GDK_KEY_Cyrillic_BE_ 1762
#DEFINE GDK_KEY_Cyrillic_CHE_ 1790
#DEFINE GDK_KEY_Cyrillic_CHE_descender_ 16778422
#DEFINE GDK_KEY_Cyrillic_CHE_vertstroke_ 16778424
#DEFINE GDK_KEY_Cyrillic_DE_ 1764
#DEFINE GDK_KEY_Cyrillic_DZHE_ 1727
#DEFINE GDK_KEY_Cyrillic_E_ 1788
#DEFINE GDK_KEY_Cyrillic_EF_ 1766
#DEFINE GDK_KEY_Cyrillic_EL_ 1772
#DEFINE GDK_KEY_Cyrillic_EM_ 1773
#DEFINE GDK_KEY_Cyrillic_EN_ 1774
#DEFINE GDK_KEY_Cyrillic_EN_descender_ 16778402
#DEFINE GDK_KEY_Cyrillic_ER_ 1778
#DEFINE GDK_KEY_Cyrillic_ES_ 1779
#DEFINE GDK_KEY_Cyrillic_GHE_ 1767
#DEFINE GDK_KEY_Cyrillic_GHE_bar_ 16778386
#DEFINE GDK_KEY_Cyrillic_HA_ 1768
#DEFINE GDK_KEY_Cyrillic_HARDSIGN_ 1791
#DEFINE GDK_KEY_Cyrillic_HA_descender_ 16778418
#DEFINE GDK_KEY_Cyrillic_I_ 1769
#DEFINE GDK_KEY_Cyrillic_IE_ 1765
#DEFINE GDK_KEY_Cyrillic_IO_ 1715
#DEFINE GDK_KEY_Cyrillic_I_macron_ 16778466
#DEFINE GDK_KEY_Cyrillic_JE_ 1720
#DEFINE GDK_KEY_Cyrillic_KA_ 1771
#DEFINE GDK_KEY_Cyrillic_KA_descender_ 16778394
#DEFINE GDK_KEY_Cyrillic_KA_vertstroke_ 16778396
#DEFINE GDK_KEY_Cyrillic_LJE_ 1721
#DEFINE GDK_KEY_Cyrillic_NJE_ 1722
#DEFINE GDK_KEY_Cyrillic_O_ 1775
#DEFINE GDK_KEY_Cyrillic_O_bar_ 16778472
#DEFINE GDK_KEY_Cyrillic_PE_ 1776
#DEFINE GDK_KEY_Cyrillic_SCHWA_ 16778456
#DEFINE GDK_KEY_Cyrillic_SHA_ 1787
#DEFINE GDK_KEY_Cyrillic_SHCHA_ 1789
#DEFINE GDK_KEY_Cyrillic_SHHA_ 16778426
#DEFINE GDK_KEY_Cyrillic_SHORTI_ 1770
#DEFINE GDK_KEY_Cyrillic_SOFTSIGN_ 1784
#DEFINE GDK_KEY_Cyrillic_TE_ 1780
#DEFINE GDK_KEY_Cyrillic_TSE_ 1763
#DEFINE GDK_KEY_Cyrillic_U_ 1781
#DEFINE GDK_KEY_Cyrillic_U_macron_ 16778478
#DEFINE GDK_KEY_Cyrillic_U_straight_ 16778414
#DEFINE GDK_KEY_Cyrillic_U_straight_bar_ 16778416
#DEFINE GDK_KEY_Cyrillic_VE_ 1783
#DEFINE GDK_KEY_Cyrillic_YA_ 1777
#DEFINE GDK_KEY_Cyrillic_YERU_ 1785
#DEFINE GDK_KEY_Cyrillic_YU_ 1760
#DEFINE GDK_KEY_Cyrillic_ZE_ 1786
#DEFINE GDK_KEY_Cyrillic_ZHE_ 1782
#DEFINE GDK_KEY_Cyrillic_ZHE_descender_ 16778390
#DEFINE GDK_KEY_Cyrillic_a 1729
#DEFINE GDK_KEY_Cyrillic_be 1730
#DEFINE GDK_KEY_Cyrillic_che 1758
#DEFINE GDK_KEY_Cyrillic_che_descender 16778423
#DEFINE GDK_KEY_Cyrillic_che_vertstroke 16778425
#DEFINE GDK_KEY_Cyrillic_de 1732
#DEFINE GDK_KEY_Cyrillic_dzhe 1711
#DEFINE GDK_KEY_Cyrillic_e 1756
#DEFINE GDK_KEY_Cyrillic_ef 1734
#DEFINE GDK_KEY_Cyrillic_el 1740
#DEFINE GDK_KEY_Cyrillic_em 1741
#DEFINE GDK_KEY_Cyrillic_en 1742
#DEFINE GDK_KEY_Cyrillic_en_descender 16778403
#DEFINE GDK_KEY_Cyrillic_er 1746
#DEFINE GDK_KEY_Cyrillic_es 1747
#DEFINE GDK_KEY_Cyrillic_ghe 1735
#DEFINE GDK_KEY_Cyrillic_ghe_bar 16778387
#DEFINE GDK_KEY_Cyrillic_ha 1736
#DEFINE GDK_KEY_Cyrillic_ha_descender 16778419
#DEFINE GDK_KEY_Cyrillic_hardsign 1759
#DEFINE GDK_KEY_Cyrillic_i 1737
#DEFINE GDK_KEY_Cyrillic_i_macron 16778467
#DEFINE GDK_KEY_Cyrillic_ie 1733
#DEFINE GDK_KEY_Cyrillic_io 1699
#DEFINE GDK_KEY_Cyrillic_je 1704
#DEFINE GDK_KEY_Cyrillic_ka 1739
#DEFINE GDK_KEY_Cyrillic_ka_descender 16778395
#DEFINE GDK_KEY_Cyrillic_ka_vertstroke 16778397
#DEFINE GDK_KEY_Cyrillic_lje 1705
#DEFINE GDK_KEY_Cyrillic_nje 1706
#DEFINE GDK_KEY_Cyrillic_o 1743
#DEFINE GDK_KEY_Cyrillic_o_bar 16778473
#DEFINE GDK_KEY_Cyrillic_pe 1744
#DEFINE GDK_KEY_Cyrillic_schwa 16778457
#DEFINE GDK_KEY_Cyrillic_sha 1755
#DEFINE GDK_KEY_Cyrillic_shcha 1757
#DEFINE GDK_KEY_Cyrillic_shha 16778427
#DEFINE GDK_KEY_Cyrillic_shorti 1738
#DEFINE GDK_KEY_Cyrillic_softsign 1752
#DEFINE GDK_KEY_Cyrillic_te 1748
#DEFINE GDK_KEY_Cyrillic_tse 1731
#DEFINE GDK_KEY_Cyrillic_u 1749
#DEFINE GDK_KEY_Cyrillic_u_macron 16778479
#DEFINE GDK_KEY_Cyrillic_u_straight 16778415
#DEFINE GDK_KEY_Cyrillic_u_straight_bar 16778417
#DEFINE GDK_KEY_Cyrillic_ve 1751
#DEFINE GDK_KEY_Cyrillic_ya 1745
#DEFINE GDK_KEY_Cyrillic_yeru 1753
#DEFINE GDK_KEY_Cyrillic_yu 1728
#DEFINE GDK_KEY_Cyrillic_ze 1754
#DEFINE GDK_KEY_Cyrillic_zhe 1750
#DEFINE GDK_KEY_Cyrillic_zhe_descender 16778391
#DEFINE GDK_KEY_D_ 68
#DEFINE GDK_KEY_DOS 269025114
#DEFINE GDK_KEY_Dabovedot_ 16784906
#DEFINE GDK_KEY_Dcaron_ 463
#DEFINE GDK_KEY_Delete 65535
#DEFINE GDK_KEY_Display 269025113
#DEFINE GDK_KEY_Documents 269025115
#DEFINE GDK_KEY_DongSign 16785579
#DEFINE GDK_KEY_Down 65364
#DEFINE GDK_KEY_Dstroke_ 464
#DEFINE GDK_KEY_E_ 69
#DEFINE GDK_KEY_ENG_ 957
#DEFINE GDK_KEY_ETH_2 208
#DEFINE GDK_KEY_Eabovedot_ 972
#DEFINE GDK_KEY_Eacute_ 201
#DEFINE GDK_KEY_Ebelowdot_ 16785080
#DEFINE GDK_KEY_Ecaron_ 460
#DEFINE GDK_KEY_Ecircumflex_ 202
#DEFINE GDK_KEY_Ecircumflexacute_ 16785086
#DEFINE GDK_KEY_Ecircumflexbelowdot_ 16785094
#DEFINE GDK_KEY_Ecircumflexgrave_ 16785088
#DEFINE GDK_KEY_Ecircumflexhook_ 16785090
#DEFINE GDK_KEY_Ecircumflextilde_ 16785092
#DEFINE GDK_KEY_EcuSign 16785568
#DEFINE GDK_KEY_Ediaeresis_ 203
#DEFINE GDK_KEY_Egrave_ 200
#DEFINE GDK_KEY_Ehook_ 16785082
#DEFINE GDK_KEY_Eisu_Shift 65327
#DEFINE GDK_KEY_Eisu_toggle 65328
#DEFINE GDK_KEY_Eject 269025068
#DEFINE GDK_KEY_Emacron_ 938
#DEFINE GDK_KEY_End 65367
#DEFINE GDK_KEY_Eogonek_ 458
#DEFINE GDK_KEY_Escape 65307
#DEFINE GDK_KEY_Eth_ 208
#DEFINE GDK_KEY_Etilde_ 16785084
#DEFINE GDK_KEY_EuroSign 8364
#DEFINE GDK_KEY_Excel 269025116
#DEFINE GDK_KEY_Execute 65378
#DEFINE GDK_KEY_Explorer 269025117
#DEFINE GDK_KEY_F_ 70
#DEFINE GDK_KEY_F1 65470
#DEFINE GDK_KEY_F10 65479
#DEFINE GDK_KEY_F11 65480
#DEFINE GDK_KEY_F12 65481
#DEFINE GDK_KEY_F13 65482
#DEFINE GDK_KEY_F14 65483
#DEFINE GDK_KEY_F15 65484
#DEFINE GDK_KEY_F16 65485
#DEFINE GDK_KEY_F17 65486
#DEFINE GDK_KEY_F18 65487
#DEFINE GDK_KEY_F19 65488
#DEFINE GDK_KEY_F2 65471
#DEFINE GDK_KEY_F20 65489
#DEFINE GDK_KEY_F21 65490
#DEFINE GDK_KEY_F22 65491
#DEFINE GDK_KEY_F23 65492
#DEFINE GDK_KEY_F24 65493
#DEFINE GDK_KEY_F25 65494
#DEFINE GDK_KEY_F26 65495
#DEFINE GDK_KEY_F27 65496
#DEFINE GDK_KEY_F28 65497
#DEFINE GDK_KEY_F29 65498
#DEFINE GDK_KEY_F3 65472
#DEFINE GDK_KEY_F30 65499
#DEFINE GDK_KEY_F31 65500
#DEFINE GDK_KEY_F32 65501
#DEFINE GDK_KEY_F33 65502
#DEFINE GDK_KEY_F34 65503
#DEFINE GDK_KEY_F35 65504
#DEFINE GDK_KEY_F4 65473
#DEFINE GDK_KEY_F5 65474
#DEFINE GDK_KEY_F6 65475
#DEFINE GDK_KEY_F7 65476
#DEFINE GDK_KEY_F8 65477
#DEFINE GDK_KEY_F9 65478
#DEFINE GDK_KEY_FFrancSign 16785571
#DEFINE GDK_KEY_Fabovedot_ 16784926
#DEFINE GDK_KEY_Farsi_0 16778992
#DEFINE GDK_KEY_Farsi_1 16778993
#DEFINE GDK_KEY_Farsi_2 16778994
#DEFINE GDK_KEY_Farsi_3 16778995
#DEFINE GDK_KEY_Farsi_4 16778996
#DEFINE GDK_KEY_Farsi_5 16778997
#DEFINE GDK_KEY_Farsi_6 16778998
#DEFINE GDK_KEY_Farsi_7 16778999
#DEFINE GDK_KEY_Farsi_8 16779000
#DEFINE GDK_KEY_Farsi_9 16779001
#DEFINE GDK_KEY_Farsi_yeh 16778956
#DEFINE GDK_KEY_Favorites 269025072
#DEFINE GDK_KEY_Finance 269025084
#DEFINE GDK_KEY_Find 65384
#DEFINE GDK_KEY_First_Virtual_Screen 65232
#DEFINE GDK_KEY_Forward 269025063
#DEFINE GDK_KEY_FrameBack 269025181
#DEFINE GDK_KEY_FrameForward 269025182
#DEFINE GDK_KEY_G_ 71
#DEFINE GDK_KEY_Gabovedot_ 725
#DEFINE GDK_KEY_Game 269025118
#DEFINE GDK_KEY_Gbreve_ 683
#DEFINE GDK_KEY_Gcaron_ 16777702
#DEFINE GDK_KEY_Gcedilla_ 939
#DEFINE GDK_KEY_Gcircumflex_ 728
#DEFINE GDK_KEY_Georgian_an 16781520
#DEFINE GDK_KEY_Georgian_ban 16781521
#DEFINE GDK_KEY_Georgian_can 16781546
#DEFINE GDK_KEY_Georgian_char 16781549
#DEFINE GDK_KEY_Georgian_chin 16781545
#DEFINE GDK_KEY_Georgian_cil 16781548
#DEFINE GDK_KEY_Georgian_don 16781523
#DEFINE GDK_KEY_Georgian_en 16781524
#DEFINE GDK_KEY_Georgian_fi 16781558
#DEFINE GDK_KEY_Georgian_gan 16781522
#DEFINE GDK_KEY_Georgian_ghan 16781542
#DEFINE GDK_KEY_Georgian_hae 16781552
#DEFINE GDK_KEY_Georgian_har 16781556
#DEFINE GDK_KEY_Georgian_he 16781553
#DEFINE GDK_KEY_Georgian_hie 16781554
#DEFINE GDK_KEY_Georgian_hoe 16781557
#DEFINE GDK_KEY_Georgian_in 16781528
#DEFINE GDK_KEY_Georgian_jhan 16781551
#DEFINE GDK_KEY_Georgian_jil 16781547
#DEFINE GDK_KEY_Georgian_kan 16781529
#DEFINE GDK_KEY_Georgian_khar 16781541
#DEFINE GDK_KEY_Georgian_las 16781530
#DEFINE GDK_KEY_Georgian_man 16781531
#DEFINE GDK_KEY_Georgian_nar 16781532
#DEFINE GDK_KEY_Georgian_on 16781533
#DEFINE GDK_KEY_Georgian_par 16781534
#DEFINE GDK_KEY_Georgian_phar 16781540
#DEFINE GDK_KEY_Georgian_qar 16781543
#DEFINE GDK_KEY_Georgian_rae 16781536
#DEFINE GDK_KEY_Georgian_san 16781537
#DEFINE GDK_KEY_Georgian_shin 16781544
#DEFINE GDK_KEY_Georgian_tan 16781527
#DEFINE GDK_KEY_Georgian_tar 16781538
#DEFINE GDK_KEY_Georgian_un 16781539
#DEFINE GDK_KEY_Georgian_vin 16781525
#DEFINE GDK_KEY_Georgian_we 16781555
#DEFINE GDK_KEY_Georgian_xan 16781550
#DEFINE GDK_KEY_Georgian_zen 16781526
#DEFINE GDK_KEY_Georgian_zhar 16781535
#DEFINE GDK_KEY_Go 269025119
#DEFINE GDK_KEY_Greek_ALPHA_ 1985
#DEFINE GDK_KEY_Greek_ALPHAaccent_ 1953
#DEFINE GDK_KEY_Greek_BETA_ 1986
#DEFINE GDK_KEY_Greek_CHI_ 2007
#DEFINE GDK_KEY_Greek_DELTA_ 1988
#DEFINE GDK_KEY_Greek_EPSILON_ 1989
#DEFINE GDK_KEY_Greek_EPSILONaccent_ 1954
#DEFINE GDK_KEY_Greek_ETA_ 1991
#DEFINE GDK_KEY_Greek_ETAaccent_ 1955
#DEFINE GDK_KEY_Greek_GAMMA_ 1987
#DEFINE GDK_KEY_Greek_IOTA_ 1993
#DEFINE GDK_KEY_Greek_IOTAaccent_ 1956
#DEFINE GDK_KEY_Greek_IOTAdiaeresis 1957
#DEFINE GDK_KEY_Greek_IOTAdieresis_ 1957
#DEFINE GDK_KEY_Greek_KAPPA_ 1994
#DEFINE GDK_KEY_Greek_LAMBDA_ 1995
#DEFINE GDK_KEY_Greek_LAMDA_ 1995
#DEFINE GDK_KEY_Greek_MU_ 1996
#DEFINE GDK_KEY_Greek_NU_ 1997
#DEFINE GDK_KEY_Greek_OMEGA_ 2009
#DEFINE GDK_KEY_Greek_OMEGAaccent_ 1963
#DEFINE GDK_KEY_Greek_OMICRON_ 1999
#DEFINE GDK_KEY_Greek_OMICRONaccent_ 1959
#DEFINE GDK_KEY_Greek_PHI_ 2006
#DEFINE GDK_KEY_Greek_PI_ 2000
#DEFINE GDK_KEY_Greek_PSI_ 2008
#DEFINE GDK_KEY_Greek_RHO_ 2001
#DEFINE GDK_KEY_Greek_SIGMA_ 2002
#DEFINE GDK_KEY_Greek_TAU_ 2004
#DEFINE GDK_KEY_Greek_THETA_ 1992
#DEFINE GDK_KEY_Greek_UPSILON_ 2005
#DEFINE GDK_KEY_Greek_UPSILONaccent_ 1960
#DEFINE GDK_KEY_Greek_UPSILONdieresis_ 1961
#DEFINE GDK_KEY_Greek_XI_ 1998
#DEFINE GDK_KEY_Greek_ZETA_ 1990
#DEFINE GDK_KEY_Greek_accentdieresis 1966
#DEFINE GDK_KEY_Greek_alpha 2017
#DEFINE GDK_KEY_Greek_alphaaccent 1969
#DEFINE GDK_KEY_Greek_beta 2018
#DEFINE GDK_KEY_Greek_chi 2039
#DEFINE GDK_KEY_Greek_delta 2020
#DEFINE GDK_KEY_Greek_epsilon 2021
#DEFINE GDK_KEY_Greek_epsilonaccent 1970
#DEFINE GDK_KEY_Greek_eta 2023
#DEFINE GDK_KEY_Greek_etaaccent 1971
#DEFINE GDK_KEY_Greek_finalsmallsigma 2035
#DEFINE GDK_KEY_Greek_gamma 2019
#DEFINE GDK_KEY_Greek_horizbar 1967
#DEFINE GDK_KEY_Greek_iota 2025
#DEFINE GDK_KEY_Greek_iotaaccent 1972
#DEFINE GDK_KEY_Greek_iotaaccentdieresis 1974
#DEFINE GDK_KEY_Greek_iotadieresis 1973
#DEFINE GDK_KEY_Greek_kappa 2026
#DEFINE GDK_KEY_Greek_lambda 2027
#DEFINE GDK_KEY_Greek_lamda 2027
#DEFINE GDK_KEY_Greek_mu 2028
#DEFINE GDK_KEY_Greek_nu 2029
#DEFINE GDK_KEY_Greek_omega 2041
#DEFINE GDK_KEY_Greek_omegaaccent 1979
#DEFINE GDK_KEY_Greek_omicron 2031
#DEFINE GDK_KEY_Greek_omicronaccent 1975
#DEFINE GDK_KEY_Greek_phi 2038
#DEFINE GDK_KEY_Greek_pi 2032
#DEFINE GDK_KEY_Greek_psi 2040
#DEFINE GDK_KEY_Greek_rho 2033
#DEFINE GDK_KEY_Greek_sigma 2034
#DEFINE GDK_KEY_Greek_switch 65406
#DEFINE GDK_KEY_Greek_tau 2036
#DEFINE GDK_KEY_Greek_theta 2024
#DEFINE GDK_KEY_Greek_upsilon 2037
#DEFINE GDK_KEY_Greek_upsilonaccent 1976
#DEFINE GDK_KEY_Greek_upsilonaccentdieresis 1978
#DEFINE GDK_KEY_Greek_upsilondieresis 1977
#DEFINE GDK_KEY_Greek_xi 2030
#DEFINE GDK_KEY_Greek_zeta 2022
#DEFINE GDK_KEY_Green 269025188
#DEFINE GDK_KEY_H_ 72
#DEFINE GDK_KEY_Hangul 65329
#DEFINE GDK_KEY_Hangul_A 3775
#DEFINE GDK_KEY_Hangul_AE 3776
#DEFINE GDK_KEY_Hangul_AraeA 3830
#DEFINE GDK_KEY_Hangul_AraeAE 3831
#DEFINE GDK_KEY_Hangul_Banja 65337
#DEFINE GDK_KEY_Hangul_Cieuc 3770
#DEFINE GDK_KEY_Hangul_Codeinput 65335
#DEFINE GDK_KEY_Hangul_Dikeud 3751
#DEFINE GDK_KEY_Hangul_E 3780
#DEFINE GDK_KEY_Hangul_EO 3779
#DEFINE GDK_KEY_Hangul_EU 3793
#DEFINE GDK_KEY_Hangul_End 65331
#DEFINE GDK_KEY_Hangul_Hanja 65332
#DEFINE GDK_KEY_Hangul_Hieuh 3774
#DEFINE GDK_KEY_Hangul_I 3795
#DEFINE GDK_KEY_Hangul_Ieung 3767
#DEFINE GDK_KEY_Hangul_J_Cieuc 3818
#DEFINE GDK_KEY_Hangul_J_Dikeud 3802
#DEFINE GDK_KEY_Hangul_J_Hieuh 3822
#DEFINE GDK_KEY_Hangul_J_Ieung 3816
#DEFINE GDK_KEY_Hangul_J_Jieuj 3817
#DEFINE GDK_KEY_Hangul_J_Khieuq 3819
#DEFINE GDK_KEY_Hangul_J_Kiyeog 3796
#DEFINE GDK_KEY_Hangul_J_KiyeogSios 3798
#DEFINE GDK_KEY_Hangul_J_KkogjiDalrinIeung 3833
#DEFINE GDK_KEY_Hangul_J_Mieum 3811
#DEFINE GDK_KEY_Hangul_J_Nieun 3799
#DEFINE GDK_KEY_Hangul_J_NieunHieuh 3801
#DEFINE GDK_KEY_Hangul_J_NieunJieuj 3800
#DEFINE GDK_KEY_Hangul_J_PanSios 3832
#DEFINE GDK_KEY_Hangul_J_Phieuf 3821
#DEFINE GDK_KEY_Hangul_J_Pieub 3812
#DEFINE GDK_KEY_Hangul_J_PieubSios 3813
#DEFINE GDK_KEY_Hangul_J_Rieul 3803
#DEFINE GDK_KEY_Hangul_J_RieulHieuh 3810
#DEFINE GDK_KEY_Hangul_J_RieulKiyeog 3804
#DEFINE GDK_KEY_Hangul_J_RieulMieum 3805
#DEFINE GDK_KEY_Hangul_J_RieulPhieuf 3809
#DEFINE GDK_KEY_Hangul_J_RieulPieub 3806
#DEFINE GDK_KEY_Hangul_J_RieulSios 3807
#DEFINE GDK_KEY_Hangul_J_RieulTieut 3808
#DEFINE GDK_KEY_Hangul_J_Sios 3814
#DEFINE GDK_KEY_Hangul_J_SsangKiyeog 3797
#DEFINE GDK_KEY_Hangul_J_SsangSios 3815
#DEFINE GDK_KEY_Hangul_J_Tieut 3820
#DEFINE GDK_KEY_Hangul_J_YeorinHieuh 3834
#DEFINE GDK_KEY_Hangul_Jamo 65333
#DEFINE GDK_KEY_Hangul_Jeonja 65336
#DEFINE GDK_KEY_Hangul_Jieuj 3768
#DEFINE GDK_KEY_Hangul_Khieuq 3771
#DEFINE GDK_KEY_Hangul_Kiyeog 3745
#DEFINE GDK_KEY_Hangul_KiyeogSios 3747
#DEFINE GDK_KEY_Hangul_KkogjiDalrinIeung 3827
#DEFINE GDK_KEY_Hangul_Mieum 3761
#DEFINE GDK_KEY_Hangul_MultipleCandidate 65341
#DEFINE GDK_KEY_Hangul_Nieun 3748
#DEFINE GDK_KEY_Hangul_NieunHieuh 3750
#DEFINE GDK_KEY_Hangul_NieunJieuj 3749
#DEFINE GDK_KEY_Hangul_O 3783
#DEFINE GDK_KEY_Hangul_OE 3786
#DEFINE GDK_KEY_Hangul_PanSios 3826
#DEFINE GDK_KEY_Hangul_Phieuf 3773
#DEFINE GDK_KEY_Hangul_Pieub 3762
#DEFINE GDK_KEY_Hangul_PieubSios 3764
#DEFINE GDK_KEY_Hangul_PostHanja 65339
#DEFINE GDK_KEY_Hangul_PreHanja 65338
#DEFINE GDK_KEY_Hangul_PreviousCandidate 65342
#DEFINE GDK_KEY_Hangul_Rieul 3753
#DEFINE GDK_KEY_Hangul_RieulHieuh 3760
#DEFINE GDK_KEY_Hangul_RieulKiyeog 3754
#DEFINE GDK_KEY_Hangul_RieulMieum 3755
#DEFINE GDK_KEY_Hangul_RieulPhieuf 3759
#DEFINE GDK_KEY_Hangul_RieulPieub 3756
#DEFINE GDK_KEY_Hangul_RieulSios 3757
#DEFINE GDK_KEY_Hangul_RieulTieut 3758
#DEFINE GDK_KEY_Hangul_RieulYeorinHieuh 3823
#DEFINE GDK_KEY_Hangul_Romaja 65334
#DEFINE GDK_KEY_Hangul_SingleCandidate 65340
#DEFINE GDK_KEY_Hangul_Sios 3765
#DEFINE GDK_KEY_Hangul_Special 65343
#DEFINE GDK_KEY_Hangul_SsangDikeud 3752
#DEFINE GDK_KEY_Hangul_SsangJieuj 3769
#DEFINE GDK_KEY_Hangul_SsangKiyeog 3746
#DEFINE GDK_KEY_Hangul_SsangPieub 3763
#DEFINE GDK_KEY_Hangul_SsangSios 3766
#DEFINE GDK_KEY_Hangul_Start 65330
#DEFINE GDK_KEY_Hangul_SunkyeongeumMieum 3824
#DEFINE GDK_KEY_Hangul_SunkyeongeumPhieuf 3828
#DEFINE GDK_KEY_Hangul_SunkyeongeumPieub 3825
#DEFINE GDK_KEY_Hangul_Tieut 3772
#DEFINE GDK_KEY_Hangul_U 3788
#DEFINE GDK_KEY_Hangul_WA 3784
#DEFINE GDK_KEY_Hangul_WAE 3785
#DEFINE GDK_KEY_Hangul_WE 3790
#DEFINE GDK_KEY_Hangul_WEO 3789
#DEFINE GDK_KEY_Hangul_WI 3791
#DEFINE GDK_KEY_Hangul_YA 3777
#DEFINE GDK_KEY_Hangul_YAE 3778
#DEFINE GDK_KEY_Hangul_YE 3782
#DEFINE GDK_KEY_Hangul_YEO 3781
#DEFINE GDK_KEY_Hangul_YI 3794
#DEFINE GDK_KEY_Hangul_YO 3787
#DEFINE GDK_KEY_Hangul_YU 3792
#DEFINE GDK_KEY_Hangul_YeorinHieuh 3829
#DEFINE GDK_KEY_Hangul_switch 65406
#DEFINE GDK_KEY_Hankaku 65321
#DEFINE GDK_KEY_Hcircumflex_ 678
#DEFINE GDK_KEY_Hebrew_switch 65406
#DEFINE GDK_KEY_Help 65386
#DEFINE GDK_KEY_Henkan 65315
#DEFINE GDK_KEY_Henkan_Mode 65315
#DEFINE GDK_KEY_Hibernate 269025192
#DEFINE GDK_KEY_Hiragana 65317
#DEFINE GDK_KEY_Hiragana_Katakana 65319
#DEFINE GDK_KEY_History 269025079
#DEFINE GDK_KEY_Home 65360
#DEFINE GDK_KEY_HomePage 269025048
#DEFINE GDK_KEY_HotLinks 269025082
#DEFINE GDK_KEY_Hstroke_ 673
#DEFINE GDK_KEY_Hyper_L 65517
#DEFINE GDK_KEY_Hyper_R 65518
#DEFINE GDK_KEY_I_ 73
#DEFINE GDK_KEY_ISO_Center_Object 65075
#DEFINE GDK_KEY_ISO_Continuous_Underline 65072
#DEFINE GDK_KEY_ISO_Discontinuous_Underline 65073
#DEFINE GDK_KEY_ISO_Emphasize 65074
#DEFINE GDK_KEY_ISO_Enter 65076
#DEFINE GDK_KEY_ISO_Fast_Cursor_Down 65071
#DEFINE GDK_KEY_ISO_Fast_Cursor_Left 65068
#DEFINE GDK_KEY_ISO_Fast_Cursor_Right 65069
#DEFINE GDK_KEY_ISO_Fast_Cursor_Up 65070
#DEFINE GDK_KEY_ISO_First_Group 65036
#DEFINE GDK_KEY_ISO_First_Group_Lock 65037
#DEFINE GDK_KEY_ISO_Group_Latch 65030
#DEFINE GDK_KEY_ISO_Group_Lock 65031
#DEFINE GDK_KEY_ISO_Group_Shift 65406
#DEFINE GDK_KEY_ISO_Last_Group 65038
#DEFINE GDK_KEY_ISO_Last_Group_Lock 65039
#DEFINE GDK_KEY_ISO_Left_Tab 65056
#DEFINE GDK_KEY_ISO_Level2_Latch 65026
#DEFINE GDK_KEY_ISO_Level3_Latch 65028
#DEFINE GDK_KEY_ISO_Level3_Lock 65029
#DEFINE GDK_KEY_ISO_Level3_Shift 65027
#DEFINE GDK_KEY_ISO_Level5_Latch 65042
#DEFINE GDK_KEY_ISO_Level5_Lock 65043
#DEFINE GDK_KEY_ISO_Level5_Shift 65041
#DEFINE GDK_KEY_ISO_Lock 65025
#DEFINE GDK_KEY_ISO_Move_Line_Down 65058
#DEFINE GDK_KEY_ISO_Move_Line_Up 65057
#DEFINE GDK_KEY_ISO_Next_Group 65032
#DEFINE GDK_KEY_ISO_Next_Group_Lock 65033
#DEFINE GDK_KEY_ISO_Partial_Line_Down 65060
#DEFINE GDK_KEY_ISO_Partial_Line_Up 65059
#DEFINE GDK_KEY_ISO_Partial_Space_Left 65061
#DEFINE GDK_KEY_ISO_Partial_Space_Right 65062
#DEFINE GDK_KEY_ISO_Prev_Group 65034
#DEFINE GDK_KEY_ISO_Prev_Group_Lock 65035
#DEFINE GDK_KEY_ISO_Release_Both_Margins 65067
#DEFINE GDK_KEY_ISO_Release_Margin_Left 65065
#DEFINE GDK_KEY_ISO_Release_Margin_Right 65066
#DEFINE GDK_KEY_ISO_Set_Margin_Left 65063
#DEFINE GDK_KEY_ISO_Set_Margin_Right 65064
#DEFINE GDK_KEY_Iabovedot 681
#DEFINE GDK_KEY_Iacute_ 205
#DEFINE GDK_KEY_Ibelowdot_ 16785098
#DEFINE GDK_KEY_Ibreve_ 16777516
#DEFINE GDK_KEY_Icircumflex_ 206
#DEFINE GDK_KEY_Idiaeresis_ 207
#DEFINE GDK_KEY_Igrave_ 204
#DEFINE GDK_KEY_Ihook_ 16785096
#DEFINE GDK_KEY_Imacron_ 975
#DEFINE GDK_KEY_Insert 65379
#DEFINE GDK_KEY_Iogonek_ 967
#DEFINE GDK_KEY_Itilde_ 933
#DEFINE GDK_KEY_J_ 74
#DEFINE GDK_KEY_Jcircumflex_ 684
#DEFINE GDK_KEY_K_ 75
#DEFINE GDK_KEY_KP_0 65456
#DEFINE GDK_KEY_KP_1 65457
#DEFINE GDK_KEY_KP_2 65458
#DEFINE GDK_KEY_KP_3 65459
#DEFINE GDK_KEY_KP_4 65460
#DEFINE GDK_KEY_KP_5 65461
#DEFINE GDK_KEY_KP_6 65462
#DEFINE GDK_KEY_KP_7 65463
#DEFINE GDK_KEY_KP_8 65464
#DEFINE GDK_KEY_KP_9 65465
#DEFINE GDK_KEY_KP_Add 65451
#DEFINE GDK_KEY_KP_Begin 65437
#DEFINE GDK_KEY_KP_Decimal 65454
#DEFINE GDK_KEY_KP_Delete 65439
#DEFINE GDK_KEY_KP_Divide 65455
#DEFINE GDK_KEY_KP_Down 65433
#DEFINE GDK_KEY_KP_End 65436
#DEFINE GDK_KEY_KP_Enter 65421
#DEFINE GDK_KEY_KP_Equal 65469
#DEFINE GDK_KEY_KP_F1 65425
#DEFINE GDK_KEY_KP_F2 65426
#DEFINE GDK_KEY_KP_F3 65427
#DEFINE GDK_KEY_KP_F4 65428
#DEFINE GDK_KEY_KP_Home 65429
#DEFINE GDK_KEY_KP_Insert 65438
#DEFINE GDK_KEY_KP_Left 65430
#DEFINE GDK_KEY_KP_Multiply 65450
#DEFINE GDK_KEY_KP_Next 65435
#DEFINE GDK_KEY_KP_Page_Down 65435
#DEFINE GDK_KEY_KP_Page_Up 65434
#DEFINE GDK_KEY_KP_Prior 65434
#DEFINE GDK_KEY_KP_Right 65432
#DEFINE GDK_KEY_KP_Separator 65452
#DEFINE GDK_KEY_KP_Space 65408
#DEFINE GDK_KEY_KP_Subtract 65453
#DEFINE GDK_KEY_KP_Tab 65417
#DEFINE GDK_KEY_KP_Up 65431
#DEFINE GDK_KEY_Kana_Lock 65325
#DEFINE GDK_KEY_Kana_Shift 65326
#DEFINE GDK_KEY_Kanji 65313
#DEFINE GDK_KEY_Kanji_Bangou 65335
#DEFINE GDK_KEY_Katakana 65318
#DEFINE GDK_KEY_KbdBrightnessDown 269025030
#DEFINE GDK_KEY_KbdBrightnessUp 269025029
#DEFINE GDK_KEY_KbdLightOnOff 269025028
#DEFINE GDK_KEY_Kcedilla_ 979
#DEFINE GDK_KEY_Korean_Won 3839
#DEFINE GDK_KEY_L_ 76
#DEFINE GDK_KEY_L1 65480
#DEFINE GDK_KEY_L10 65489
#DEFINE GDK_KEY_L2 65481
#DEFINE GDK_KEY_L3 65482
#DEFINE GDK_KEY_L4 65483
#DEFINE GDK_KEY_L5 65484
#DEFINE GDK_KEY_L6 65485
#DEFINE GDK_KEY_L7 65486
#DEFINE GDK_KEY_L8 65487
#DEFINE GDK_KEY_L9 65488
#DEFINE GDK_KEY_Lacute_ 453
#DEFINE GDK_KEY_Last_Virtual_Screen 65236
#DEFINE GDK_KEY_Launch0 269025088
#DEFINE GDK_KEY_Launch1 269025089
#DEFINE GDK_KEY_Launch2 269025090
#DEFINE GDK_KEY_Launch3 269025091
#DEFINE GDK_KEY_Launch4 269025092
#DEFINE GDK_KEY_Launch5 269025093
#DEFINE GDK_KEY_Launch6 269025094
#DEFINE GDK_KEY_Launch7 269025095
#DEFINE GDK_KEY_Launch8 269025096
#DEFINE GDK_KEY_Launch9 269025097
#DEFINE GDK_KEY_LaunchA 269025098
#DEFINE GDK_KEY_LaunchB 269025099
#DEFINE GDK_KEY_LaunchC 269025100
#DEFINE GDK_KEY_LaunchD 269025101
#DEFINE GDK_KEY_LaunchE 269025102
#DEFINE GDK_KEY_LaunchF 269025103
#DEFINE GDK_KEY_Lbelowdot_ 16784950
#DEFINE GDK_KEY_Lcaron_ 421
#DEFINE GDK_KEY_Lcedilla_ 934
#DEFINE GDK_KEY_Left 65361
#DEFINE GDK_KEY_LightBulb 269025077
#DEFINE GDK_KEY_Linefeed 65290
#DEFINE GDK_KEY_LiraSign 16785572
#DEFINE GDK_KEY_LogOff 269025121
#DEFINE GDK_KEY_Lstroke_ 419
#DEFINE GDK_KEY_M_ 77
#DEFINE GDK_KEY_Mabovedot_ 16784960
#DEFINE GDK_KEY_Macedonia_DSE_ 1717
#DEFINE GDK_KEY_Macedonia_GJE_ 1714
#DEFINE GDK_KEY_Macedonia_KJE_ 1724
#DEFINE GDK_KEY_Macedonia_dse 1701
#DEFINE GDK_KEY_Macedonia_gje 1698
#DEFINE GDK_KEY_Macedonia_kje 1708
#DEFINE GDK_KEY_Mae_Koho 65342
#DEFINE GDK_KEY_Mail 269025049
#DEFINE GDK_KEY_MailForward 269025168
#DEFINE GDK_KEY_Market 269025122
#DEFINE GDK_KEY_Massyo 65324
#DEFINE GDK_KEY_Meeting 269025123
#DEFINE GDK_KEY_Memo 269025054
#DEFINE GDK_KEY_Menu 65383
#DEFINE GDK_KEY_MenuKB 269025125
#DEFINE GDK_KEY_MenuPB 269025126
#DEFINE GDK_KEY_Messenger 269025166
#DEFINE GDK_KEY_Meta_L 65511
#DEFINE GDK_KEY_Meta_R 65512
#DEFINE GDK_KEY_MillSign 16785573
#DEFINE GDK_KEY_ModeLock 269025025
#DEFINE GDK_KEY_Mode_switch 65406
#DEFINE GDK_KEY_MonBrightnessDown 269025027
#DEFINE GDK_KEY_MonBrightnessUp 269025026
#DEFINE GDK_KEY_MouseKeys_Accel_Enable 65143
#DEFINE GDK_KEY_MouseKeys_Enable 65142
#DEFINE GDK_KEY_Muhenkan 65314
#DEFINE GDK_KEY_Multi_key 65312
#DEFINE GDK_KEY_MultipleCandidate 65341
#DEFINE GDK_KEY_Music 269025170
#DEFINE GDK_KEY_MyComputer 269025075
#DEFINE GDK_KEY_MySites 269025127
#DEFINE GDK_KEY_N_ 78
#DEFINE GDK_KEY_Nacute_ 465
#DEFINE GDK_KEY_NairaSign 16785574
#DEFINE GDK_KEY_Ncaron_ 466
#DEFINE GDK_KEY_Ncedilla_ 977
#DEFINE GDK_KEY_New 269025128
#DEFINE GDK_KEY_NewSheqelSign 16785578
#DEFINE GDK_KEY_News 269025129
#DEFINE GDK_KEY_Next 65366
#DEFINE GDK_KEY_Next_VMode 269024802
#DEFINE GDK_KEY_Next_Virtual_Screen 65234
#DEFINE GDK_KEY_Ntilde_ 209
#DEFINE GDK_KEY_Num_Lock 65407
#DEFINE GDK_KEY_O_ 79
#DEFINE GDK_KEY_OE_ 5052
#DEFINE GDK_KEY_Oacute_ 211
#DEFINE GDK_KEY_Obarred_ 16777631
#DEFINE GDK_KEY_Obelowdot_ 16785100
#DEFINE GDK_KEY_Ocaron_ 16777681
#DEFINE GDK_KEY_Ocircumflex_ 212
#DEFINE GDK_KEY_Ocircumflexacute_ 16785104
#DEFINE GDK_KEY_Ocircumflexbelowdot_ 16785112
#DEFINE GDK_KEY_Ocircumflexgrave_ 16785106
#DEFINE GDK_KEY_Ocircumflexhook_ 16785108
#DEFINE GDK_KEY_Ocircumflextilde_ 16785110
#DEFINE GDK_KEY_Odiaeresis_ 214
#DEFINE GDK_KEY_Odoubleacute_ 469
#DEFINE GDK_KEY_OfficeHome 269025130
#DEFINE GDK_KEY_Ograve_ 210
#DEFINE GDK_KEY_Ohook_ 16785102
#DEFINE GDK_KEY_Ohorn_ 16777632
#DEFINE GDK_KEY_Ohornacute_ 16785114
#DEFINE GDK_KEY_Ohornbelowdot_ 16785122
#DEFINE GDK_KEY_Ohorngrave_ 16785116
#DEFINE GDK_KEY_Ohornhook_ 16785118
#DEFINE GDK_KEY_Ohorntilde_ 16785120
#DEFINE GDK_KEY_Omacron_ 978
#DEFINE GDK_KEY_Ooblique_ 216
#DEFINE GDK_KEY_Open 269025131
#DEFINE GDK_KEY_OpenURL 269025080
#DEFINE GDK_KEY_Option 269025132
#DEFINE GDK_KEY_Oslash_ 216
#DEFINE GDK_KEY_Otilde_ 213
#DEFINE GDK_KEY_Overlay1_Enable 65144
#DEFINE GDK_KEY_Overlay2_Enable 65145
#DEFINE GDK_KEY_P_ 80
#DEFINE GDK_KEY_Pabovedot_ 16784982
#DEFINE GDK_KEY_Page_Down 65366
#DEFINE GDK_KEY_Page_Up 65365
#DEFINE GDK_KEY_Paste 269025133
#DEFINE GDK_KEY_Pause 65299
#DEFINE GDK_KEY_PesetaSign 16785575
#DEFINE GDK_KEY_Phone 269025134
#DEFINE GDK_KEY_Pictures 269025169
#DEFINE GDK_KEY_Pointer_Accelerate 65274
#DEFINE GDK_KEY_Pointer_Button1 65257
#DEFINE GDK_KEY_Pointer_Button2 65258
#DEFINE GDK_KEY_Pointer_Button3 65259
#DEFINE GDK_KEY_Pointer_Button4 65260
#DEFINE GDK_KEY_Pointer_Button5 65261
#DEFINE GDK_KEY_Pointer_Button_Dflt 65256
#DEFINE GDK_KEY_Pointer_DblClick1 65263
#DEFINE GDK_KEY_Pointer_DblClick2 65264
#DEFINE GDK_KEY_Pointer_DblClick3 65265
#DEFINE GDK_KEY_Pointer_DblClick4 65266
#DEFINE GDK_KEY_Pointer_DblClick5 65267
#DEFINE GDK_KEY_Pointer_DblClick_Dflt 65262
#DEFINE GDK_KEY_Pointer_DfltBtnNext 65275
#DEFINE GDK_KEY_Pointer_DfltBtnPrev 65276
#DEFINE GDK_KEY_Pointer_Down 65251
#DEFINE GDK_KEY_Pointer_DownLeft 65254
#DEFINE GDK_KEY_Pointer_DownRight 65255
#DEFINE GDK_KEY_Pointer_Drag1 65269
#DEFINE GDK_KEY_Pointer_Drag2 65270
#DEFINE GDK_KEY_Pointer_Drag3 65271
#DEFINE GDK_KEY_Pointer_Drag4 65272
#DEFINE GDK_KEY_Pointer_Drag5 65277
#DEFINE GDK_KEY_Pointer_Drag_Dflt 65268
#DEFINE GDK_KEY_Pointer_EnableKeys 65273
#DEFINE GDK_KEY_Pointer_Left 65248
#DEFINE GDK_KEY_Pointer_Right 65249
#DEFINE GDK_KEY_Pointer_Up 65250
#DEFINE GDK_KEY_Pointer_UpLeft 65252
#DEFINE GDK_KEY_Pointer_UpRight 65253
#DEFINE GDK_KEY_PowerDown 269025057
#DEFINE GDK_KEY_PowerOff 269025066
#DEFINE GDK_KEY_Prev_VMode 269024803
#DEFINE GDK_KEY_Prev_Virtual_Screen 65233
#DEFINE GDK_KEY_PreviousCandidate 65342
#DEFINE GDK_KEY_Print 65377
#DEFINE GDK_KEY_Prior 65365
#DEFINE GDK_KEY_Q_ 81
#DEFINE GDK_KEY_R_ 82
#DEFINE GDK_KEY_R1 65490
#DEFINE GDK_KEY_R10 65499
#DEFINE GDK_KEY_R11 65500
#DEFINE GDK_KEY_R12 65501
#DEFINE GDK_KEY_R13 65502
#DEFINE GDK_KEY_R14 65503
#DEFINE GDK_KEY_R15 65504
#DEFINE GDK_KEY_R2 65491
#DEFINE GDK_KEY_R3 65492
#DEFINE GDK_KEY_R4 65493
#DEFINE GDK_KEY_R5 65494
#DEFINE GDK_KEY_R6 65495
#DEFINE GDK_KEY_R7 65496
#DEFINE GDK_KEY_R8 65497
#DEFINE GDK_KEY_R9 65498
#DEFINE GDK_KEY_Racute_ 448
#DEFINE GDK_KEY_Rcaron_ 472
#DEFINE GDK_KEY_Rcedilla_ 931
#DEFINE GDK_KEY_Red 269025187
#DEFINE GDK_KEY_Redo 65382
#DEFINE GDK_KEY_Refresh 269025065
#DEFINE GDK_KEY_Reload 269025139
#DEFINE GDK_KEY_RepeatKeys_Enable 65138
#DEFINE GDK_KEY_Reply 269025138
#DEFINE GDK_KEY_Return 65293
#DEFINE GDK_KEY_Right 65363
#DEFINE GDK_KEY_RockerDown 269025060
#DEFINE GDK_KEY_RockerEnter 269025061
#DEFINE GDK_KEY_RockerUp 269025059
#DEFINE GDK_KEY_Romaji 65316
#DEFINE GDK_KEY_RotateWindows 269025140
#DEFINE GDK_KEY_RotationKB 269025142
#DEFINE GDK_KEY_RotationPB 269025141
#DEFINE GDK_KEY_RupeeSign 16785576
#DEFINE GDK_KEY_S_ 83
#DEFINE GDK_KEY_SCHWA_ 16777615
#DEFINE GDK_KEY_Sabovedot_ 16784992
#DEFINE GDK_KEY_Sacute_ 422
#DEFINE GDK_KEY_Save 269025143
#DEFINE GDK_KEY_Scaron_ 425
#DEFINE GDK_KEY_Scedilla_ 426
#DEFINE GDK_KEY_Scircumflex_ 734
#DEFINE GDK_KEY_ScreenSaver 269025069
#DEFINE GDK_KEY_ScrollClick 269025146
#DEFINE GDK_KEY_ScrollDown 269025145
#DEFINE GDK_KEY_ScrollUp 269025144
#DEFINE GDK_KEY_Scroll_Lock 65300
#DEFINE GDK_KEY_Search 269025051
#DEFINE GDK_KEY_Select 65376
#DEFINE GDK_KEY_SelectButton 269025184
#DEFINE GDK_KEY_Send 269025147
#DEFINE GDK_KEY_Serbian_DJE_ 1713
#DEFINE GDK_KEY_Serbian_DZE_ 1727
#DEFINE GDK_KEY_Serbian_JE_ 1720
#DEFINE GDK_KEY_Serbian_LJE_ 1721
#DEFINE GDK_KEY_Serbian_NJE_ 1722
#DEFINE GDK_KEY_Serbian_TSHE_ 1723
#DEFINE GDK_KEY_Serbian_dje 1697
#DEFINE GDK_KEY_Serbian_dze 1711
#DEFINE GDK_KEY_Serbian_je 1704
#DEFINE GDK_KEY_Serbian_lje 1705
#DEFINE GDK_KEY_Serbian_nje 1706
#DEFINE GDK_KEY_Serbian_tshe 1707
#DEFINE GDK_KEY_Shift_L 65505
#DEFINE GDK_KEY_Shift_Lock 65510
#DEFINE GDK_KEY_Shift_R 65506
#DEFINE GDK_KEY_Shop 269025078
#DEFINE GDK_KEY_SingleCandidate 65340
#DEFINE GDK_KEY_Sleep 269025071
#DEFINE GDK_KEY_SlowKeys_Enable 65139
#DEFINE GDK_KEY_Spell 269025148
#DEFINE GDK_KEY_SplitScreen 269025149
#DEFINE GDK_KEY_Standby 269025040
#DEFINE GDK_KEY_Start 269025050
#DEFINE GDK_KEY_StickyKeys_Enable 65141
#DEFINE GDK_KEY_Stop 269025064
#DEFINE GDK_KEY_Subtitle 269025178
#DEFINE GDK_KEY_Super_L 65515
#DEFINE GDK_KEY_Super_R 65516
#DEFINE GDK_KEY_Support 269025150
#DEFINE GDK_KEY_Suspend 269025191
#DEFINE GDK_KEY_Switch_VT_1 269024769
#DEFINE GDK_KEY_Switch_VT_10 269024778
#DEFINE GDK_KEY_Switch_VT_11 269024779
#DEFINE GDK_KEY_Switch_VT_12 269024780
#DEFINE GDK_KEY_Switch_VT_2 269024770
#DEFINE GDK_KEY_Switch_VT_3 269024771
#DEFINE GDK_KEY_Switch_VT_4 269024772
#DEFINE GDK_KEY_Switch_VT_5 269024773
#DEFINE GDK_KEY_Switch_VT_6 269024774
#DEFINE GDK_KEY_Switch_VT_7 269024775
#DEFINE GDK_KEY_Switch_VT_8 269024776
#DEFINE GDK_KEY_Switch_VT_9 269024777
#DEFINE GDK_KEY_Sys_Req 65301
#DEFINE GDK_KEY_T_ 84
#DEFINE GDK_KEY_THORN_2 222
#DEFINE GDK_KEY_Tab 65289
#DEFINE GDK_KEY_Tabovedot_ 16785002
#DEFINE GDK_KEY_TaskPane 269025151
#DEFINE GDK_KEY_Tcaron_ 427
#DEFINE GDK_KEY_Tcedilla_ 478
#DEFINE GDK_KEY_Terminal 269025152
#DEFINE GDK_KEY_Terminate_Server 65237
#DEFINE GDK_KEY_Thai_baht 3551
#DEFINE GDK_KEY_Thai_bobaimai 3514
#DEFINE GDK_KEY_Thai_chochan 3496
#DEFINE GDK_KEY_Thai_chochang 3498
#DEFINE GDK_KEY_Thai_choching 3497
#DEFINE GDK_KEY_Thai_chochoe 3500
#DEFINE GDK_KEY_Thai_dochada 3502
#DEFINE GDK_KEY_Thai_dodek 3508
#DEFINE GDK_KEY_Thai_fofa 3517
#DEFINE GDK_KEY_Thai_fofan 3519
#DEFINE GDK_KEY_Thai_hohip 3531
#DEFINE GDK_KEY_Thai_honokhuk 3534
#DEFINE GDK_KEY_Thai_khokhai 3490
#DEFINE GDK_KEY_Thai_khokhon 3493
#DEFINE GDK_KEY_Thai_khokhuat 3491
#DEFINE GDK_KEY_Thai_khokhwai 3492
#DEFINE GDK_KEY_Thai_khorakhang 3494
#DEFINE GDK_KEY_Thai_kokai 3489
#DEFINE GDK_KEY_Thai_lakkhangyao 3557
#DEFINE GDK_KEY_Thai_lekchet 3575
#DEFINE GDK_KEY_Thai_lekha 3573
#DEFINE GDK_KEY_Thai_lekhok 3574
#DEFINE GDK_KEY_Thai_lekkao 3577
#DEFINE GDK_KEY_Thai_leknung 3569
#DEFINE GDK_KEY_Thai_lekpaet 3576
#DEFINE GDK_KEY_Thai_leksam 3571
#DEFINE GDK_KEY_Thai_leksi 3572
#DEFINE GDK_KEY_Thai_leksong 3570
#DEFINE GDK_KEY_Thai_leksun 3568
#DEFINE GDK_KEY_Thai_lochula 3532
#DEFINE GDK_KEY_Thai_loling 3525
#DEFINE GDK_KEY_Thai_lu 3526
#DEFINE GDK_KEY_Thai_maichattawa 3563
#DEFINE GDK_KEY_Thai_maiek 3560
#DEFINE GDK_KEY_Thai_maihanakat 3537
#DEFINE GDK_KEY_Thai_maihanakat_maitho 3550
#DEFINE GDK_KEY_Thai_maitaikhu 3559
#DEFINE GDK_KEY_Thai_maitho 3561
#DEFINE GDK_KEY_Thai_maitri 3562
#DEFINE GDK_KEY_Thai_maiyamok 3558
#DEFINE GDK_KEY_Thai_moma 3521
#DEFINE GDK_KEY_Thai_ngongu 3495
#DEFINE GDK_KEY_Thai_nikhahit 3565
#DEFINE GDK_KEY_Thai_nonen 3507
#DEFINE GDK_KEY_Thai_nonu 3513
#DEFINE GDK_KEY_Thai_oang 3533
#DEFINE GDK_KEY_Thai_paiyannoi 3535
#DEFINE GDK_KEY_Thai_phinthu 3546
#DEFINE GDK_KEY_Thai_phophan 3518
#DEFINE GDK_KEY_Thai_phophung 3516
#DEFINE GDK_KEY_Thai_phosamphao 3520
#DEFINE GDK_KEY_Thai_popla 3515
#DEFINE GDK_KEY_Thai_rorua 3523
#DEFINE GDK_KEY_Thai_ru 3524
#DEFINE GDK_KEY_Thai_saraa 3536
#DEFINE GDK_KEY_Thai_saraaa 3538
#DEFINE GDK_KEY_Thai_saraae 3553
#DEFINE GDK_KEY_Thai_saraaimaimalai 3556
#DEFINE GDK_KEY_Thai_saraaimaimuan 3555
#DEFINE GDK_KEY_Thai_saraam 3539
#DEFINE GDK_KEY_Thai_sarae 3552
#DEFINE GDK_KEY_Thai_sarai 3540
#DEFINE GDK_KEY_Thai_saraii 3541
#DEFINE GDK_KEY_Thai_sarao 3554
#DEFINE GDK_KEY_Thai_sarau 3544
#DEFINE GDK_KEY_Thai_saraue 3542
#DEFINE GDK_KEY_Thai_sarauee 3543
#DEFINE GDK_KEY_Thai_sarauu 3545
#DEFINE GDK_KEY_Thai_sorusi 3529
#DEFINE GDK_KEY_Thai_sosala 3528
#DEFINE GDK_KEY_Thai_soso 3499
#DEFINE GDK_KEY_Thai_sosua 3530
#DEFINE GDK_KEY_Thai_thanthakhat 3564
#DEFINE GDK_KEY_Thai_thonangmontho 3505
#DEFINE GDK_KEY_Thai_thophuthao 3506
#DEFINE GDK_KEY_Thai_thothahan 3511
#DEFINE GDK_KEY_Thai_thothan 3504
#DEFINE GDK_KEY_Thai_thothong 3512
#DEFINE GDK_KEY_Thai_thothung 3510
#DEFINE GDK_KEY_Thai_topatak 3503
#DEFINE GDK_KEY_Thai_totao 3509
#DEFINE GDK_KEY_Thai_wowaen 3527
#DEFINE GDK_KEY_Thai_yoyak 3522
#DEFINE GDK_KEY_Thai_yoying 3501
#DEFINE GDK_KEY_Thorn_ 222
#DEFINE GDK_KEY_Time 269025183
#DEFINE GDK_KEY_ToDoList 269025055
#DEFINE GDK_KEY_Tools 269025153
#DEFINE GDK_KEY_TopMenu 269025186
#DEFINE GDK_KEY_TouchpadToggle 269025193
#DEFINE GDK_KEY_Touroku 65323
#DEFINE GDK_KEY_Travel 269025154
#DEFINE GDK_KEY_Tslash_ 940
#DEFINE GDK_KEY_U_ 85
#DEFINE GDK_KEY_UWB 269025174
#DEFINE GDK_KEY_Uacute_ 218
#DEFINE GDK_KEY_Ubelowdot_ 16785124
#DEFINE GDK_KEY_Ubreve_ 733
#DEFINE GDK_KEY_Ucircumflex_ 219
#DEFINE GDK_KEY_Udiaeresis_ 220
#DEFINE GDK_KEY_Udoubleacute_ 475
#DEFINE GDK_KEY_Ugrave_ 217
#DEFINE GDK_KEY_Uhook_ 16785126
#DEFINE GDK_KEY_Uhorn_ 16777647
#DEFINE GDK_KEY_Uhornacute_ 16785128
#DEFINE GDK_KEY_Uhornbelowdot_ 16785136
#DEFINE GDK_KEY_Uhorngrave_ 16785130
#DEFINE GDK_KEY_Uhornhook_ 16785132
#DEFINE GDK_KEY_Uhorntilde_ 16785134
#DEFINE GDK_KEY_Ukrainian_GHE_WITH_UPTURN_ 1725
#DEFINE GDK_KEY_Ukrainian_I_ 1718
#DEFINE GDK_KEY_Ukrainian_IE_ 1716
#DEFINE GDK_KEY_Ukrainian_YI_ 1719
#DEFINE GDK_KEY_Ukrainian_ghe_with_upturn 1709
#DEFINE GDK_KEY_Ukrainian_i 1702
#DEFINE GDK_KEY_Ukrainian_ie 1700
#DEFINE GDK_KEY_Ukrainian_yi 1703
#DEFINE GDK_KEY_Ukranian_I_ 1718
#DEFINE GDK_KEY_Ukranian_JE_ 1716
#DEFINE GDK_KEY_Ukranian_YI_ 1719
#DEFINE GDK_KEY_Ukranian_i 1702
#DEFINE GDK_KEY_Ukranian_je 1700
#DEFINE GDK_KEY_Ukranian_yi 1703
#DEFINE GDK_KEY_Umacron_ 990
#DEFINE GDK_KEY_Undo 65381
#DEFINE GDK_KEY_Ungrab 269024800
#DEFINE GDK_KEY_Uogonek_ 985
#DEFINE GDK_KEY_Up 65362
#DEFINE GDK_KEY_Uring_ 473
#DEFINE GDK_KEY_User1KB 269025157
#DEFINE GDK_KEY_User2KB 269025158
#DEFINE GDK_KEY_UserPB 269025156
#DEFINE GDK_KEY_Utilde_ 989
#DEFINE GDK_KEY_V_ 86
#DEFINE GDK_KEY_VendorHome 269025076
#DEFINE GDK_KEY_Video 269025159
#DEFINE GDK_KEY_View 269025185
#DEFINE GDK_KEY_VoidSymbol 16777215
#DEFINE GDK_KEY_W_ 87
#DEFINE GDK_KEY_WLAN 269025173
#DEFINE GDK_KEY_WWW 269025070
#DEFINE GDK_KEY_Wacute_ 16785026
#DEFINE GDK_KEY_WakeUp 269025067
#DEFINE GDK_KEY_Wcircumflex_ 16777588
#DEFINE GDK_KEY_Wdiaeresis_ 16785028
#DEFINE GDK_KEY_WebCam 269025167
#DEFINE GDK_KEY_Wgrave_ 16785024
#DEFINE GDK_KEY_WheelButton 269025160
#DEFINE GDK_KEY_WindowClear 269025109
#DEFINE GDK_KEY_WonSign 16785577
#DEFINE GDK_KEY_Word 269025161
#DEFINE GDK_KEY_X_ 88
#DEFINE GDK_KEY_Xabovedot_ 16785034
#DEFINE GDK_KEY_Xfer 269025162
#DEFINE GDK_KEY_Y_ 89
#DEFINE GDK_KEY_Yacute_ 221
#DEFINE GDK_KEY_Ybelowdot_ 16785140
#DEFINE GDK_KEY_Ycircumflex_ 16777590
#DEFINE GDK_KEY_Ydiaeresis_ 5054
#DEFINE GDK_KEY_Yellow 269025189
#DEFINE GDK_KEY_Ygrave_ 16785138
#DEFINE GDK_KEY_Yhook_ 16785142
#DEFINE GDK_KEY_Ytilde_ 16785144
#DEFINE GDK_KEY_Z_ 90
#DEFINE GDK_KEY_Zabovedot_ 431
#DEFINE GDK_KEY_Zacute_ 428
#DEFINE GDK_KEY_Zcaron_ 430
#DEFINE GDK_KEY_Zen_Koho 65341
#DEFINE GDK_KEY_Zenkaku 65320
#DEFINE GDK_KEY_Zenkaku_Hankaku 65322
#DEFINE GDK_KEY_ZoomIn 269025163
#DEFINE GDK_KEY_ZoomOut 269025164
#DEFINE GDK_KEY_Zstroke_ 16777653
#DEFINE GDK_KEY_a 97
#DEFINE GDK_KEY_aacute 225
#DEFINE GDK_KEY_abelowdot 16785057
#DEFINE GDK_KEY_abovedot 511
#DEFINE GDK_KEY_abreve 483
#DEFINE GDK_KEY_abreveacute 16785071
#DEFINE GDK_KEY_abrevebelowdot 16785079
#DEFINE GDK_KEY_abrevegrave 16785073
#DEFINE GDK_KEY_abrevehook 16785075
#DEFINE GDK_KEY_abrevetilde 16785077
#DEFINE GDK_KEY_acircumflex 226
#DEFINE GDK_KEY_acircumflexacute 16785061
#DEFINE GDK_KEY_acircumflexbelowdot 16785069
#DEFINE GDK_KEY_acircumflexgrave 16785063
#DEFINE GDK_KEY_acircumflexhook 16785065
#DEFINE GDK_KEY_acircumflextilde 16785067
#DEFINE GDK_KEY_acute 180
#DEFINE GDK_KEY_adiaeresis 228
#DEFINE GDK_KEY_ae 230
#DEFINE GDK_KEY_agrave 224
#DEFINE GDK_KEY_ahook 16785059
#DEFINE GDK_KEY_amacron 992
#DEFINE GDK_KEY_ampersand 38
#DEFINE GDK_KEY_aogonek 433
#DEFINE GDK_KEY_apostrophe 39
#DEFINE GDK_KEY_approxeq 16785992
#DEFINE GDK_KEY_approximate 2248
#DEFINE GDK_KEY_aring 229
#DEFINE GDK_KEY_asciicircum 94
#DEFINE GDK_KEY_asciitilde 126
#DEFINE GDK_KEY_asterisk 42
#DEFINE GDK_KEY_at 64
#DEFINE GDK_KEY_atilde 227
#DEFINE GDK_KEY_b 98
#DEFINE GDK_KEY_babovedot 16784899
#DEFINE GDK_KEY_backslash 92
#DEFINE GDK_KEY_ballotcross 2804
#DEFINE GDK_KEY_bar 124
#DEFINE GDK_KEY_because 16785973
#DEFINE GDK_KEY_blank 2527
#DEFINE GDK_KEY_botintegral 2213
#DEFINE GDK_KEY_botleftparens 2220
#DEFINE GDK_KEY_botleftsqbracket 2216
#DEFINE GDK_KEY_botleftsummation 2226
#DEFINE GDK_KEY_botrightparens 2222
#DEFINE GDK_KEY_botrightsqbracket 2218
#DEFINE GDK_KEY_botrightsummation 2230
#DEFINE GDK_KEY_bott 2550
#DEFINE GDK_KEY_botvertsummationconnector 2228
#DEFINE GDK_KEY_braceleft 123
#DEFINE GDK_KEY_braceright 125
#DEFINE GDK_KEY_bracketleft 91
#DEFINE GDK_KEY_bracketright 93
#DEFINE GDK_KEY_braille_blank 16787456
#DEFINE GDK_KEY_braille_dot_1 65521
#DEFINE GDK_KEY_braille_dot_10 65530
#DEFINE GDK_KEY_braille_dot_2 65522
#DEFINE GDK_KEY_braille_dot_3 65523
#DEFINE GDK_KEY_braille_dot_4 65524
#DEFINE GDK_KEY_braille_dot_5 65525
#DEFINE GDK_KEY_braille_dot_6 65526
#DEFINE GDK_KEY_braille_dot_7 65527
#DEFINE GDK_KEY_braille_dot_8 65528
#DEFINE GDK_KEY_braille_dot_9 65529
#DEFINE GDK_KEY_braille_dots_1 16787457
#DEFINE GDK_KEY_braille_dots_12 16787459
#DEFINE GDK_KEY_braille_dots_123 16787463
#DEFINE GDK_KEY_braille_dots_1234 16787471
#DEFINE GDK_KEY_braille_dots_12345 16787487
#DEFINE GDK_KEY_braille_dots_123456 16787519
#DEFINE GDK_KEY_braille_dots_1234567 16787583
#DEFINE GDK_KEY_braille_dots_12345678 16787711
#DEFINE GDK_KEY_braille_dots_1234568 16787647
#DEFINE GDK_KEY_braille_dots_123457 16787551
#DEFINE GDK_KEY_braille_dots_1234578 16787679
#DEFINE GDK_KEY_braille_dots_123458 16787615
#DEFINE GDK_KEY_braille_dots_12346 16787503
#DEFINE GDK_KEY_braille_dots_123467 16787567
#DEFINE GDK_KEY_braille_dots_1234678 16787695
#DEFINE GDK_KEY_braille_dots_123468 16787631
#DEFINE GDK_KEY_braille_dots_12347 16787535
#DEFINE GDK_KEY_braille_dots_123478 16787663
#DEFINE GDK_KEY_braille_dots_12348 16787599
#DEFINE GDK_KEY_braille_dots_1235 16787479
#DEFINE GDK_KEY_braille_dots_12356 16787511
#DEFINE GDK_KEY_braille_dots_123567 16787575
#DEFINE GDK_KEY_braille_dots_1235678 16787703
#DEFINE GDK_KEY_braille_dots_123568 16787639
#DEFINE GDK_KEY_braille_dots_12357 16787543
#DEFINE GDK_KEY_braille_dots_123578 16787671
#DEFINE GDK_KEY_braille_dots_12358 16787607
#DEFINE GDK_KEY_braille_dots_1236 16787495
#DEFINE GDK_KEY_braille_dots_12367 16787559
#DEFINE GDK_KEY_braille_dots_123678 16787687
#DEFINE GDK_KEY_braille_dots_12368 16787623
#DEFINE GDK_KEY_braille_dots_1237 16787527
#DEFINE GDK_KEY_braille_dots_12378 16787655
#DEFINE GDK_KEY_braille_dots_1238 16787591
#DEFINE GDK_KEY_braille_dots_124 16787467
#DEFINE GDK_KEY_braille_dots_1245 16787483
#DEFINE GDK_KEY_braille_dots_12456 16787515
#DEFINE GDK_KEY_braille_dots_124567 16787579
#DEFINE GDK_KEY_braille_dots_1245678 16787707
#DEFINE GDK_KEY_braille_dots_124568 16787643
#DEFINE GDK_KEY_braille_dots_12457 16787547
#DEFINE GDK_KEY_braille_dots_124578 16787675
#DEFINE GDK_KEY_braille_dots_12458 16787611
#DEFINE GDK_KEY_braille_dots_1246 16787499
#DEFINE GDK_KEY_braille_dots_12467 16787563
#DEFINE GDK_KEY_braille_dots_124678 16787691
#DEFINE GDK_KEY_braille_dots_12468 16787627
#DEFINE GDK_KEY_braille_dots_1247 16787531
#DEFINE GDK_KEY_braille_dots_12478 16787659
#DEFINE GDK_KEY_braille_dots_1248 16787595
#DEFINE GDK_KEY_braille_dots_125 16787475
#DEFINE GDK_KEY_braille_dots_1256 16787507
#DEFINE GDK_KEY_braille_dots_12567 16787571
#DEFINE GDK_KEY_braille_dots_125678 16787699
#DEFINE GDK_KEY_braille_dots_12568 16787635
#DEFINE GDK_KEY_braille_dots_1257 16787539
#DEFINE GDK_KEY_braille_dots_12578 16787667
#DEFINE GDK_KEY_braille_dots_1258 16787603
#DEFINE GDK_KEY_braille_dots_126 16787491
#DEFINE GDK_KEY_braille_dots_1267 16787555
#DEFINE GDK_KEY_braille_dots_12678 16787683
#DEFINE GDK_KEY_braille_dots_1268 16787619
#DEFINE GDK_KEY_braille_dots_127 16787523
#DEFINE GDK_KEY_braille_dots_1278 16787651
#DEFINE GDK_KEY_braille_dots_128 16787587
#DEFINE GDK_KEY_braille_dots_13 16787461
#DEFINE GDK_KEY_braille_dots_134 16787469
#DEFINE GDK_KEY_braille_dots_1345 16787485
#DEFINE GDK_KEY_braille_dots_13456 16787517
#DEFINE GDK_KEY_braille_dots_134567 16787581
#DEFINE GDK_KEY_braille_dots_1345678 16787709
#DEFINE GDK_KEY_braille_dots_134568 16787645
#DEFINE GDK_KEY_braille_dots_13457 16787549
#DEFINE GDK_KEY_braille_dots_134578 16787677
#DEFINE GDK_KEY_braille_dots_13458 16787613
#DEFINE GDK_KEY_braille_dots_1346 16787501
#DEFINE GDK_KEY_braille_dots_13467 16787565
#DEFINE GDK_KEY_braille_dots_134678 16787693
#DEFINE GDK_KEY_braille_dots_13468 16787629
#DEFINE GDK_KEY_braille_dots_1347 16787533
#DEFINE GDK_KEY_braille_dots_13478 16787661
#DEFINE GDK_KEY_braille_dots_1348 16787597
#DEFINE GDK_KEY_braille_dots_135 16787477
#DEFINE GDK_KEY_braille_dots_1356 16787509
#DEFINE GDK_KEY_braille_dots_13567 16787573
#DEFINE GDK_KEY_braille_dots_135678 16787701
#DEFINE GDK_KEY_braille_dots_13568 16787637
#DEFINE GDK_KEY_braille_dots_1357 16787541
#DEFINE GDK_KEY_braille_dots_13578 16787669
#DEFINE GDK_KEY_braille_dots_1358 16787605
#DEFINE GDK_KEY_braille_dots_136 16787493
#DEFINE GDK_KEY_braille_dots_1367 16787557
#DEFINE GDK_KEY_braille_dots_13678 16787685
#DEFINE GDK_KEY_braille_dots_1368 16787621
#DEFINE GDK_KEY_braille_dots_137 16787525
#DEFINE GDK_KEY_braille_dots_1378 16787653
#DEFINE GDK_KEY_braille_dots_138 16787589
#DEFINE GDK_KEY_braille_dots_14 16787465
#DEFINE GDK_KEY_braille_dots_145 16787481
#DEFINE GDK_KEY_braille_dots_1456 16787513
#DEFINE GDK_KEY_braille_dots_14567 16787577
#DEFINE GDK_KEY_braille_dots_145678 16787705
#DEFINE GDK_KEY_braille_dots_14568 16787641
#DEFINE GDK_KEY_braille_dots_1457 16787545
#DEFINE GDK_KEY_braille_dots_14578 16787673
#DEFINE GDK_KEY_braille_dots_1458 16787609
#DEFINE GDK_KEY_braille_dots_146 16787497
#DEFINE GDK_KEY_braille_dots_1467 16787561
#DEFINE GDK_KEY_braille_dots_14678 16787689
#DEFINE GDK_KEY_braille_dots_1468 16787625
#DEFINE GDK_KEY_braille_dots_147 16787529
#DEFINE GDK_KEY_braille_dots_1478 16787657
#DEFINE GDK_KEY_braille_dots_148 16787593
#DEFINE GDK_KEY_braille_dots_15 16787473
#DEFINE GDK_KEY_braille_dots_156 16787505
#DEFINE GDK_KEY_braille_dots_1567 16787569
#DEFINE GDK_KEY_braille_dots_15678 16787697
#DEFINE GDK_KEY_braille_dots_1568 16787633
#DEFINE GDK_KEY_braille_dots_157 16787537
#DEFINE GDK_KEY_braille_dots_1578 16787665
#DEFINE GDK_KEY_braille_dots_158 16787601
#DEFINE GDK_KEY_braille_dots_16 16787489
#DEFINE GDK_KEY_braille_dots_167 16787553
#DEFINE GDK_KEY_braille_dots_1678 16787681
#DEFINE GDK_KEY_braille_dots_168 16787617
#DEFINE GDK_KEY_braille_dots_17 16787521
#DEFINE GDK_KEY_braille_dots_178 16787649
#DEFINE GDK_KEY_braille_dots_18 16787585
#DEFINE GDK_KEY_braille_dots_2 16787458
#DEFINE GDK_KEY_braille_dots_23 16787462
#DEFINE GDK_KEY_braille_dots_234 16787470
#DEFINE GDK_KEY_braille_dots_2345 16787486
#DEFINE GDK_KEY_braille_dots_23456 16787518
#DEFINE GDK_KEY_braille_dots_234567 16787582
#DEFINE GDK_KEY_braille_dots_2345678 16787710
#DEFINE GDK_KEY_braille_dots_234568 16787646
#DEFINE GDK_KEY_braille_dots_23457 16787550
#DEFINE GDK_KEY_braille_dots_234578 16787678
#DEFINE GDK_KEY_braille_dots_23458 16787614
#DEFINE GDK_KEY_braille_dots_2346 16787502
#DEFINE GDK_KEY_braille_dots_23467 16787566
#DEFINE GDK_KEY_braille_dots_234678 16787694
#DEFINE GDK_KEY_braille_dots_23468 16787630
#DEFINE GDK_KEY_braille_dots_2347 16787534
#DEFINE GDK_KEY_braille_dots_23478 16787662
#DEFINE GDK_KEY_braille_dots_2348 16787598
#DEFINE GDK_KEY_braille_dots_235 16787478
#DEFINE GDK_KEY_braille_dots_2356 16787510
#DEFINE GDK_KEY_braille_dots_23567 16787574
#DEFINE GDK_KEY_braille_dots_235678 16787702
#DEFINE GDK_KEY_braille_dots_23568 16787638
#DEFINE GDK_KEY_braille_dots_2357 16787542
#DEFINE GDK_KEY_braille_dots_23578 16787670
#DEFINE GDK_KEY_braille_dots_2358 16787606
#DEFINE GDK_KEY_braille_dots_236 16787494
#DEFINE GDK_KEY_braille_dots_2367 16787558
#DEFINE GDK_KEY_braille_dots_23678 16787686
#DEFINE GDK_KEY_braille_dots_2368 16787622
#DEFINE GDK_KEY_braille_dots_237 16787526
#DEFINE GDK_KEY_braille_dots_2378 16787654
#DEFINE GDK_KEY_braille_dots_238 16787590
#DEFINE GDK_KEY_braille_dots_24 16787466
#DEFINE GDK_KEY_braille_dots_245 16787482
#DEFINE GDK_KEY_braille_dots_2456 16787514
#DEFINE GDK_KEY_braille_dots_24567 16787578
#DEFINE GDK_KEY_braille_dots_245678 16787706
#DEFINE GDK_KEY_braille_dots_24568 16787642
#DEFINE GDK_KEY_braille_dots_2457 16787546
#DEFINE GDK_KEY_braille_dots_24578 16787674
#DEFINE GDK_KEY_braille_dots_2458 16787610
#DEFINE GDK_KEY_braille_dots_246 16787498
#DEFINE GDK_KEY_braille_dots_2467 16787562
#DEFINE GDK_KEY_braille_dots_24678 16787690
#DEFINE GDK_KEY_braille_dots_2468 16787626
#DEFINE GDK_KEY_braille_dots_247 16787530
#DEFINE GDK_KEY_braille_dots_2478 16787658
#DEFINE GDK_KEY_braille_dots_248 16787594
#DEFINE GDK_KEY_braille_dots_25 16787474
#DEFINE GDK_KEY_braille_dots_256 16787506
#DEFINE GDK_KEY_braille_dots_2567 16787570
#DEFINE GDK_KEY_braille_dots_25678 16787698
#DEFINE GDK_KEY_braille_dots_2568 16787634
#DEFINE GDK_KEY_braille_dots_257 16787538
#DEFINE GDK_KEY_braille_dots_2578 16787666
#DEFINE GDK_KEY_braille_dots_258 16787602
#DEFINE GDK_KEY_braille_dots_26 16787490
#DEFINE GDK_KEY_braille_dots_267 16787554
#DEFINE GDK_KEY_braille_dots_2678 16787682
#DEFINE GDK_KEY_braille_dots_268 16787618
#DEFINE GDK_KEY_braille_dots_27 16787522
#DEFINE GDK_KEY_braille_dots_278 16787650
#DEFINE GDK_KEY_braille_dots_28 16787586
#DEFINE GDK_KEY_braille_dots_3 16787460
#DEFINE GDK_KEY_braille_dots_34 16787468
#DEFINE GDK_KEY_braille_dots_345 16787484
#DEFINE GDK_KEY_braille_dots_3456 16787516
#DEFINE GDK_KEY_braille_dots_34567 16787580
#DEFINE GDK_KEY_braille_dots_345678 16787708
#DEFINE GDK_KEY_braille_dots_34568 16787644
#DEFINE GDK_KEY_braille_dots_3457 16787548
#DEFINE GDK_KEY_braille_dots_34578 16787676
#DEFINE GDK_KEY_braille_dots_3458 16787612
#DEFINE GDK_KEY_braille_dots_346 16787500
#DEFINE GDK_KEY_braille_dots_3467 16787564
#DEFINE GDK_KEY_braille_dots_34678 16787692
#DEFINE GDK_KEY_braille_dots_3468 16787628
#DEFINE GDK_KEY_braille_dots_347 16787532
#DEFINE GDK_KEY_braille_dots_3478 16787660
#DEFINE GDK_KEY_braille_dots_348 16787596
#DEFINE GDK_KEY_braille_dots_35 16787476
#DEFINE GDK_KEY_braille_dots_356 16787508
#DEFINE GDK_KEY_braille_dots_3567 16787572
#DEFINE GDK_KEY_braille_dots_35678 16787700
#DEFINE GDK_KEY_braille_dots_3568 16787636
#DEFINE GDK_KEY_braille_dots_357 16787540
#DEFINE GDK_KEY_braille_dots_3578 16787668
#DEFINE GDK_KEY_braille_dots_358 16787604
#DEFINE GDK_KEY_braille_dots_36 16787492
#DEFINE GDK_KEY_braille_dots_367 16787556
#DEFINE GDK_KEY_braille_dots_3678 16787684
#DEFINE GDK_KEY_braille_dots_368 16787620
#DEFINE GDK_KEY_braille_dots_37 16787524
#DEFINE GDK_KEY_braille_dots_378 16787652
#DEFINE GDK_KEY_braille_dots_38 16787588
#DEFINE GDK_KEY_braille_dots_4 16787464
#DEFINE GDK_KEY_braille_dots_45 16787480
#DEFINE GDK_KEY_braille_dots_456 16787512
#DEFINE GDK_KEY_braille_dots_4567 16787576
#DEFINE GDK_KEY_braille_dots_45678 16787704
#DEFINE GDK_KEY_braille_dots_4568 16787640
#DEFINE GDK_KEY_braille_dots_457 16787544
#DEFINE GDK_KEY_braille_dots_4578 16787672
#DEFINE GDK_KEY_braille_dots_458 16787608
#DEFINE GDK_KEY_braille_dots_46 16787496
#DEFINE GDK_KEY_braille_dots_467 16787560
#DEFINE GDK_KEY_braille_dots_4678 16787688
#DEFINE GDK_KEY_braille_dots_468 16787624
#DEFINE GDK_KEY_braille_dots_47 16787528
#DEFINE GDK_KEY_braille_dots_478 16787656
#DEFINE GDK_KEY_braille_dots_48 16787592
#DEFINE GDK_KEY_braille_dots_5 16787472
#DEFINE GDK_KEY_braille_dots_56 16787504
#DEFINE GDK_KEY_braille_dots_567 16787568
#DEFINE GDK_KEY_braille_dots_5678 16787696
#DEFINE GDK_KEY_braille_dots_568 16787632
#DEFINE GDK_KEY_braille_dots_57 16787536
#DEFINE GDK_KEY_braille_dots_578 16787664
#DEFINE GDK_KEY_braille_dots_58 16787600
#DEFINE GDK_KEY_braille_dots_6 16787488
#DEFINE GDK_KEY_braille_dots_67 16787552
#DEFINE GDK_KEY_braille_dots_678 16787680
#DEFINE GDK_KEY_braille_dots_68 16787616
#DEFINE GDK_KEY_braille_dots_7 16787520
#DEFINE GDK_KEY_braille_dots_78 16787648
#DEFINE GDK_KEY_braille_dots_8 16787584
#DEFINE GDK_KEY_breve 418
#DEFINE GDK_KEY_brokenbar 166
#DEFINE GDK_KEY_c 99
#DEFINE GDK_KEY_cabovedot 741
#DEFINE GDK_KEY_cacute 486
#DEFINE GDK_KEY_careof 2744
#DEFINE GDK_KEY_caret 2812
#DEFINE GDK_KEY_caron 439
#DEFINE GDK_KEY_ccaron 488
#DEFINE GDK_KEY_ccedilla 231
#DEFINE GDK_KEY_ccircumflex 742
#DEFINE GDK_KEY_cedilla 184
#DEFINE GDK_KEY_cent 162
#DEFINE GDK_KEY_checkerboard 2529
#DEFINE GDK_KEY_checkmark 2803
#DEFINE GDK_KEY_circle 3023
#DEFINE GDK_KEY_club 2796
#DEFINE GDK_KEY_colon 58
#DEFINE GDK_KEY_comma 44
#DEFINE GDK_KEY_containsas 16785931
#DEFINE GDK_KEY_copyright 169
#DEFINE GDK_KEY_cr 2532
#DEFINE GDK_KEY_crossinglines 2542
#DEFINE GDK_KEY_cuberoot 16785947
#DEFINE GDK_KEY_currency 164
#DEFINE GDK_KEY_cursor 2815
#DEFINE GDK_KEY_d 100
#DEFINE GDK_KEY_dabovedot 16784907
#DEFINE GDK_KEY_dagger 2801
#DEFINE GDK_KEY_dcaron 495
#DEFINE GDK_KEY_dead_A_ 65153
#DEFINE GDK_KEY_dead_E_ 65155
#DEFINE GDK_KEY_dead_I_ 65157
#DEFINE GDK_KEY_dead_O_ 65159
#DEFINE GDK_KEY_dead_U_ 65161
#DEFINE GDK_KEY_dead_a 65152
#DEFINE GDK_KEY_dead_abovecomma 65124
#DEFINE GDK_KEY_dead_abovedot 65110
#DEFINE GDK_KEY_dead_abovereversedcomma 65125
#DEFINE GDK_KEY_dead_abovering 65112
#DEFINE GDK_KEY_dead_acute 65105
#DEFINE GDK_KEY_dead_belowbreve 65131
#DEFINE GDK_KEY_dead_belowcircumflex 65129
#DEFINE GDK_KEY_dead_belowcomma 65134
#DEFINE GDK_KEY_dead_belowdiaeresis 65132
#DEFINE GDK_KEY_dead_belowdot 65120
#DEFINE GDK_KEY_dead_belowmacron 65128
#DEFINE GDK_KEY_dead_belowring 65127
#DEFINE GDK_KEY_dead_belowtilde 65130
#DEFINE GDK_KEY_dead_breve 65109
#DEFINE GDK_KEY_dead_capital_schwa 65163
#DEFINE GDK_KEY_dead_caron 65114
#DEFINE GDK_KEY_dead_cedilla 65115
#DEFINE GDK_KEY_dead_circumflex 65106
#DEFINE GDK_KEY_dead_currency 65135
#DEFINE GDK_KEY_dead_dasia 65125
#DEFINE GDK_KEY_dead_diaeresis 65111
#DEFINE GDK_KEY_dead_doubleacute 65113
#DEFINE GDK_KEY_dead_doublegrave 65126
#DEFINE GDK_KEY_dead_e 65154
#DEFINE GDK_KEY_dead_grave 65104
#DEFINE GDK_KEY_dead_hook 65121
#DEFINE GDK_KEY_dead_horn 65122
#DEFINE GDK_KEY_dead_i 65156
#DEFINE GDK_KEY_dead_invertedbreve 65133
#DEFINE GDK_KEY_dead_iota 65117
#DEFINE GDK_KEY_dead_macron 65108
#DEFINE GDK_KEY_dead_o 65158
#DEFINE GDK_KEY_dead_ogonek 65116
#DEFINE GDK_KEY_dead_perispomeni 65107
#DEFINE GDK_KEY_dead_psili 65124
#DEFINE GDK_KEY_dead_semivoiced_sound 65119
#DEFINE GDK_KEY_dead_small_schwa 65162
#DEFINE GDK_KEY_dead_stroke 65123
#DEFINE GDK_KEY_dead_tilde 65107
#DEFINE GDK_KEY_dead_u 65160
#DEFINE GDK_KEY_dead_voiced_sound 65118
#DEFINE GDK_KEY_decimalpoint 2749
#DEFINE GDK_KEY_degree 176
#DEFINE GDK_KEY_diaeresis 168
#DEFINE GDK_KEY_diamond 2797
#DEFINE GDK_KEY_digitspace 2725
#DEFINE GDK_KEY_dintegral 16785964
#DEFINE GDK_KEY_division 247
#DEFINE GDK_KEY_dollar 36
#DEFINE GDK_KEY_doubbaselinedot 2735
#DEFINE GDK_KEY_doubleacute 445
#DEFINE GDK_KEY_doubledagger 2802
#DEFINE GDK_KEY_doublelowquotemark 2814
#DEFINE GDK_KEY_downarrow 2302
#DEFINE GDK_KEY_downcaret 2984
#DEFINE GDK_KEY_downshoe 3030
#DEFINE GDK_KEY_downstile 3012
#DEFINE GDK_KEY_downtack 3010
#DEFINE GDK_KEY_dstroke 496
#DEFINE GDK_KEY_e 101
#DEFINE GDK_KEY_eabovedot 1004
#DEFINE GDK_KEY_eacute 233
#DEFINE GDK_KEY_ebelowdot 16785081
#DEFINE GDK_KEY_ecaron 492
#DEFINE GDK_KEY_ecircumflex 234
#DEFINE GDK_KEY_ecircumflexacute 16785087
#DEFINE GDK_KEY_ecircumflexbelowdot 16785095
#DEFINE GDK_KEY_ecircumflexgrave 16785089
#DEFINE GDK_KEY_ecircumflexhook 16785091
#DEFINE GDK_KEY_ecircumflextilde 16785093
#DEFINE GDK_KEY_ediaeresis 235
#DEFINE GDK_KEY_egrave 232
#DEFINE GDK_KEY_ehook 16785083
#DEFINE GDK_KEY_eightsubscript 16785544
#DEFINE GDK_KEY_eightsuperior 16785528
#DEFINE GDK_KEY_elementof 16785928
#DEFINE GDK_KEY_ellipsis 2734
#DEFINE GDK_KEY_em3space 2723
#DEFINE GDK_KEY_em4space 2724
#DEFINE GDK_KEY_emacron 954
#DEFINE GDK_KEY_emdash 2729
#DEFINE GDK_KEY_emfilledcircle 2782
#DEFINE GDK_KEY_emfilledrect 2783
#DEFINE GDK_KEY_emopencircle 2766
#DEFINE GDK_KEY_emopenrectangle 2767
#DEFINE GDK_KEY_emptyset 16785925
#DEFINE GDK_KEY_emspace 2721
#DEFINE GDK_KEY_endash 2730
#DEFINE GDK_KEY_enfilledcircbullet 2790
#DEFINE GDK_KEY_enfilledsqbullet 2791
#DEFINE GDK_KEY_eng 959
#DEFINE GDK_KEY_enopencircbullet 2784
#DEFINE GDK_KEY_enopensquarebullet 2785
#DEFINE GDK_KEY_enspace 2722
#DEFINE GDK_KEY_eogonek 490
#DEFINE GDK_KEY_equal 61
#DEFINE GDK_KEY_eth 240
#DEFINE GDK_KEY_etilde 16785085
#DEFINE GDK_KEY_exclam 33
#DEFINE GDK_KEY_exclamdown 161
#DEFINE GDK_KEY_f 102
#DEFINE GDK_KEY_fabovedot 16784927
#DEFINE GDK_KEY_femalesymbol 2808
#DEFINE GDK_KEY_ff 2531
#DEFINE GDK_KEY_figdash 2747
#DEFINE GDK_KEY_filledlefttribullet 2780
#DEFINE GDK_KEY_filledrectbullet 2779
#DEFINE GDK_KEY_filledrighttribullet 2781
#DEFINE GDK_KEY_filledtribulletdown 2793
#DEFINE GDK_KEY_filledtribulletup 2792
#DEFINE GDK_KEY_fiveeighths 2757
#DEFINE GDK_KEY_fivesixths 2743
#DEFINE GDK_KEY_fivesubscript 16785541
#DEFINE GDK_KEY_fivesuperior 16785525
#DEFINE GDK_KEY_fourfifths 2741
#DEFINE GDK_KEY_foursubscript 16785540
#DEFINE GDK_KEY_foursuperior 16785524
#DEFINE GDK_KEY_fourthroot 16785948
#DEFINE GDK_KEY_function 2294
#DEFINE GDK_KEY_g 103
#DEFINE GDK_KEY_gabovedot 757
#DEFINE GDK_KEY_gbreve 699
#DEFINE GDK_KEY_gcaron 16777703
#DEFINE GDK_KEY_gcedilla 955
#DEFINE GDK_KEY_gcircumflex 760
#DEFINE GDK_KEY_grave 96
#DEFINE GDK_KEY_greater 62
#DEFINE GDK_KEY_greaterthanequal 2238
#DEFINE GDK_KEY_guillemotleft 171
#DEFINE GDK_KEY_guillemotright 187
#DEFINE GDK_KEY_h 104
#DEFINE GDK_KEY_hairspace 2728
#DEFINE GDK_KEY_hcircumflex 694
#DEFINE GDK_KEY_heart 2798
#DEFINE GDK_KEY_hebrew_aleph 3296
#DEFINE GDK_KEY_hebrew_ayin 3314
#DEFINE GDK_KEY_hebrew_bet 3297
#DEFINE GDK_KEY_hebrew_beth 3297
#DEFINE GDK_KEY_hebrew_chet 3303
#DEFINE GDK_KEY_hebrew_dalet 3299
#DEFINE GDK_KEY_hebrew_daleth 3299
#DEFINE GDK_KEY_hebrew_doublelowline 3295
#DEFINE GDK_KEY_hebrew_finalkaph 3306
#DEFINE GDK_KEY_hebrew_finalmem 3309
#DEFINE GDK_KEY_hebrew_finalnun 3311
#DEFINE GDK_KEY_hebrew_finalpe 3315
#DEFINE GDK_KEY_hebrew_finalzade 3317
#DEFINE GDK_KEY_hebrew_finalzadi 3317
#DEFINE GDK_KEY_hebrew_gimel 3298
#DEFINE GDK_KEY_hebrew_gimmel 3298
#DEFINE GDK_KEY_hebrew_he 3300
#DEFINE GDK_KEY_hebrew_het 3303
#DEFINE GDK_KEY_hebrew_kaph 3307
#DEFINE GDK_KEY_hebrew_kuf 3319
#DEFINE GDK_KEY_hebrew_lamed 3308
#DEFINE GDK_KEY_hebrew_mem 3310
#DEFINE GDK_KEY_hebrew_nun 3312
#DEFINE GDK_KEY_hebrew_pe 3316
#DEFINE GDK_KEY_hebrew_qoph 3319
#DEFINE GDK_KEY_hebrew_resh 3320
#DEFINE GDK_KEY_hebrew_samech 3313
#DEFINE GDK_KEY_hebrew_samekh 3313
#DEFINE GDK_KEY_hebrew_shin 3321
#DEFINE GDK_KEY_hebrew_taf 3322
#DEFINE GDK_KEY_hebrew_taw 3322
#DEFINE GDK_KEY_hebrew_tet 3304
#DEFINE GDK_KEY_hebrew_teth 3304
#DEFINE GDK_KEY_hebrew_waw 3301
#DEFINE GDK_KEY_hebrew_yod 3305
#DEFINE GDK_KEY_hebrew_zade 3318
#DEFINE GDK_KEY_hebrew_zadi 3318
#DEFINE GDK_KEY_hebrew_zain 3302
#DEFINE GDK_KEY_hebrew_zayin 3302
#DEFINE GDK_KEY_hexagram 2778
#DEFINE GDK_KEY_horizconnector 2211
#DEFINE GDK_KEY_horizlinescan1 2543
#DEFINE GDK_KEY_horizlinescan3 2544
#DEFINE GDK_KEY_horizlinescan5 2545
#DEFINE GDK_KEY_horizlinescan7 2546
#DEFINE GDK_KEY_horizlinescan9 2547
#DEFINE GDK_KEY_hstroke 689
#DEFINE GDK_KEY_ht 2530
#DEFINE GDK_KEY_hyphen 173
#DEFINE GDK_KEY_i 105
#DEFINE GDK_KEY_iTouch 269025120
#DEFINE GDK_KEY_iacute 237
#DEFINE GDK_KEY_ibelowdot 16785099
#DEFINE GDK_KEY_ibreve 16777517
#DEFINE GDK_KEY_icircumflex 238
#DEFINE GDK_KEY_identical 2255
#DEFINE GDK_KEY_idiaeresis 239
#DEFINE GDK_KEY_idotless 697
#DEFINE GDK_KEY_ifonlyif 2253
#DEFINE GDK_KEY_igrave 236
#DEFINE GDK_KEY_ihook 16785097
#DEFINE GDK_KEY_imacron 1007
#DEFINE GDK_KEY_implies 2254
#DEFINE GDK_KEY_includedin 2266
#DEFINE GDK_KEY_includes 2267
#DEFINE GDK_KEY_infinity 2242
#DEFINE GDK_KEY_integral 2239
#DEFINE GDK_KEY_intersection 2268
#DEFINE GDK_KEY_iogonek 999
#DEFINE GDK_KEY_itilde 949
#DEFINE GDK_KEY_j 106
#DEFINE GDK_KEY_jcircumflex 700
#DEFINE GDK_KEY_jot 3018
#DEFINE GDK_KEY_k 107
#DEFINE GDK_KEY_kana_A_ 1201
#DEFINE GDK_KEY_kana_CHI 1217
#DEFINE GDK_KEY_kana_E_ 1204
#DEFINE GDK_KEY_kana_FU 1228
#DEFINE GDK_KEY_kana_HA 1226
#DEFINE GDK_KEY_kana_HE 1229
#DEFINE GDK_KEY_kana_HI 1227
#DEFINE GDK_KEY_kana_HO 1230
#DEFINE GDK_KEY_kana_HU 1228
#DEFINE GDK_KEY_kana_I_ 1202
#DEFINE GDK_KEY_kana_KA 1206
#DEFINE GDK_KEY_kana_KE 1209
#DEFINE GDK_KEY_kana_KI 1207
#DEFINE GDK_KEY_kana_KO 1210
#DEFINE GDK_KEY_kana_KU 1208
#DEFINE GDK_KEY_kana_MA 1231
#DEFINE GDK_KEY_kana_ME 1234
#DEFINE GDK_KEY_kana_MI 1232
#DEFINE GDK_KEY_kana_MO 1235
#DEFINE GDK_KEY_kana_MU 1233
#DEFINE GDK_KEY_kana_N 1245
#DEFINE GDK_KEY_kana_NA 1221
#DEFINE GDK_KEY_kana_NE 1224
#DEFINE GDK_KEY_kana_NI 1222
#DEFINE GDK_KEY_kana_NO 1225
#DEFINE GDK_KEY_kana_NU 1223
#DEFINE GDK_KEY_kana_O_ 1205
#DEFINE GDK_KEY_kana_RA 1239
#DEFINE GDK_KEY_kana_RE 1242
#DEFINE GDK_KEY_kana_RI 1240
#DEFINE GDK_KEY_kana_RO 1243
#DEFINE GDK_KEY_kana_RU 1241
#DEFINE GDK_KEY_kana_SA 1211
#DEFINE GDK_KEY_kana_SE 1214
#DEFINE GDK_KEY_kana_SHI 1212
#DEFINE GDK_KEY_kana_SO 1215
#DEFINE GDK_KEY_kana_SU 1213
#DEFINE GDK_KEY_kana_TA 1216
#DEFINE GDK_KEY_kana_TE 1219
#DEFINE GDK_KEY_kana_TI 1217
#DEFINE GDK_KEY_kana_TO 1220
#DEFINE GDK_KEY_kana_TSU_ 1218
#DEFINE GDK_KEY_kana_TU_ 1218
#DEFINE GDK_KEY_kana_U_ 1203
#DEFINE GDK_KEY_kana_WA 1244
#DEFINE GDK_KEY_kana_WO 1190
#DEFINE GDK_KEY_kana_YA_ 1236
#DEFINE GDK_KEY_kana_YO_ 1238
#DEFINE GDK_KEY_kana_YU_ 1237
#DEFINE GDK_KEY_kana_a 1191
#DEFINE GDK_KEY_kana_closingbracket 1187
#DEFINE GDK_KEY_kana_comma 1188
#DEFINE GDK_KEY_kana_conjunctive 1189
#DEFINE GDK_KEY_kana_e 1194
#DEFINE GDK_KEY_kana_fullstop 1185
#DEFINE GDK_KEY_kana_i 1192
#DEFINE GDK_KEY_kana_middledot 1189
#DEFINE GDK_KEY_kana_o 1195
#DEFINE GDK_KEY_kana_openingbracket 1186
#DEFINE GDK_KEY_kana_switch 65406
#DEFINE GDK_KEY_kana_tsu 1199
#DEFINE GDK_KEY_kana_tu 1199
#DEFINE GDK_KEY_kana_u 1193
#DEFINE GDK_KEY_kana_ya 1196
#DEFINE GDK_KEY_kana_yo 1198
#DEFINE GDK_KEY_kana_yu 1197
#DEFINE GDK_KEY_kappa 930
#DEFINE GDK_KEY_kcedilla 1011
#DEFINE GDK_KEY_kra 930
#DEFINE GDK_KEY_l 108
#DEFINE GDK_KEY_lacute 485
#DEFINE GDK_KEY_latincross 2777
#DEFINE GDK_KEY_lbelowdot 16784951
#DEFINE GDK_KEY_lcaron 437
#DEFINE GDK_KEY_lcedilla 950
#DEFINE GDK_KEY_leftanglebracket 2748
#DEFINE GDK_KEY_leftarrow 2299
#DEFINE GDK_KEY_leftcaret 2979
#DEFINE GDK_KEY_leftdoublequotemark 2770
#DEFINE GDK_KEY_leftmiddlecurlybrace 2223
#DEFINE GDK_KEY_leftopentriangle 2764
#DEFINE GDK_KEY_leftpointer 2794
#DEFINE GDK_KEY_leftradical 2209
#DEFINE GDK_KEY_leftshoe 3034
#DEFINE GDK_KEY_leftsinglequotemark 2768
#DEFINE GDK_KEY_leftt 2548
#DEFINE GDK_KEY_lefttack 3036
#DEFINE GDK_KEY_less 60
#DEFINE GDK_KEY_lessthanequal 2236
#DEFINE GDK_KEY_lf 2533
#DEFINE GDK_KEY_logicaland 2270
#DEFINE GDK_KEY_logicalor 2271
#DEFINE GDK_KEY_lowleftcorner 2541
#DEFINE GDK_KEY_lowrightcorner 2538
#DEFINE GDK_KEY_lstroke 435
#DEFINE GDK_KEY_m 109
#DEFINE GDK_KEY_mabovedot 16784961
#DEFINE GDK_KEY_macron 175
#DEFINE GDK_KEY_malesymbol 2807
#DEFINE GDK_KEY_maltesecross 2800
#DEFINE GDK_KEY_marker 2751
#DEFINE GDK_KEY_masculine 186
#DEFINE GDK_KEY_minus 45
#DEFINE GDK_KEY_minutes 2774
#DEFINE GDK_KEY_mu 181
#DEFINE GDK_KEY_multiply 215
#DEFINE GDK_KEY_musicalflat 2806
#DEFINE GDK_KEY_musicalsharp 2805
#DEFINE GDK_KEY_n 110
#DEFINE GDK_KEY_nabla 2245
#DEFINE GDK_KEY_nacute 497
#DEFINE GDK_KEY_ncaron 498
#DEFINE GDK_KEY_ncedilla 1009
#DEFINE GDK_KEY_ninesubscript 16785545
#DEFINE GDK_KEY_ninesuperior 16785529
#DEFINE GDK_KEY_nl 2536
#DEFINE GDK_KEY_nobreakspace 160
#DEFINE GDK_KEY_notapproxeq 16785991
#DEFINE GDK_KEY_notelementof 16785929
#DEFINE GDK_KEY_notequal 2237
#DEFINE GDK_KEY_notidentical 16786018
#DEFINE GDK_KEY_notsign 172
#DEFINE GDK_KEY_ntilde 241
#DEFINE GDK_KEY_numbersign 35
#DEFINE GDK_KEY_numerosign 1712
#DEFINE GDK_KEY_o 111
#DEFINE GDK_KEY_oacute 243
#DEFINE GDK_KEY_obarred 16777845
#DEFINE GDK_KEY_obelowdot 16785101
#DEFINE GDK_KEY_ocaron 16777682
#DEFINE GDK_KEY_ocircumflex 244
#DEFINE GDK_KEY_ocircumflexacute 16785105
#DEFINE GDK_KEY_ocircumflexbelowdot 16785113
#DEFINE GDK_KEY_ocircumflexgrave 16785107
#DEFINE GDK_KEY_ocircumflexhook 16785109
#DEFINE GDK_KEY_ocircumflextilde 16785111
#DEFINE GDK_KEY_odiaeresis 246
#DEFINE GDK_KEY_odoubleacute 501
#DEFINE GDK_KEY_oe 5053
#DEFINE GDK_KEY_ogonek 434
#DEFINE GDK_KEY_ograve 242
#DEFINE GDK_KEY_ohook 16785103
#DEFINE GDK_KEY_ohorn 16777633
#DEFINE GDK_KEY_ohornacute 16785115
#DEFINE GDK_KEY_ohornbelowdot 16785123
#DEFINE GDK_KEY_ohorngrave 16785117
#DEFINE GDK_KEY_ohornhook 16785119
#DEFINE GDK_KEY_ohorntilde 16785121
#DEFINE GDK_KEY_omacron 1010
#DEFINE GDK_KEY_oneeighth 2755
#DEFINE GDK_KEY_onefifth 2738
#DEFINE GDK_KEY_onehalf 189
#DEFINE GDK_KEY_onequarter 188
#DEFINE GDK_KEY_onesixth 2742
#DEFINE GDK_KEY_onesubscript 16785537
#DEFINE GDK_KEY_onesuperior 185
#DEFINE GDK_KEY_onethird 2736
#DEFINE GDK_KEY_ooblique 248
#DEFINE GDK_KEY_openrectbullet 2786
#DEFINE GDK_KEY_openstar 2789
#DEFINE GDK_KEY_opentribulletdown 2788
#DEFINE GDK_KEY_opentribulletup 2787
#DEFINE GDK_KEY_ordfeminine 170
#DEFINE GDK_KEY_oslash 248
#DEFINE GDK_KEY_otilde 245
#DEFINE GDK_KEY_overbar 3008
#DEFINE GDK_KEY_overline 1150
#DEFINE GDK_KEY_p 112
#DEFINE GDK_KEY_pabovedot 16784983
#DEFINE GDK_KEY_paragraph 182
#DEFINE GDK_KEY_parenleft 40
#DEFINE GDK_KEY_parenright 41
#DEFINE GDK_KEY_partdifferential 16785922
#DEFINE GDK_KEY_partialderivative 2287
#DEFINE GDK_KEY_percent 37
#DEFINE GDK_KEY_period 46
#DEFINE GDK_KEY_periodcentered 183
#DEFINE GDK_KEY_phonographcopyright 2811
#DEFINE GDK_KEY_plus 43
#DEFINE GDK_KEY_plusminus 177
#DEFINE GDK_KEY_prescription 2772
#DEFINE GDK_KEY_prolongedsound 1200
#DEFINE GDK_KEY_punctspace 2726
#DEFINE GDK_KEY_q 113
#DEFINE GDK_KEY_quad 3020
#DEFINE GDK_KEY_question 63
#DEFINE GDK_KEY_questiondown 191
#DEFINE GDK_KEY_quotedbl 34
#DEFINE GDK_KEY_quoteleft 96
#DEFINE GDK_KEY_quoteright 39
#DEFINE GDK_KEY_r 114
#DEFINE GDK_KEY_racute 480
#DEFINE GDK_KEY_radical 2262
#DEFINE GDK_KEY_rcaron 504
#DEFINE GDK_KEY_rcedilla 947
#DEFINE GDK_KEY_registered 174
#DEFINE GDK_KEY_rightanglebracket 2750
#DEFINE GDK_KEY_rightarrow 2301
#DEFINE GDK_KEY_rightcaret 2982
#DEFINE GDK_KEY_rightdoublequotemark 2771
#DEFINE GDK_KEY_rightmiddlecurlybrace 2224
#DEFINE GDK_KEY_rightmiddlesummation 2231
#DEFINE GDK_KEY_rightopentriangle 2765
#DEFINE GDK_KEY_rightpointer 2795
#DEFINE GDK_KEY_rightshoe 3032
#DEFINE GDK_KEY_rightsinglequotemark 2769
#DEFINE GDK_KEY_rightt 2549
#DEFINE GDK_KEY_righttack 3068
#DEFINE GDK_KEY_s 115
#DEFINE GDK_KEY_sabovedot 16784993
#DEFINE GDK_KEY_sacute 438
#DEFINE GDK_KEY_scaron 441
#DEFINE GDK_KEY_scedilla 442
#DEFINE GDK_KEY_schwa 16777817
#DEFINE GDK_KEY_scircumflex 766
#DEFINE GDK_KEY_script_switch 65406
#DEFINE GDK_KEY_seconds 2775
#DEFINE GDK_KEY_section 167
#DEFINE GDK_KEY_semicolon 59
#DEFINE GDK_KEY_semivoicedsound 1247
#DEFINE GDK_KEY_seveneighths 2758
#DEFINE GDK_KEY_sevensubscript 16785543
#DEFINE GDK_KEY_sevensuperior 16785527
#DEFINE GDK_KEY_signaturemark 2762
#DEFINE GDK_KEY_signifblank 2732
#DEFINE GDK_KEY_similarequal 2249
#DEFINE GDK_KEY_singlelowquotemark 2813
#DEFINE GDK_KEY_sixsubscript 16785542
#DEFINE GDK_KEY_sixsuperior 16785526
#DEFINE GDK_KEY_slash 47
#DEFINE GDK_KEY_soliddiamond 2528
#DEFINE GDK_KEY_space 32
#DEFINE GDK_KEY_squareroot 16785946
#DEFINE GDK_KEY_ssharp 223
#DEFINE GDK_KEY_sterling 163
#DEFINE GDK_KEY_stricteq 16786019
#DEFINE GDK_KEY_t 116
#DEFINE GDK_KEY_tabovedot 16785003
#DEFINE GDK_KEY_tcaron 443
#DEFINE GDK_KEY_tcedilla 510
#DEFINE GDK_KEY_telephone 2809
#DEFINE GDK_KEY_telephonerecorder 2810
#DEFINE GDK_KEY_therefore 2240
#DEFINE GDK_KEY_thinspace 2727
#DEFINE GDK_KEY_thorn 254
#DEFINE GDK_KEY_threeeighths 2756
#DEFINE GDK_KEY_threefifths 2740
#DEFINE GDK_KEY_threequarters 190
#DEFINE GDK_KEY_threesubscript 16785539
#DEFINE GDK_KEY_threesuperior 179
#DEFINE GDK_KEY_tintegral 16785965
#DEFINE GDK_KEY_topintegral 2212
#DEFINE GDK_KEY_topleftparens 2219
#DEFINE GDK_KEY_topleftradical 2210
#DEFINE GDK_KEY_topleftsqbracket 2215
#DEFINE GDK_KEY_topleftsummation 2225
#DEFINE GDK_KEY_toprightparens 2221
#DEFINE GDK_KEY_toprightsqbracket 2217
#DEFINE GDK_KEY_toprightsummation 2229
#DEFINE GDK_KEY_topt 2551
#DEFINE GDK_KEY_topvertsummationconnector 2227
#DEFINE GDK_KEY_trademark 2761
#DEFINE GDK_KEY_trademarkincircle 2763
#DEFINE GDK_KEY_tslash 956
#DEFINE GDK_KEY_twofifths 2739
#DEFINE GDK_KEY_twosubscript 16785538
#DEFINE GDK_KEY_twosuperior 178
#DEFINE GDK_KEY_twothirds 2737
#DEFINE GDK_KEY_u 117
#DEFINE GDK_KEY_uacute 250
#DEFINE GDK_KEY_ubelowdot 16785125
#DEFINE GDK_KEY_ubreve 765
#DEFINE GDK_KEY_ucircumflex 251
#DEFINE GDK_KEY_udiaeresis 252
#DEFINE GDK_KEY_udoubleacute 507
#DEFINE GDK_KEY_ugrave 249
#DEFINE GDK_KEY_uhook 16785127
#DEFINE GDK_KEY_uhorn 16777648
#DEFINE GDK_KEY_uhornacute 16785129
#DEFINE GDK_KEY_uhornbelowdot 16785137
#DEFINE GDK_KEY_uhorngrave 16785131
#DEFINE GDK_KEY_uhornhook 16785133
#DEFINE GDK_KEY_uhorntilde 16785135
#DEFINE GDK_KEY_umacron 1022
#DEFINE GDK_KEY_underbar 3014
#DEFINE GDK_KEY_underscore 95
#DEFINE GDK_KEY_union 2269
#DEFINE GDK_KEY_uogonek 1017
#DEFINE GDK_KEY_uparrow 2300
#DEFINE GDK_KEY_upcaret 2985
#DEFINE GDK_KEY_upleftcorner 2540
#DEFINE GDK_KEY_uprightcorner 2539
#DEFINE GDK_KEY_upshoe 3011
#DEFINE GDK_KEY_upstile 3027
#DEFINE GDK_KEY_uptack 3022
#DEFINE GDK_KEY_uring 505
#DEFINE GDK_KEY_utilde 1021
#DEFINE GDK_KEY_v 118
#DEFINE GDK_KEY_variation 2241
#DEFINE GDK_KEY_vertbar 2552
#DEFINE GDK_KEY_vertconnector 2214
#DEFINE GDK_KEY_voicedsound 1246
#DEFINE GDK_KEY_vt 2537
#DEFINE GDK_KEY_w 119
#DEFINE GDK_KEY_wacute 16785027
#DEFINE GDK_KEY_wcircumflex 16777589
#DEFINE GDK_KEY_wdiaeresis 16785029
#DEFINE GDK_KEY_wgrave 16785025
#DEFINE GDK_KEY_x 120
#DEFINE GDK_KEY_xabovedot 16785035
#DEFINE GDK_KEY_y 121
#DEFINE GDK_KEY_yacute 253
#DEFINE GDK_KEY_ybelowdot 16785141
#DEFINE GDK_KEY_ycircumflex 16777591
#DEFINE GDK_KEY_ydiaeresis 255
#DEFINE GDK_KEY_yen 165
#DEFINE GDK_KEY_ygrave 16785139
#DEFINE GDK_KEY_yhook 16785143
#DEFINE GDK_KEY_ytilde 16785145
#DEFINE GDK_KEY_z 122
#DEFINE GDK_KEY_zabovedot 447
#DEFINE GDK_KEY_zacute 444
#DEFINE GDK_KEY_zcaron 446
#DEFINE GDK_KEY_zerosubscript 16785536
#DEFINE GDK_KEY_zerosuperior 16785520
#DEFINE GDK_KEY_zstroke 16777654
TYPE AS _GdkKeyboardGrabInfo GdkKeyboardGrabInfo
TYPE AS _GdkKeymap GdkKeymap
TYPE AS _GdkKeymapClass GdkKeymapClass
TYPE AS _GdkKeymapKey GdkKeymapKey
TYPE AS LONG GdkLineStyle
ENUM
  GDK_LINE_SOLID = 0
  GDK_LINE_ON_OFF_DASH = 1
  GDK_LINE_DOUBLE_DASH = 2
END ENUM
#DEFINE GDK_MAX_TIMECOORD_AXES 128
TYPE AS LONG GdkModifierType
ENUM
  GDK_SHIFT_MASK = 1
  GDK_LOCK_MASK = 2
  GDK_CONTROL_MASK = 4
  GDK_MOD1_MASK = 8
  GDK_MOD2_MASK = 16
  GDK_MOD3_MASK = 32
  GDK_MOD4_MASK = 64
  GDK_MOD5_MASK = 128
  GDK_BUTTON1_MASK = 256
  GDK_BUTTON2_MASK = 512
  GDK_BUTTON3_MASK = 1024
  GDK_BUTTON4_MASK = 2048
  GDK_BUTTON5_MASK = 4096
  GDK_SUPER_MASK = 67108864
  GDK_HYPER_MASK = 134217728
  GDK_META_MASK = 268435456
  GDK_RELEASE_MASK = 1073741824
  GDK_MODIFIER_MASK = 1543512063
END ENUM
#DEFINE GDK_NO_BG 2
TYPE AS LONG GdkNotifyType
ENUM
  GDK_NOTIFY_ANCESTOR = 0
  GDK_NOTIFY_VIRTUAL = 1
  GDK_NOTIFY_INFERIOR = 2
  GDK_NOTIFY_NONLINEAR = 3
  GDK_NOTIFY_NONLINEAR_VIRTUAL = 4
  GDK_NOTIFY_UNKNOWN = 5
END ENUM
TYPE AS LONG GdkOverlapType
ENUM
  GDK_OVERLAP_RECTANGLE_IN = 0
  GDK_OVERLAP_RECTANGLE_OUT = 1
  GDK_OVERLAP_RECTANGLE_PART = 2
END ENUM
TYPE AS LONG GdkOwnerChange
ENUM
  GDK_OWNER_CHANGE_NEW_OWNER = 0
  GDK_OWNER_CHANGE_DESTROY = 1
  GDK_OWNER_CHANGE_CLOSE = 2
END ENUM
#DEFINE GDK_PARENT_RELATIVE 1
#DEFINE GDK_PARENT_RELATIVE_BG 1
#DEFINE GDK_PRIORITY_REDRAW 20
TYPE AS _GdkPangoAttrEmbossColor GdkPangoAttrEmbossColor
TYPE AS _GdkPangoAttrEmbossed GdkPangoAttrEmbossed
TYPE AS _GdkPangoAttrStipple GdkPangoAttrStipple
TYPE AS _GdkPangoRenderer GdkPangoRenderer
TYPE AS _GdkPangoRendererClass GdkPangoRendererClass
TYPE AS _GdkPangoRendererPrivate GdkPangoRendererPrivate
TYPE AS _GdkPixmap GdkPixmap
TYPE AS _GdkPixmapObject GdkPixmapObject
TYPE AS _GdkPixmapObjectClass GdkPixmapObjectClass
TYPE AS _GdkPoint GdkPoint
TYPE AS _GdkPointerHooks GdkPointerHooks
TYPE AS _GdkPointerWindowInfo GdkPointerWindowInfo
TYPE AS LONG GdkPropMode
ENUM
  GDK_PROP_MODE_REPLACE = 0
  GDK_PROP_MODE_PREPEND = 1
  GDK_PROP_MODE_APPEND = 2
END ENUM
TYPE AS LONG GdkPropertyState
ENUM
  GDK_PROPERTY_NEW_VALUE = 0
  GDK_PROPERTY_DELETE_ = 1
END ENUM
TYPE AS _GdkRectangle GdkRectangle
TYPE AS _GdkRegion GdkRegion
TYPE AS _GdkRgbCmap GdkRgbCmap
TYPE AS LONG GdkRgbDither
ENUM
  GDK_RGB_DITHER_NONE = 0
  GDK_RGB_DITHER_NORMAL = 1
  GDK_RGB_DITHER_MAX = 2
END ENUM
TYPE AS _GdkScreen GdkScreen
TYPE AS _GdkScreenClass GdkScreenClass
TYPE AS LONG GdkScrollDirection
ENUM
  GDK_SCROLL_UP = 0
  GDK_SCROLL_DOWN = 1
  GDK_SCROLL_LEFT = 2
  GDK_SCROLL_RIGHT = 3
END ENUM
TYPE AS _GdkSegment GdkSegment
TYPE AS LONG GdkSettingAction
ENUM
  GDK_SETTING_ACTION_NEW = 0
  GDK_SETTING_ACTION_CHANGED = 1
  GDK_SETTING_ACTION_DELETED = 2
END ENUM
TYPE AS _GdkSpan GdkSpan
TYPE AS LONG GdkStatus
ENUM
  GDK_OK = 0
  GDK_ERROR = -1
  GDK_ERROR_PARAM = -2
  GDK_ERROR_FILE = -3
  GDK_ERROR_MEM = -4
END ENUM
TYPE AS LONG GdkSubwindowMode
ENUM
  GDK_CLIP_BY_CHILDREN = 0
  GDK_INCLUDE_INFERIORS = 1
END ENUM
TYPE AS _GdkTimeCoord GdkTimeCoord
TYPE AS _GdkTrapezoid GdkTrapezoid
TYPE AS LONG GdkVisibilityState
ENUM
  GDK_VISIBILITY_UNOBSCURED = 0
  GDK_VISIBILITY_PARTIAL = 1
  GDK_VISIBILITY_FULLY_OBSCURED = 2
END ENUM
TYPE AS _GdkVisual GdkVisual
TYPE AS _GdkVisualClass GdkVisualClass
TYPE AS LONG GdkVisualType
ENUM
  GDK_VISUAL_STATIC_GRAY = 0
  GDK_VISUAL_GRAYSCALE = 1
  GDK_VISUAL_STATIC_COLOR = 2
  GDK_VISUAL_PSEUDO_COLOR = 3
  GDK_VISUAL_TRUE_COLOR = 4
  GDK_VISUAL_DIRECT_COLOR = 5
END ENUM
TYPE AS LONG GdkWMDecoration
ENUM
  GDK_DECOR_ALL = 1
  GDK_DECOR_BORDER = 2
  GDK_DECOR_RESIZEH = 4
  GDK_DECOR_TITLE = 8
  GDK_DECOR_MENU = 16
  GDK_DECOR_MINIMIZE = 32
  GDK_DECOR_MAXIMIZE = 64
END ENUM
TYPE AS LONG GdkWMFunction
ENUM
  GDK_FUNC_ALL = 1
  GDK_FUNC_RESIZE = 2
  GDK_FUNC_MOVE = 4
  GDK_FUNC_MINIMIZE = 8
  GDK_FUNC_MAXIMIZE = 16
  GDK_FUNC_CLOSE = 32
END ENUM
TYPE AS _GdkWindow GdkWindow
TYPE AS _GdkWindowAttr GdkWindowAttr
TYPE AS LONG GdkWindowAttributesType
ENUM
  GDK_WA_TITLE = 2
  GDK_WA_X = 4
  GDK_WA_Y = 8
  GDK_WA_CURSOR = 16
  GDK_WA_COLORMAP = 32
  GDK_WA_VISUAL = 64
  GDK_WA_WMCLASS = 128
  GDK_WA_NOREDIR = 256
  GDK_WA_TYPE_HINT = 512
END ENUM
TYPE AS LONG GdkWindowClass
ENUM
  GDK_INPUT_OUTPUT = 0
  GDK_INPUT_ONLY = 1
END ENUM
TYPE AS LONG GdkWindowEdge
ENUM
  GDK_WINDOW_EDGE_NORTH_WEST = 0
  GDK_WINDOW_EDGE_NORTH = 1
  GDK_WINDOW_EDGE_NORTH_EAST = 2
  GDK_WINDOW_EDGE_WEST = 3
  GDK_WINDOW_EDGE_EAST = 4
  GDK_WINDOW_EDGE_SOUTH_WEST = 5
  GDK_WINDOW_EDGE_SOUTH = 6
  GDK_WINDOW_EDGE_SOUTH_EAST = 7
END ENUM
TYPE AS LONG GdkWindowHints
ENUM
  GDK_HINT_POS = 1
  GDK_HINT_MIN_SIZE = 2
  GDK_HINT_MAX_SIZE = 4
  GDK_HINT_BASE_SIZE = 8
  GDK_HINT_ASPECT = 16
  GDK_HINT_RESIZE_INC = 32
  GDK_HINT_WIN_GRAVITY = 64
  GDK_HINT_USER_POS = 128
  GDK_HINT_USER_SIZE = 256
END ENUM
TYPE AS _GdkWindowObject GdkWindowObject
TYPE AS _GdkWindowObjectClass GdkWindowObjectClass
TYPE AS _GdkWindowRedirect GdkWindowRedirect
TYPE AS LONG GdkWindowState
ENUM
  GDK_WINDOW_STATE_WITHDRAWN = 1
  GDK_WINDOW_STATE_ICONIFIED = 2
  GDK_WINDOW_STATE_MAXIMIZED = 4
  GDK_WINDOW_STATE_STICKY = 8
  GDK_WINDOW_STATE_FULLSCREEN = 16
  GDK_WINDOW_STATE_ABOVE = 32
  GDK_WINDOW_STATE_BELOW = 64
END ENUM
TYPE AS LONG GdkWindowType
ENUM
  GDK_WINDOW_ROOT = 0
  GDK_WINDOW_TOPLEVEL = 1
  GDK_WINDOW_CHILD = 2
  GDK_WINDOW_DIALOG = 3
  GDK_WINDOW_TEMP = 4
  GDK_WINDOW_FOREIGN = 5
  GDK_WINDOW_OFFSCREEN = 6
END ENUM
TYPE AS LONG GdkWindowTypeHint
ENUM
  GDK_WINDOW_TYPE_HINT_NORMAL = 0
  GDK_WINDOW_TYPE_HINT_DIALOG = 1
  GDK_WINDOW_TYPE_HINT_MENU = 2
  GDK_WINDOW_TYPE_HINT_TOOLBAR = 3
  GDK_WINDOW_TYPE_HINT_SPLASHSCREEN = 4
  GDK_WINDOW_TYPE_HINT_UTILITY = 5
  GDK_WINDOW_TYPE_HINT_DOCK = 6
  GDK_WINDOW_TYPE_HINT_DESKTOP = 7
  GDK_WINDOW_TYPE_HINT_DROPDOWN_MENU = 8
  GDK_WINDOW_TYPE_HINT_POPUP_MENU = 9
  GDK_WINDOW_TYPE_HINT_TOOLTIP = 10
  GDK_WINDOW_TYPE_HINT_NOTIFICATION = 11
  GDK_WINDOW_TYPE_HINT_COMBO = 12
  GDK_WINDOW_TYPE_HINT_DND = 13
END ENUM
#IFDEF __FB_UNIX__
	EXTERN "C" LIB "gdk-x11-2.0"
#ELSE
	EXTERN "C" LIB "gdk-win32-2.0"
#ENDIF
' P_X

TYPE _GdkRectangle
  AS gint x
  AS gint y
  AS gint width
  AS gint height
END TYPE
DECLARE FUNCTION gdk_rectangle_intersect(BYVAL AS const GdkRectangle PTR, BYVAL AS const GdkRectangle PTR, BYVAL AS GdkRectangle PTR) AS gboolean
DECLARE SUB gdk_rectangle_union(BYVAL AS const GdkRectangle PTR, BYVAL AS const GdkRectangle PTR, BYVAL AS GdkRectangle PTR)
TYPE _GdkKeyboardGrabInfo
  AS GdkWindow PTR window
  AS GdkWindow PTR native_window
  AS gulong serial
  AS gboolean owner_events
  AS guint32 time
END TYPE
DECLARE FUNCTION gdk_keyboard_grab_info_libgtk_only(BYVAL AS GdkDisplay PTR, BYVAL AS GdkWindow PTR PTR, BYVAL AS gboolean PTR) AS gboolean
TYPE _GdkPointerWindowInfo
  AS GdkWindow PTR toplevel_under_pointer
  AS GdkWindow PTR window_under_pointer
  AS gdouble toplevel_x
  AS gdouble toplevel_y
  AS guint32 state
  AS guint32 button
  AS gulong motion_hint_serial
END TYPE
TYPE GdkFilterFunc AS FUNCTION CDECL(BYVAL AS GdkXEvent PTR, BYVAL AS any ptr /'GdkEvent'/ PTR, BYVAL AS gpointer) AS GdkFilterReturn
TYPE _GdkEventAny
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
END TYPE
TYPE _GdkEventExpose
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS GdkRectangle area
  AS GdkRegion PTR region
  AS gint count
END TYPE
TYPE _GdkEventVisibility
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS GdkVisibilityState state
END TYPE
TYPE _GdkEventMotion
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS guint32 time
  AS gdouble x
  AS gdouble y
  AS gdouble PTR axes
  AS guint state
  AS gint16 is_hint
  AS GdkDevice PTR device
  AS gdouble x_root
  AS gdouble y_root
END TYPE
TYPE _GdkEventButton
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS guint32 time
  AS gdouble x
  AS gdouble y
  AS gdouble PTR axes
  AS guint state
  AS guint button
  AS GdkDevice PTR device
  AS gdouble x_root
  AS gdouble y_root
END TYPE
TYPE _GdkEventScroll
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS guint32 time
  AS gdouble x
  AS gdouble y
  AS guint state
  AS GdkScrollDirection direction
  AS GdkDevice PTR device
  AS gdouble x_root
  AS gdouble y_root
END TYPE
TYPE _GdkEventKey
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS guint32 time
  AS guint state
  AS guint keyval
  AS gint length
  AS gchar PTR string
  AS guint16 hardware_keycode
  AS guint8 group
  AS guint is_modifier : 1
END TYPE
TYPE _GdkEventCrossing
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS GdkWindow PTR subwindow
  AS guint32 time
  AS gdouble x
  AS gdouble y
  AS gdouble x_root
  AS gdouble y_root
  AS GdkCrossingMode mode
  AS GdkNotifyType detail
  AS gboolean focus
  AS guint state
END TYPE
TYPE _GdkEventFocus
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS gint16 in
END TYPE
TYPE _GdkEventConfigure
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS gint x
  AS gint y
  AS gint width
  AS gint height
END TYPE
TYPE _GdkEventProperty
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS GdkAtom atom
  AS guint32 time
  AS guint state
END TYPE
TYPE _GdkEventSelection
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS GdkAtom selection
  AS GdkAtom target
  AS GdkAtom property
  AS guint32 time
  AS GdkNativeWindow requestor
END TYPE
TYPE _GdkEventOwnerChange
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS GdkNativeWindow owner
  AS GdkOwnerChange reason
  AS GdkAtom selection
  AS guint32 time
  AS guint32 selection_time
END TYPE
TYPE _GdkEventProximity
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS guint32 time
  AS GdkDevice PTR device
END TYPE
TYPE _GdkEventDND
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS GdkDragContext PTR context
  AS guint32 time
  AS gshort x_root
  AS gshort y_root
END TYPE
TYPE _GdkEventWindowState
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS GdkWindowState changed_mask
  AS GdkWindowState new_window_state
END TYPE
TYPE _GdkEventSetting
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS GdkSettingAction action
  AS char PTR name
END TYPE
TYPE _GdkEventGrabBroken
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS gboolean keyboard
  AS gboolean implicit
  AS GdkWindow PTR grab_window
END TYPE
TYPE _GdkEventNoExpose
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
END TYPE
TYPE _GdkEventClient
  AS GdkEventType type
  AS GdkWindow PTR window
  AS gint8 send_event
  AS GdkAtom message_type
  AS gushort data_format
END TYPE
TYPE GdkSpanFunc AS SUB CDECL(BYVAL AS GdkSpan PTR, BYVAL AS gpointer)
' P_3

TYPE _GdkAppLaunchContext
  AS GAppLaunchContext parent_instance
  AS GdkAppLaunchContextPrivate PTR priv
END TYPE
DECLARE FUNCTION gdk_app_launch_context_get_type() AS GType
#DEFINE GDK_TYPE_APP_LAUNCH_CONTEXT (gdk_app_launch_context_get_type())
#DEFINE GDK_APP_LAUNCH_CONTEXT(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_APP_LAUNCH_CONTEXT, GdkAppLaunchContext))
#DEFINE GDK_APP_LAUNCH_CONTEXT_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_APP_LAUNCH_CONTEXT, GdkAppLaunchContextClass))
#DEFINE GDK_IS_APP_LAUNCH_CONTEXT(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_APP_LAUNCH_CONTEXT))
#DEFINE GDK_IS_CLASS_APP_LAUNCH_CONTEXT(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_APP_LAUNCH_CONTEXT))
#DEFINE GDK_APP_LAUNCH_CONTEXT_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_APP_LAUNCH_CONTEXT, GdkAppLaunchContextClass))
DECLARE FUNCTION gdk_app_launch_context_new() AS GdkAppLaunchContext PTR
DECLARE SUB gdk_app_launch_context_set_desktop(BYVAL AS GdkAppLaunchContext PTR, BYVAL AS gint)
DECLARE SUB gdk_app_launch_context_set_display(BYVAL AS GdkAppLaunchContext PTR, BYVAL AS GdkDisplay PTR)
DECLARE SUB gdk_app_launch_context_set_icon(BYVAL AS GdkAppLaunchContext PTR, BYVAL AS GIcon PTR)
DECLARE SUB gdk_app_launch_context_set_icon_name(BYVAL AS GdkAppLaunchContext PTR, BYVAL AS const char PTR)
DECLARE SUB gdk_app_launch_context_set_screen(BYVAL AS GdkAppLaunchContext PTR, BYVAL AS GdkScreen PTR)
DECLARE SUB gdk_app_launch_context_set_timestamp(BYVAL AS GdkAppLaunchContext PTR, BYVAL AS guint32)
TYPE _GdkAppLaunchContextClass
  AS GAppLaunchContextClass parent_class
END TYPE
DECLARE FUNCTION gdk_atom_name(BYVAL AS GdkAtom) AS gchar PTR
DECLARE FUNCTION gdk_atom_intern(BYVAL AS const gchar PTR, BYVAL AS gboolean) AS GdkAtom
DECLARE FUNCTION gdk_atom_intern_static_string(BYVAL AS const gchar PTR) AS GdkAtom
TYPE _GdkBitmap
  AS GObject parent_instance
END TYPE
DECLARE FUNCTION gdk_bitmap_create_from_data(BYVAL AS GdkDrawable PTR, BYVAL AS const gchar PTR, BYVAL AS gint, BYVAL AS gint) AS GdkBitmap PTR
TYPE _GdkColor
  AS guint32 pixel
  AS guint16 red
  AS guint16 green
  AS guint16 blue
END TYPE
DECLARE FUNCTION gdk_color_copy(BYVAL AS const GdkColor PTR) AS GdkColor PTR
DECLARE FUNCTION gdk_color_equal(BYVAL AS const GdkColor PTR, BYVAL AS const GdkColor PTR) AS gboolean
DECLARE SUB gdk_color_free(BYVAL AS GdkColor PTR)
DECLARE FUNCTION gdk_color_hash(BYVAL AS const GdkColor PTR) AS guint
DECLARE FUNCTION gdk_color_to_string(BYVAL AS const GdkColor PTR) AS gchar PTR
DECLARE FUNCTION gdk_color_alloc(BYVAL AS GdkColormap PTR, BYVAL AS GdkColor PTR) AS gint
DECLARE FUNCTION gdk_color_black(BYVAL AS GdkColormap PTR, BYVAL AS GdkColor PTR) AS gint
DECLARE FUNCTION gdk_color_change(BYVAL AS GdkColormap PTR, BYVAL AS GdkColor PTR) AS gint
DECLARE FUNCTION gdk_color_parse(BYVAL AS const gchar PTR, BYVAL AS GdkColor PTR) AS gboolean
DECLARE FUNCTION gdk_color_white(BYVAL AS GdkColormap PTR, BYVAL AS GdkColor PTR) AS gint
TYPE _GdkColormap
  AS GObject parent_instance
  AS gint size
  AS GdkColor PTR colors
  AS GdkVisual PTR visual
  AS gpointer windowing_data
END TYPE
DECLARE FUNCTION gdk_colormap_get_type() AS GType
#DEFINE GDK_TYPE_COLORMAP (gdk_colormap_get_type())
#DEFINE GDK_COLORMAP(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_COLORMAP, GdkColormap))
#DEFINE GDK_COLORMAP_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_COLORMAP, GdkColormapClass))
#DEFINE GDK_IS_COLORMAP(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_COLORMAP))
#DEFINE GDK_IS_CLASS_COLORMAP(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_COLORMAP))
#DEFINE GDK_COLORMAP_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_COLORMAP, GdkColormapClass))
DECLARE FUNCTION gdk_colormap_new(BYVAL AS GdkVisual PTR, BYVAL AS gboolean) AS GdkColormap PTR
DECLARE FUNCTION gdk_colormap_get_system() AS GdkColormap PTR
DECLARE FUNCTION gdk_colormap_get_system_size() AS gint
DECLARE FUNCTION gdk_colormap_alloc_color(BYVAL AS GdkColormap PTR, BYVAL AS GdkColor PTR, BYVAL AS gboolean, BYVAL AS gboolean) AS gboolean
DECLARE FUNCTION gdk_colormap_alloc_colors(BYVAL AS GdkColormap PTR, BYVAL AS GdkColor PTR, BYVAL AS gint, BYVAL AS gboolean, BYVAL AS gboolean, BYVAL AS gboolean PTR) AS gint
DECLARE SUB gdk_colormap_change(BYVAL AS GdkColormap PTR, BYVAL AS gint)
DECLARE SUB gdk_colormap_free_colors(BYVAL AS GdkColormap PTR, BYVAL AS const GdkColor PTR, BYVAL AS gint)
DECLARE FUNCTION gdk_colormap_get_screen(BYVAL AS GdkColormap PTR) AS GdkScreen PTR
DECLARE FUNCTION gdk_colormap_get_visual(BYVAL AS GdkColormap PTR) AS GdkVisual PTR
DECLARE SUB gdk_colormap_query_color(BYVAL AS GdkColormap PTR, BYVAL AS gulong, BYVAL AS GdkColor PTR)
DECLARE FUNCTION gdk_colormap_ref(BYVAL AS GdkColormap PTR) AS GdkColormap PTR
DECLARE SUB gdk_colormap_unref(BYVAL AS GdkColormap PTR)
TYPE _GdkColormapClass
  AS GObjectClass parent_class
END TYPE
TYPE _GdkCursor
  AS GdkCursorType type
  AS guint ref_count
END TYPE
DECLARE FUNCTION gdk_cursor_new(BYVAL AS GdkCursorType) AS GdkCursor PTR
DECLARE FUNCTION gdk_cursor_new_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkCursorType) AS GdkCursor PTR
DECLARE FUNCTION gdk_cursor_new_from_name(BYVAL AS GdkDisplay PTR, BYVAL AS const gchar PTR) AS GdkCursor PTR
DECLARE FUNCTION gdk_cursor_new_from_pixbuf(BYVAL AS GdkDisplay PTR, BYVAL AS GdkPixbuf PTR, BYVAL AS gint, BYVAL AS gint) AS GdkCursor PTR
DECLARE FUNCTION gdk_cursor_new_from_pixmap(BYVAL AS GdkPixmap PTR, BYVAL AS GdkPixmap PTR, BYVAL AS const GdkColor PTR, BYVAL AS const GdkColor PTR, BYVAL AS gint, BYVAL AS gint) AS GdkCursor PTR
DECLARE FUNCTION gdk_cursor_get_cursor_type(BYVAL AS GdkCursor PTR) AS GdkCursorType
DECLARE FUNCTION gdk_cursor_get_display(BYVAL AS GdkCursor PTR) AS GdkDisplay PTR
DECLARE FUNCTION gdk_cursor_get_image(BYVAL AS GdkCursor PTR) AS GdkPixbuf PTR
DECLARE FUNCTION gdk_cursor_ref(BYVAL AS GdkCursor PTR) AS GdkCursor PTR
DECLARE SUB gdk_cursor_unref(BYVAL AS GdkCursor PTR)
TYPE GdkDestroyNotify AS SUB CDECL(BYVAL AS gpointer)
TYPE _GdkDevice
  AS GObject parent_instance
  AS gchar PTR name
  AS GdkInputSource source
  AS GdkInputMode mode
  AS gboolean has_cursor
  AS gint num_axes
  AS GdkDeviceAxis PTR axes
  AS gint num_keys
  AS GdkDeviceKey PTR keys
END TYPE
DECLARE FUNCTION gdk_device_get_type() AS GType
#DEFINE GDK_TYPE_DEVICE (gdk_device_get_type())
#DEFINE GDK_DEVICE(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_DEVICE, GdkDevice))
#DEFINE GDK_DEVICE_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_DEVICE, GdkDeviceClass))
#DEFINE GDK_IS_DEVICE(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_DEVICE))
#DEFINE GDK_IS_CLASS_DEVICE(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_DEVICE))
#DEFINE GDK_DEVICE_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_DEVICE, GdkDeviceClass))
DECLARE SUB gdk_device_free_history(BYVAL AS GdkTimeCoord PTR PTR, BYVAL AS gint)
DECLARE FUNCTION gdk_device_get_core_pointer() AS GdkDevice PTR
DECLARE FUNCTION gdk_device_get_axis(BYVAL AS GdkDevice PTR, BYVAL AS gdouble PTR, BYVAL AS GdkAxisUse, BYVAL AS gdouble PTR) AS gboolean
DECLARE FUNCTION gdk_device_get_axis_use(BYVAL AS GdkDevice PTR, BYVAL AS guint) AS GdkAxisUse
DECLARE FUNCTION gdk_device_get_has_cursor(BYVAL AS GdkDevice PTR) AS gboolean
DECLARE FUNCTION gdk_device_get_history(BYVAL AS GdkDevice PTR, BYVAL AS GdkWindow PTR, BYVAL AS guint32, BYVAL AS guint32, BYVAL AS GdkTimeCoord PTR PTR PTR, BYVAL AS gint PTR) AS gboolean
DECLARE SUB gdk_device_get_key(BYVAL AS GdkDevice PTR, BYVAL AS guint, BYVAL AS guint PTR, BYVAL AS GdkModifierType PTR)
DECLARE FUNCTION gdk_device_get_mode(BYVAL AS GdkDevice PTR) AS GdkInputMode
DECLARE FUNCTION gdk_device_get_n_axes(BYVAL AS GdkDevice PTR) AS gint
DECLARE FUNCTION gdk_device_get_n_keys(BYVAL AS GdkDevice PTR) AS gint
DECLARE FUNCTION gdk_device_get_name(BYVAL AS GdkDevice PTR) AS const gchar PTR
DECLARE FUNCTION gdk_device_get_source(BYVAL AS GdkDevice PTR) AS GdkInputSource
DECLARE SUB gdk_device_get_state(BYVAL AS GdkDevice PTR, BYVAL AS GdkWindow PTR, BYVAL AS gdouble PTR, BYVAL AS GdkModifierType PTR)
DECLARE SUB gdk_device_set_axis_use(BYVAL AS GdkDevice PTR, BYVAL AS guint, BYVAL AS GdkAxisUse)
DECLARE SUB gdk_device_set_key(BYVAL AS GdkDevice PTR, BYVAL AS guint, BYVAL AS guint, BYVAL AS GdkModifierType)
DECLARE FUNCTION gdk_device_set_mode(BYVAL AS GdkDevice PTR, BYVAL AS GdkInputMode) AS gboolean
DECLARE SUB gdk_device_set_source(BYVAL AS GdkDevice PTR, BYVAL AS GdkInputSource)
TYPE _GdkDeviceAxis
  AS GdkAxisUse use
  AS gdouble min
  AS gdouble max
END TYPE
TYPE _GdkDeviceKey
  AS guint keyval
  AS GdkModifierType modifiers
END TYPE
TYPE _GdkDisplay
  AS GObject parent_instance
  AS GList PTR queued_events
  AS GList PTR queued_tail
  AS guint32 button_click_time(1)
  AS GdkWindow PTR button_window(1)
  AS gint button_number(1)
  AS guint double_click_time
  AS GdkDevice PTR core_pointer
  AS const GdkDisplayPointerHooks PTR pointer_hooks
  AS guint closed : 1
  AS guint ignore_core_events : 1
  AS guint double_click_distance
  AS gint button_x(1)
  AS gint button_y(1)
  AS GList PTR pointer_grabs
  AS GdkKeyboardGrabInfo keyboard_grab
  AS GdkPointerWindowInfo pointer_info
  AS guint32 last_event_time
END TYPE
DECLARE FUNCTION gdk_display_get_type() AS GType
#DEFINE GDK_TYPE_DISPLAY (gdk_display_get_type())
#DEFINE GDK_DISPLAY(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_DISPLAY, GdkDisplay))
#DEFINE GDK_DISPLAY_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_DISPLAY, GdkDisplayClass))
#DEFINE GDK_IS_DISPLAY(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_DISPLAY))
#DEFINE GDK_IS_CLASS_DISPLAY(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_DISPLAY))
#DEFINE GDK_DISPLAY_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_DISPLAY, GdkDisplayClass))
DECLARE FUNCTION gdk_display_get_default() AS GdkDisplay PTR
DECLARE FUNCTION gdk_display_open(BYVAL AS const gchar PTR) AS GdkDisplay PTR
DECLARE FUNCTION gdk_display_open_default_libgtk_only() AS GdkDisplay PTR
DECLARE SUB gdk_display_add_client_message_filter(BYVAL AS GdkDisplay PTR, BYVAL AS GdkAtom, BYVAL AS GdkFilterFunc, BYVAL AS gpointer)
DECLARE SUB gdk_display_beep(BYVAL AS GdkDisplay PTR)
DECLARE SUB gdk_display_close(BYVAL AS GdkDisplay PTR)
DECLARE SUB gdk_display_flush(BYVAL AS GdkDisplay PTR)
DECLARE FUNCTION gdk_display_get_core_pointer(BYVAL AS GdkDisplay PTR) AS GdkDevice PTR
DECLARE FUNCTION gdk_display_get_default_cursor_size(BYVAL AS GdkDisplay PTR) AS guint
DECLARE FUNCTION gdk_display_get_default_group(BYVAL AS GdkDisplay PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_display_get_default_screen(BYVAL AS GdkDisplay PTR) AS GdkScreen PTR
DECLARE FUNCTION gdk_display_get_event(BYVAL AS GdkDisplay PTR) AS any ptr /'GdkEvent'/ PTR
DECLARE SUB gdk_display_get_maximal_cursor_size(BYVAL AS GdkDisplay PTR, BYVAL AS guint PTR, BYVAL AS guint PTR)
DECLARE FUNCTION gdk_display_get_n_screens(BYVAL AS GdkDisplay PTR) AS gint
DECLARE FUNCTION gdk_display_get_name(BYVAL AS GdkDisplay PTR) AS const gchar PTR
DECLARE SUB gdk_display_get_pointer(BYVAL AS GdkDisplay PTR, BYVAL AS GdkScreen PTR PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS GdkModifierType PTR)
DECLARE FUNCTION gdk_display_get_screen(BYVAL AS GdkDisplay PTR, BYVAL AS gint) AS GdkScreen PTR
DECLARE FUNCTION gdk_display_get_window_at_pointer(BYVAL AS GdkDisplay PTR, BYVAL AS gint PTR, BYVAL AS gint PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_display_is_closed(BYVAL AS GdkDisplay PTR) AS gboolean
DECLARE SUB gdk_display_keyboard_ungrab(BYVAL AS GdkDisplay PTR, BYVAL AS guint32)
DECLARE FUNCTION gdk_display_list_devices(BYVAL AS GdkDisplay PTR) AS GList PTR
DECLARE FUNCTION gdk_display_peek_event(BYVAL AS GdkDisplay PTR) AS any ptr /'GdkEvent'/ PTR
DECLARE FUNCTION gdk_display_pointer_is_grabbed(BYVAL AS GdkDisplay PTR) AS gboolean
DECLARE SUB gdk_display_pointer_ungrab(BYVAL AS GdkDisplay PTR, BYVAL AS guint32)
DECLARE SUB gdk_display_put_event(BYVAL AS GdkDisplay PTR, BYVAL AS const any ptr /'const GdkEvent'/ PTR)
DECLARE FUNCTION gdk_display_request_selection_notification(BYVAL AS GdkDisplay PTR, BYVAL AS GdkAtom) AS gboolean
DECLARE SUB gdk_display_set_double_click_distance(BYVAL AS GdkDisplay PTR, BYVAL AS guint)
DECLARE SUB gdk_display_set_double_click_time(BYVAL AS GdkDisplay PTR, BYVAL AS guint)
DECLARE FUNCTION gdk_display_set_pointer_hooks(BYVAL AS GdkDisplay PTR, BYVAL AS const GdkDisplayPointerHooks PTR) AS GdkDisplayPointerHooks PTR
DECLARE SUB gdk_display_store_clipboard(BYVAL AS GdkDisplay PTR, BYVAL AS GdkWindow PTR, BYVAL AS guint32, BYVAL AS const GdkAtom PTR, BYVAL AS gint)
DECLARE FUNCTION gdk_display_supports_clipboard_persistence(BYVAL AS GdkDisplay PTR) AS gboolean
DECLARE FUNCTION gdk_display_supports_composite(BYVAL AS GdkDisplay PTR) AS gboolean
DECLARE FUNCTION gdk_display_supports_cursor_alpha(BYVAL AS GdkDisplay PTR) AS gboolean
DECLARE FUNCTION gdk_display_supports_cursor_color(BYVAL AS GdkDisplay PTR) AS gboolean
DECLARE FUNCTION gdk_display_supports_input_shapes(BYVAL AS GdkDisplay PTR) AS gboolean
DECLARE FUNCTION gdk_display_supports_selection_notification(BYVAL AS GdkDisplay PTR) AS gboolean
DECLARE FUNCTION gdk_display_supports_shapes(BYVAL AS GdkDisplay PTR) AS gboolean
DECLARE SUB gdk_display_sync(BYVAL AS GdkDisplay PTR)
DECLARE SUB gdk_display_warp_pointer(BYVAL AS GdkDisplay PTR, BYVAL AS GdkScreen PTR, BYVAL AS gint, BYVAL AS gint)
TYPE _GdkDisplayClass
  AS GObjectClass parent_class
  get_display_name AS FUNCTION CDECL(BYVAL AS GdkDisplay PTR) AS const gchar PTR
  get_n_screens AS FUNCTION CDECL(BYVAL AS GdkDisplay PTR) AS gint
  get_screen AS FUNCTION CDECL(BYVAL AS GdkDisplay PTR, BYVAL AS gint) AS GdkScreen PTR
  get_default_screen AS FUNCTION CDECL(BYVAL AS GdkDisplay PTR) AS GdkScreen PTR
  closed AS SUB CDECL(BYVAL AS GdkDisplay PTR, BYVAL AS gboolean)
END TYPE
DECLARE FUNCTION gdk_display_manager_get_type() AS GType
#DEFINE GDK_TYPE_DISPLAY_MANAGER (gdk_display_manager_get_type())
#DEFINE GDK_DISPLAY_MANAGER(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_DISPLAY_MANAGER, GdkDisplayManager))
#DEFINE GDK_DISPLAY_MANAGER_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_DISPLAY_MANAGER, GdkDisplayManagerClass))
#DEFINE GDK_IS_DISPLAY_MANAGER(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_DISPLAY_MANAGER))
#DEFINE GDK_IS_CLASS_DISPLAY_MANAGER(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_DISPLAY_MANAGER))
#DEFINE GDK_DISPLAY_MANAGER_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_DISPLAY_MANAGER, GdkDisplayManagerClass))
DECLARE FUNCTION gdk_display_manager_get() AS GdkDisplayManager PTR
DECLARE FUNCTION gdk_display_manager_get_default_display(BYVAL AS GdkDisplayManager PTR) AS GdkDisplay PTR
DECLARE FUNCTION gdk_display_manager_list_displays(BYVAL AS GdkDisplayManager PTR) AS GSList PTR
DECLARE SUB gdk_display_manager_set_default_display(BYVAL AS GdkDisplayManager PTR, BYVAL AS GdkDisplay PTR)
TYPE _GdkDisplayManagerClass
  AS GObjectClass parent_class
  display_opened AS SUB CDECL(BYVAL AS GdkDisplayManager PTR, BYVAL AS GdkDisplay PTR)
END TYPE
TYPE _GdkDisplayPointerHooks
  get_pointer AS SUB CDECL(BYVAL AS GdkDisplay PTR, BYVAL AS GdkScreen PTR PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS GdkModifierType PTR)
  window_get_pointer AS FUNCTION CDECL(BYVAL AS GdkDisplay PTR, BYVAL AS GdkWindow PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS GdkModifierType PTR) AS GdkWindow PTR
  window_at_pointer AS FUNCTION CDECL(BYVAL AS GdkDisplay PTR, BYVAL AS gint PTR, BYVAL AS gint PTR) AS GdkWindow PTR
END TYPE
TYPE _GdkDragContext
  AS GObject parent_instance
  AS GdkDragProtocol protocol
  AS gboolean is_source
  AS GdkWindow PTR source_window
  AS GdkWindow PTR dest_window
  AS GList PTR targets
  AS GdkDragAction actions
  AS GdkDragAction suggested_action
  AS GdkDragAction action
  AS guint32 start_time
  AS gpointer windowing_data
END TYPE
DECLARE FUNCTION gdk_drag_context_get_type() AS GType
#DEFINE GDK_TYPE_DRAG_CONTEXT (gdk_drag_context_get_type())
#DEFINE GDK_DRAG_CONTEXT(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_DRAG_CONTEXT, GdkDragContext))
#DEFINE GDK_DRAG_CONTEXT_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_DRAG_CONTEXT, GdkDragContextClass))
#DEFINE GDK_IS_DRAG_CONTEXT(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_DRAG_CONTEXT))
#DEFINE GDK_IS_CLASS_DRAG_CONTEXT(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_DRAG_CONTEXT))
#DEFINE GDK_DRAG_CONTEXT_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_DRAG_CONTEXT, GdkDragContextClass))
DECLARE FUNCTION gdk_drag_context_new() AS GdkDragContext PTR
DECLARE FUNCTION gdk_drag_context_get_actions(BYVAL AS GdkDragContext PTR) AS GdkDragAction
DECLARE FUNCTION gdk_drag_context_get_dest_window(BYVAL AS GdkDragContext PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_drag_context_get_protocol(BYVAL AS GdkDragContext PTR) AS GdkDragProtocol
DECLARE FUNCTION gdk_drag_context_get_selected_action(BYVAL AS GdkDragContext PTR) AS GdkDragAction
DECLARE FUNCTION gdk_drag_context_get_source_window(BYVAL AS GdkDragContext PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_drag_context_get_suggested_action(BYVAL AS GdkDragContext PTR) AS GdkDragAction
DECLARE FUNCTION gdk_drag_context_list_targets(BYVAL AS GdkDragContext PTR) AS GList PTR
DECLARE SUB gdk_drag_context_ref(BYVAL AS GdkDragContext PTR)
DECLARE SUB gdk_drag_context_unref(BYVAL AS GdkDragContext PTR)
TYPE _GdkDragContextClass
  AS GObjectClass parent_class
END TYPE
TYPE _GdkDrawable
  AS GObject parent_instance
END TYPE
DECLARE FUNCTION gdk_drawable_get_type() AS GType
#DEFINE GDK_TYPE_DRAWABLE (gdk_drawable_get_type())
#DEFINE GDK_DRAWABLE(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_DRAWABLE, GdkDrawable))
#DEFINE GDK_DRAWABLE_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_DRAWABLE, GdkDrawableClass))
#DEFINE GDK_IS_DRAWABLE(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_DRAWABLE))
#DEFINE GDK_IS_CLASS_DRAWABLE(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_DRAWABLE))
#DEFINE GDK_DRAWABLE_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_DRAWABLE, GdkDrawableClass))
DECLARE FUNCTION gdk_drawable_copy_to_image(BYVAL AS GdkDrawable PTR, BYVAL AS GdkImage PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint) AS GdkImage PTR
DECLARE FUNCTION gdk_drawable_get_clip_region(BYVAL AS GdkDrawable PTR) AS GdkRegion PTR
DECLARE FUNCTION gdk_drawable_get_colormap(BYVAL AS GdkDrawable PTR) AS GdkColormap PTR
DECLARE FUNCTION gdk_drawable_get_data(BYVAL AS GdkDrawable PTR, BYVAL AS const gchar PTR) AS gpointer
DECLARE FUNCTION gdk_drawable_get_depth(BYVAL AS GdkDrawable PTR) AS gint
DECLARE FUNCTION gdk_drawable_get_display(BYVAL AS GdkDrawable PTR) AS GdkDisplay PTR
DECLARE FUNCTION gdk_drawable_get_image(BYVAL AS GdkDrawable PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint) AS GdkImage PTR
DECLARE FUNCTION gdk_drawable_get_screen(BYVAL AS GdkDrawable PTR) AS GdkScreen PTR
DECLARE SUB gdk_drawable_get_size(BYVAL AS GdkDrawable PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE FUNCTION gdk_drawable_get_visible_region(BYVAL AS GdkDrawable PTR) AS GdkRegion PTR
DECLARE FUNCTION gdk_drawable_get_visual(BYVAL AS GdkDrawable PTR) AS GdkVisual PTR
DECLARE FUNCTION gdk_drawable_ref(BYVAL AS GdkDrawable PTR) AS GdkDrawable PTR
DECLARE SUB gdk_drawable_set_colormap(BYVAL AS GdkDrawable PTR, BYVAL AS GdkColormap PTR)
DECLARE SUB gdk_drawable_set_data(BYVAL AS GdkDrawable PTR, BYVAL AS const gchar PTR, BYVAL AS gpointer, BYVAL AS GDestroyNotify)
DECLARE SUB gdk_drawable_unref(BYVAL AS GdkDrawable PTR)
TYPE _GdkDrawableClass
  AS GObjectClass parent_class
  create_gc AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGCValues PTR, BYVAL AS GdkGCValuesMask) AS GdkGC PTR
  draw_rectangle AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gboolean, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
  draw_arc AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gboolean, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
  draw_polygon AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gboolean, BYVAL AS GdkPoint PTR, BYVAL AS gint)
  draw_text AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkFont PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS const gchar PTR, BYVAL AS gint)
  draw_text_wc AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkFont PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS const GdkWChar PTR, BYVAL AS gint)
  draw_drawable AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS GdkDrawable PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
  draw_points AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS GdkPoint PTR, BYVAL AS gint)
  draw_segments AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS GdkSegment PTR, BYVAL AS gint)
  draw_lines AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS GdkPoint PTR, BYVAL AS gint)
  draw_glyphs AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS PangoFont PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS PangoGlyphString PTR)
  draw_image AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS GdkImage PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
  get_depth AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR) AS gint
  get_size AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
  set_colormap AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkColormap PTR)
  get_colormap AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR) AS GdkColormap PTR
  get_visual AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR) AS GdkVisual PTR
  get_screen AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR) AS GdkScreen PTR
  get_image AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint) AS GdkImage PTR
  get_clip_region AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR) AS GdkRegion PTR
  get_visible_region AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR) AS GdkRegion PTR
  get_composite_drawable AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint PTR, BYVAL AS gint PTR) AS GdkDrawable PTR
  draw_pixbuf AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS GdkPixbuf PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkRgbDither, BYVAL AS gint, BYVAL AS gint)
  _copy_to_image AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkImage PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint) AS GdkImage PTR
  draw_glyphs_transformed AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS PangoMatrix PTR, BYVAL AS PangoFont PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS PangoGlyphString PTR)
  draw_trapezoids AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS GdkTrapezoid PTR, BYVAL AS gint)
  ref_cairo_surface AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR) AS cairo_surface_t PTR
  get_source_drawable AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR) AS GdkDrawable PTR
  set_cairo_clip AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS cairo_t PTR)
  create_cairo_surface AS FUNCTION CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/) AS cairo_surface_t PTR
  draw_drawable_with_src AS SUB CDECL(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS GdkDrawable PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkDrawable PTR)
  _gdk_reserved7 AS SUB CDECL()
  _gdk_reserved9 AS SUB CDECL()
  _gdk_reserved10 AS SUB CDECL()
  _gdk_reserved11 AS SUB CDECL()
  _gdk_reserved12 AS SUB CDECL()
  _gdk_reserved13 AS SUB CDECL()
  _gdk_reserved14 AS SUB CDECL()
  _gdk_reserved15 AS SUB CDECL()
END TYPE
UNION GdkEvent
  AS GdkEventType type
  AS GdkEventAny any
  AS GdkEventExpose expose
  AS GdkEventNoExpose no_expose
  AS GdkEventVisibility visibility
  AS GdkEventMotion motion
  AS GdkEventButton button
  AS GdkEventScroll scroll
  AS GdkEventKey key
  AS GdkEventCrossing crossing
  AS GdkEventFocus focus_change
  AS GdkEventConfigure configure
  AS GdkEventProperty property
  AS GdkEventSelection selection
  AS GdkEventOwnerChange owner_change
  AS GdkEventProximity proximity
  AS GdkEventClient client
  AS GdkEventDND dnd
  AS GdkEventWindowState window_state
  AS GdkEventSetting setting
  AS GdkEventGrabBroken grab_broken
END UNION
TYPE GdkEventFunc AS SUB CDECL(BYVAL AS any ptr /'GdkEvent'/ PTR, BYVAL AS gpointer)
TYPE _GdkFont
  AS GdkFontType type
  AS gint ascent
  AS gint descent
END TYPE
DECLARE FUNCTION gdk_font_equal(BYVAL AS const GdkFont PTR, BYVAL AS const GdkFont PTR) AS gboolean
DECLARE FUNCTION gdk_font_get_display(BYVAL AS GdkFont PTR) AS GdkDisplay PTR
DECLARE FUNCTION gdk_font_id(BYVAL AS const GdkFont PTR) AS gint
DECLARE FUNCTION gdk_font_ref(BYVAL AS GdkFont PTR) AS GdkFont PTR
DECLARE SUB gdk_font_unref(BYVAL AS GdkFont PTR)
DECLARE FUNCTION gdk_font_from_description(BYVAL AS PangoFontDescription PTR) AS GdkFont PTR
DECLARE FUNCTION gdk_font_from_description_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS PangoFontDescription PTR) AS GdkFont PTR
DECLARE FUNCTION gdk_font_load(BYVAL AS const gchar PTR) AS GdkFont PTR
DECLARE FUNCTION gdk_font_load_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS const gchar PTR) AS GdkFont PTR
TYPE _GdkGC
  AS GObject parent_instance
  AS gint clip_x_origin
  AS gint clip_y_origin
  AS gint ts_x_origin
  AS gint ts_y_origin
  AS GdkColormap PTR colormap
END TYPE
DECLARE FUNCTION gdk_gc_get_type() AS GType
#DEFINE GDK_TYPE_GC (gdk_gc_get_type())
#DEFINE GDK_GC(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_GC, GdkGC))
#DEFINE GDK_GC_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_GC, GdkGCClass))
#DEFINE GDK_IS_GC(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_GC))
#DEFINE GDK_IS_CLASS_GC(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_GC))
#DEFINE GDK_GC_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_GC, GdkGCClass))
DECLARE FUNCTION gdk_gc_new(BYVAL AS GdkDrawable PTR) AS GdkGC PTR
DECLARE FUNCTION gdk_gc_new_with_values(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGCValues PTR, BYVAL AS GdkGCValuesMask) AS GdkGC PTR
DECLARE SUB gdk_gc_copy(BYVAL AS GdkGC PTR, BYVAL AS GdkGC PTR)
DECLARE FUNCTION gdk_gc_get_colormap(BYVAL AS GdkGC PTR) AS GdkColormap PTR
DECLARE FUNCTION gdk_gc_get_screen(BYVAL AS GdkGC PTR) AS GdkScreen PTR
DECLARE SUB gdk_gc_get_values(BYVAL AS GdkGC PTR, BYVAL AS GdkGCValues PTR)
DECLARE SUB gdk_gc_offset(BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE FUNCTION gdk_gc_ref(BYVAL AS GdkGC PTR) AS GdkGC PTR
DECLARE SUB gdk_gc_set_background(BYVAL AS GdkGC PTR, BYVAL AS const GdkColor PTR)
DECLARE SUB gdk_gc_set_clip_mask(BYVAL AS GdkGC PTR, BYVAL AS GdkBitmap PTR)
DECLARE SUB gdk_gc_set_clip_origin(BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_gc_set_clip_rectangle(BYVAL AS GdkGC PTR, BYVAL AS const GdkRectangle PTR)
DECLARE SUB gdk_gc_set_clip_region(BYVAL AS GdkGC PTR, BYVAL AS const GdkRegion PTR)
DECLARE SUB gdk_gc_set_colormap(BYVAL AS GdkGC PTR, BYVAL AS GdkColormap PTR)
DECLARE SUB gdk_gc_set_dashes(BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint8, BYVAL AS gint)
DECLARE SUB gdk_gc_set_exposures(BYVAL AS GdkGC PTR, BYVAL AS gboolean)
DECLARE SUB gdk_gc_set_fill(BYVAL AS GdkGC PTR, BYVAL AS GdkFill)
DECLARE SUB gdk_gc_set_font(BYVAL AS GdkGC PTR, BYVAL AS GdkFont PTR)
DECLARE SUB gdk_gc_set_foreground(BYVAL AS GdkGC PTR, BYVAL AS const GdkColor PTR)
DECLARE SUB gdk_gc_set_function(BYVAL AS GdkGC PTR, BYVAL AS GdkFunction)
DECLARE SUB gdk_gc_set_line_attributes(BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS GdkLineStyle, BYVAL AS GdkCapStyle, BYVAL AS GdkJoinStyle)
DECLARE SUB gdk_gc_set_rgb_bg_color(BYVAL AS GdkGC PTR, BYVAL AS const GdkColor PTR)
DECLARE SUB gdk_gc_set_rgb_fg_color(BYVAL AS GdkGC PTR, BYVAL AS const GdkColor PTR)
DECLARE SUB gdk_gc_set_stipple(BYVAL AS GdkGC PTR, BYVAL AS GdkPixmap PTR)
DECLARE SUB gdk_gc_set_subwindow(BYVAL AS GdkGC PTR, BYVAL AS GdkSubwindowMode)
DECLARE SUB gdk_gc_set_tile(BYVAL AS GdkGC PTR, BYVAL AS GdkPixmap PTR)
DECLARE SUB gdk_gc_set_ts_origin(BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_gc_set_values(BYVAL AS GdkGC PTR, BYVAL AS GdkGCValues PTR, BYVAL AS GdkGCValuesMask)
DECLARE SUB gdk_gc_unref(BYVAL AS GdkGC PTR)
TYPE _GdkGCClass
  AS GObjectClass parent_class
  get_values AS SUB CDECL(BYVAL AS GdkGC PTR, BYVAL AS GdkGCValues PTR)
  set_values AS SUB CDECL(BYVAL AS GdkGC PTR, BYVAL AS GdkGCValues PTR, BYVAL AS GdkGCValuesMask)
  set_dashes AS SUB CDECL(BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint8, BYVAL AS gint)
  _gdk_reserved1 AS SUB CDECL()
  _gdk_reserved2 AS SUB CDECL()
  _gdk_reserved3 AS SUB CDECL()
  _gdk_reserved4 AS SUB CDECL()
END TYPE
TYPE _GdkGCValues
  AS GdkColor foreground
  AS GdkColor background
  AS GdkFont PTR font
  AS GdkFunction function
  AS GdkFill fill
  AS GdkPixmap PTR tile
  AS GdkPixmap PTR stipple
  AS GdkPixmap PTR clip_mask
  AS GdkSubwindowMode subwindow_mode
  AS gint ts_x_origin
  AS gint ts_y_origin
  AS gint clip_x_origin
  AS gint clip_y_origin
  AS gint graphics_exposures
  AS gint line_width
  AS GdkLineStyle line_style
  AS GdkCapStyle cap_style
  AS GdkJoinStyle join_style
END TYPE
TYPE _GdkGeometry
  AS gint min_width
  AS gint min_height
  AS gint max_width
  AS gint max_height
  AS gint base_width
  AS gint base_height
  AS gint width_inc
  AS gint height_inc
  AS gdouble min_aspect
  AS gdouble max_aspect
  AS GdkGravity win_gravity
END TYPE
TYPE _GdkImage
  AS GObject parent_instance
  AS GdkImageType type
  AS GdkVisual PTR visual
  AS GdkByteOrder byte_order
  AS gint width
  AS gint height
  AS guint16 depth
  AS guint16 bpp
  AS guint16 bpl
  AS guint16 bits_per_pixel
  AS gpointer mem
  AS GdkColormap PTR colormap
  AS gpointer windowing_data
END TYPE
DECLARE FUNCTION gdk_image_get_type() AS GType
#DEFINE GDK_TYPE_IMAGE (gdk_image_get_type())
#DEFINE GDK_IMAGE(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_IMAGE, GdkImage))
#DEFINE GDK_IMAGE_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_IMAGE, GdkImageClass))
#DEFINE GDK_IS_IMAGE(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_IMAGE))
#DEFINE GDK_IS_CLASS_IMAGE(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_IMAGE))
#DEFINE GDK_IMAGE_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_IMAGE, GdkImageClass))
DECLARE FUNCTION gdk_image_new(BYVAL AS GdkImageType, BYVAL AS GdkVisual PTR, BYVAL AS gint, BYVAL AS gint) AS GdkImage PTR
DECLARE FUNCTION gdk_image_get(BYVAL AS GdkDrawable PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint) AS GdkImage PTR
DECLARE FUNCTION gdk_image_get_bits_per_pixel(BYVAL AS GdkImage PTR) AS guint16
DECLARE FUNCTION gdk_image_get_byte_order(BYVAL AS GdkImage PTR) AS GdkByteOrder
DECLARE FUNCTION gdk_image_get_bytes_per_line(BYVAL AS GdkImage PTR) AS guint16
DECLARE FUNCTION gdk_image_get_bytes_per_pixel(BYVAL AS GdkImage PTR) AS guint16
DECLARE FUNCTION gdk_image_get_colormap(BYVAL AS GdkImage PTR) AS GdkColormap PTR
DECLARE FUNCTION gdk_image_get_depth(BYVAL AS GdkImage PTR) AS guint16
DECLARE FUNCTION gdk_image_get_height(BYVAL AS GdkImage PTR) AS gint
DECLARE FUNCTION gdk_image_get_image_type(BYVAL AS GdkImage PTR) AS GdkImageType
DECLARE FUNCTION gdk_image_get_pixel(BYVAL AS GdkImage PTR, BYVAL AS gint, BYVAL AS gint) AS guint32
DECLARE FUNCTION gdk_image_get_pixels(BYVAL AS GdkImage PTR) AS gpointer
DECLARE FUNCTION gdk_image_get_visual(BYVAL AS GdkImage PTR) AS GdkVisual PTR
DECLARE FUNCTION gdk_image_get_width(BYVAL AS GdkImage PTR) AS gint
DECLARE SUB gdk_image_put_pixel(BYVAL AS GdkImage PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS guint32)
DECLARE FUNCTION gdk_image_ref(BYVAL AS GdkImage PTR) AS GdkImage PTR
DECLARE SUB gdk_image_set_colormap(BYVAL AS GdkImage PTR, BYVAL AS GdkColormap PTR)
DECLARE SUB gdk_image_unref(BYVAL AS GdkImage PTR)
TYPE _GdkImageClass
  AS GObjectClass parent_class
END TYPE
TYPE GdkInputFunction AS SUB CDECL(BYVAL AS gpointer, BYVAL AS gint, BYVAL AS GdkInputCondition)
TYPE _GdkKeymap
  AS GObject parent_instance
  AS GdkDisplay PTR display
END TYPE
DECLARE FUNCTION gdk_keymap_get_type() AS GType
#DEFINE GDK_TYPE_KEYMAP (gdk_keymap_get_type())
#DEFINE GDK_KEYMAP(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_KEYMAP, GdkKeymap))
#DEFINE GDK_KEYMAP_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_KEYMAP, GdkKeymapClass))
#DEFINE GDK_IS_KEYMAP(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_KEYMAP))
#DEFINE GDK_IS_CLASS_KEYMAP(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_KEYMAP))
#DEFINE GDK_KEYMAP_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_KEYMAP, GdkKeymapClass))
DECLARE FUNCTION gdk_keymap_get_default() AS GdkKeymap PTR
DECLARE FUNCTION gdk_keymap_get_for_display(BYVAL AS GdkDisplay PTR) AS GdkKeymap PTR
DECLARE SUB gdk_keymap_add_virtual_modifiers(BYVAL AS GdkKeymap PTR, BYVAL AS GdkModifierType PTR)
DECLARE FUNCTION gdk_keymap_get_caps_lock_state(BYVAL AS GdkKeymap PTR) AS gboolean
DECLARE FUNCTION gdk_keymap_get_direction(BYVAL AS GdkKeymap PTR) AS PangoDirection
DECLARE FUNCTION gdk_keymap_get_entries_for_keycode(BYVAL AS GdkKeymap PTR, BYVAL AS guint, BYVAL AS GdkKeymapKey PTR PTR, BYVAL AS guint PTR PTR, BYVAL AS gint PTR) AS gboolean
DECLARE FUNCTION gdk_keymap_get_entries_for_keyval(BYVAL AS GdkKeymap PTR, BYVAL AS guint, BYVAL AS GdkKeymapKey PTR PTR, BYVAL AS gint PTR) AS gboolean
DECLARE FUNCTION gdk_keymap_have_bidi_layouts(BYVAL AS GdkKeymap PTR) AS gboolean
DECLARE FUNCTION gdk_keymap_lookup_key(BYVAL AS GdkKeymap PTR, BYVAL AS const GdkKeymapKey PTR) AS guint
DECLARE FUNCTION gdk_keymap_map_virtual_modifiers(BYVAL AS GdkKeymap PTR, BYVAL AS GdkModifierType PTR) AS gboolean
DECLARE FUNCTION gdk_keymap_translate_keyboard_state(BYVAL AS GdkKeymap PTR, BYVAL AS guint, BYVAL AS GdkModifierType, BYVAL AS gint, BYVAL AS guint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS GdkModifierType PTR) AS gboolean
TYPE _GdkKeymapClass
  AS GObjectClass parent_class
  direction_changed AS SUB CDECL(BYVAL AS GdkKeymap PTR)
  keys_changed AS SUB CDECL(BYVAL AS GdkKeymap PTR)
  state_changed AS SUB CDECL(BYVAL AS GdkKeymap PTR)
END TYPE
TYPE _GdkKeymapKey
  AS guint keycode
  AS gint group
  AS gint level
END TYPE
TYPE _GdkPangoAttrEmbossColor
  AS PangoAttribute attr
  AS PangoColor color
END TYPE
DECLARE FUNCTION gdk_pango_attr_emboss_color_new(BYVAL AS const GdkColor PTR) AS PangoAttribute PTR
TYPE _GdkPangoAttrEmbossed
  AS PangoAttribute attr
  AS gboolean embossed
END TYPE
DECLARE FUNCTION gdk_pango_attr_embossed_new(BYVAL AS gboolean) AS PangoAttribute PTR
TYPE _GdkPangoAttrStipple
  AS PangoAttribute attr
  AS GdkBitmap PTR stipple
END TYPE
DECLARE FUNCTION gdk_pango_attr_stipple_new(BYVAL AS GdkBitmap PTR) AS PangoAttribute PTR
TYPE _GdkPangoRenderer
  AS PangoRenderer parent_instance
  AS GdkPangoRendererPrivate PTR priv
END TYPE
DECLARE FUNCTION gdk_pango_renderer_get_type() AS GType
#DEFINE GDK_TYPE_PANGO_RENDERER (gdk_pango_renderer_get_type())
#DEFINE GDK_PANGO_RENDERER(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_PANGO_RENDERER, GdkPangoRenderer))
#DEFINE GDK_PANGO_RENDERER_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_PANGO_RENDERER, GdkPangoRendererClass))
#DEFINE GDK_IS_PANGO_RENDERER(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_PANGO_RENDERER))
#DEFINE GDK_IS_CLASS_PANGO_RENDERER(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_PANGO_RENDERER))
#DEFINE GDK_PANGO_RENDERER_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_PANGO_RENDERER, GdkPangoRendererClass))
DECLARE FUNCTION gdk_pango_renderer_new(BYVAL AS GdkScreen PTR) AS PangoRenderer PTR
DECLARE FUNCTION gdk_pango_renderer_get_default(BYVAL AS GdkScreen PTR) AS PangoRenderer PTR
DECLARE SUB gdk_pango_renderer_set_drawable(BYVAL AS GdkPangoRenderer PTR, BYVAL AS GdkDrawable PTR)
DECLARE SUB gdk_pango_renderer_set_gc(BYVAL AS GdkPangoRenderer PTR, BYVAL AS GdkGC PTR)
DECLARE SUB gdk_pango_renderer_set_override_color(BYVAL AS GdkPangoRenderer PTR, BYVAL AS PangoRenderPart, BYVAL AS const GdkColor PTR)
DECLARE SUB gdk_pango_renderer_set_stipple(BYVAL AS GdkPangoRenderer PTR, BYVAL AS PangoRenderPart, BYVAL AS GdkBitmap PTR)
TYPE _GdkPangoRendererClass
  AS PangoRendererClass parent_class
END TYPE
TYPE _GdkPixmap
  AS GObject parent_instance
END TYPE
DECLARE FUNCTION gdk_pixmap_get_type() AS GType
#DEFINE GDK_TYPE_PIXMAP (gdk_pixmap_get_type())
#DEFINE GDK_PIXMAP(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_PIXMAP, GdkPixmap))
#DEFINE GDK_PIXMAP_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_PIXMAP, Gdk))
#DEFINE GDK_IS_PIXMAP(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_PIXMAP))
#DEFINE GDK_IS_CLASS_PIXMAP(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_PIXMAP))
#DEFINE GDK_PIXMAP_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_PIXMAP, Gdk))
DECLARE FUNCTION gdk_pixmap_foreign_new(BYVAL AS GdkNativeWindow) AS GdkPixmap PTR
DECLARE FUNCTION gdk_pixmap_foreign_new_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkNativeWindow) AS GdkPixmap PTR
DECLARE FUNCTION gdk_pixmap_foreign_new_for_screen(BYVAL AS GdkScreen PTR, BYVAL AS GdkNativeWindow, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint) AS GdkPixmap PTR
DECLARE FUNCTION gdk_pixmap_new(BYVAL AS GdkDrawable PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint) AS GdkPixmap PTR
DECLARE FUNCTION gdk_pixmap_colormap_create_from_xpm(BYVAL AS GdkDrawable PTR, BYVAL AS GdkColormap PTR, BYVAL AS GdkBitmap PTR PTR, BYVAL AS const GdkColor PTR, BYVAL AS const gchar PTR) AS GdkPixmap PTR
DECLARE FUNCTION gdk_pixmap_colormap_create_from_xpm_d(BYVAL AS GdkDrawable PTR, BYVAL AS GdkColormap PTR, BYVAL AS GdkBitmap PTR PTR, BYVAL AS const GdkColor PTR, BYVAL AS gchar PTR PTR) AS GdkPixmap PTR
DECLARE FUNCTION gdk_pixmap_create_from_data(BYVAL AS GdkDrawable PTR, BYVAL AS const gchar PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS const GdkColor PTR, BYVAL AS const GdkColor PTR) AS GdkPixmap PTR
DECLARE FUNCTION gdk_pixmap_create_from_xpm(BYVAL AS GdkDrawable PTR, BYVAL AS GdkBitmap PTR PTR, BYVAL AS const GdkColor PTR, BYVAL AS const gchar PTR) AS GdkPixmap PTR
DECLARE FUNCTION gdk_pixmap_create_from_xpm_d(BYVAL AS GdkDrawable PTR, BYVAL AS GdkBitmap PTR PTR, BYVAL AS const GdkColor PTR, BYVAL AS gchar PTR PTR) AS GdkPixmap PTR
DECLARE FUNCTION gdk_pixmap_lookup(BYVAL AS GdkNativeWindow) AS GdkPixmap PTR
DECLARE FUNCTION gdk_pixmap_lookup_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkNativeWindow) AS GdkPixmap PTR
DECLARE SUB gdk_pixmap_get_size(BYVAL AS GdkPixmap PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
TYPE _GdkPixmapObject
  AS GdkDrawable parent_instance
  AS GdkDrawable PTR impl
  AS gint depth
END TYPE
TYPE _GdkPixmapObjectClass
  AS GdkDrawableClass parent_class
END TYPE
TYPE _GdkPoint
  AS gint x
  AS gint y
END TYPE
TYPE _GdkPointerHooks
  get_pointer AS FUNCTION CDECL(BYVAL AS GdkWindow PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS GdkModifierType PTR) AS GdkWindow PTR
  window_at_pointer AS FUNCTION CDECL(BYVAL AS GdkScreen PTR, BYVAL AS gint PTR, BYVAL AS gint PTR) AS GdkWindow PTR
END TYPE
DECLARE FUNCTION gdk_region_copy(BYVAL AS const GdkRegion PTR) AS GdkRegion PTR
DECLARE SUB gdk_region_destroy(BYVAL AS GdkRegion PTR)
DECLARE FUNCTION gdk_region_empty(BYVAL AS const GdkRegion PTR) AS gboolean
DECLARE FUNCTION gdk_region_equal(BYVAL AS const GdkRegion PTR, BYVAL AS const GdkRegion PTR) AS gboolean
DECLARE SUB gdk_region_get_clipbox(BYVAL AS const GdkRegion PTR, BYVAL AS GdkRectangle PTR)
DECLARE SUB gdk_region_get_rectangles(BYVAL AS const GdkRegion PTR, BYVAL AS GdkRectangle PTR PTR, BYVAL AS gint PTR)
DECLARE SUB gdk_region_intersect(BYVAL AS GdkRegion PTR, BYVAL AS const GdkRegion PTR)
DECLARE SUB gdk_region_offset(BYVAL AS GdkRegion PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE FUNCTION gdk_region_point_in(BYVAL AS const GdkRegion PTR, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/) AS gboolean
DECLARE FUNCTION gdk_region_rect_equal(BYVAL AS const GdkRegion PTR, BYVAL AS const GdkRectangle PTR) AS gboolean
DECLARE FUNCTION gdk_region_rect_in(BYVAL AS const GdkRegion PTR, BYVAL AS const GdkRectangle PTR) AS GdkOverlapType
DECLARE SUB gdk_region_shrink(BYVAL AS GdkRegion PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_region_spans_intersect_foreach(BYVAL AS GdkRegion PTR, BYVAL AS const GdkSpan PTR, BYVAL AS gint /'int'/, BYVAL AS gboolean, BYVAL AS GdkSpanFunc, BYVAL AS gpointer)
DECLARE SUB gdk_region_subtract(BYVAL AS GdkRegion PTR, BYVAL AS const GdkRegion PTR)
DECLARE SUB gdk_region_union(BYVAL AS GdkRegion PTR, BYVAL AS const GdkRegion PTR)
DECLARE SUB gdk_region_union_with_rect(BYVAL AS GdkRegion PTR, BYVAL AS const GdkRectangle PTR)
DECLARE SUB gdk_region_xor(BYVAL AS GdkRegion PTR, BYVAL AS const GdkRegion PTR)
DECLARE FUNCTION gdk_region_new() AS GdkRegion PTR
DECLARE FUNCTION gdk_region_polygon(BYVAL AS const GdkPoint PTR, BYVAL AS gint, BYVAL AS GdkFillRule) AS GdkRegion PTR
DECLARE FUNCTION gdk_region_rectangle(BYVAL AS const GdkRectangle PTR) AS GdkRegion PTR
TYPE _GdkRgbCmap
  AS guint32 colors(255)
  AS gint n_colors
  AS GSList PTR info_list
END TYPE
DECLARE SUB gdk_rgb_cmap_free(BYVAL AS GdkRgbCmap PTR)
DECLARE FUNCTION gdk_rgb_cmap_new(BYVAL AS guint32 PTR, BYVAL AS gint) AS GdkRgbCmap PTR
TYPE _GdkScreen
  AS GObject parent_instance
  AS guint closed : 1
  AS GdkGC PTR normal_gcs(31)
  AS GdkGC PTR exposure_gcs(31)
  AS GdkGC PTR subwindow_gcs(31)
  AS cairo_font_options_t PTR font_options
  AS double resolution
END TYPE
DECLARE FUNCTION gdk_screen_get_type() AS GType
#DEFINE GDK_TYPE_SCREEN (gdk_screen_get_type())
#DEFINE GDK_SCREEN(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_SCREEN, GdkScreen))
#DEFINE GDK_SCREEN_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_SCREEN, GdkScreenClass))
#DEFINE GDK_IS_SCREEN(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_SCREEN))
#DEFINE GDK_IS_CLASS_SCREEN(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_SCREEN))
#DEFINE GDK_SCREEN_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_SCREEN, GdkScreenClass))
DECLARE FUNCTION gdk_screen_get_default() AS GdkScreen PTR
DECLARE FUNCTION gdk_screen_height() AS gint
DECLARE FUNCTION gdk_screen_height_mm() AS gint
DECLARE FUNCTION gdk_screen_width() AS gint
DECLARE FUNCTION gdk_screen_width_mm() AS gint
DECLARE SUB gdk_screen_broadcast_client_message(BYVAL AS GdkScreen PTR, BYVAL AS any ptr /'GdkEvent'/ PTR)
DECLARE FUNCTION gdk_screen_get_active_window(BYVAL AS GdkScreen PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_screen_get_default_colormap(BYVAL AS GdkScreen PTR) AS GdkColormap PTR
DECLARE FUNCTION gdk_screen_get_display(BYVAL AS GdkScreen PTR) AS GdkDisplay PTR
DECLARE FUNCTION gdk_screen_get_font_options(BYVAL AS GdkScreen PTR) AS const cairo_font_options_t PTR
DECLARE FUNCTION gdk_screen_get_height(BYVAL AS GdkScreen PTR) AS gint
DECLARE FUNCTION gdk_screen_get_height_mm(BYVAL AS GdkScreen PTR) AS gint
DECLARE FUNCTION gdk_screen_get_monitor_at_point(BYVAL AS GdkScreen PTR, BYVAL AS gint, BYVAL AS gint) AS gint
DECLARE FUNCTION gdk_screen_get_monitor_at_window(BYVAL AS GdkScreen PTR, BYVAL AS GdkWindow PTR) AS gint
DECLARE SUB gdk_screen_get_monitor_geometry(BYVAL AS GdkScreen PTR, BYVAL AS gint, BYVAL AS GdkRectangle PTR)
DECLARE FUNCTION gdk_screen_get_monitor_height_mm(BYVAL AS GdkScreen PTR, BYVAL AS gint) AS gint
DECLARE FUNCTION gdk_screen_get_monitor_plug_name(BYVAL AS GdkScreen PTR, BYVAL AS gint) AS gchar PTR
DECLARE FUNCTION gdk_screen_get_monitor_width_mm(BYVAL AS GdkScreen PTR, BYVAL AS gint) AS gint
DECLARE FUNCTION gdk_screen_get_n_monitors(BYVAL AS GdkScreen PTR) AS gint
DECLARE FUNCTION gdk_screen_get_number(BYVAL AS GdkScreen PTR) AS gint
DECLARE FUNCTION gdk_screen_get_primary_monitor(BYVAL AS GdkScreen PTR) AS gint
DECLARE FUNCTION gdk_screen_get_resolution(BYVAL AS GdkScreen PTR) AS gdouble
DECLARE FUNCTION gdk_screen_get_rgb_colormap(BYVAL AS GdkScreen PTR) AS GdkColormap PTR
DECLARE FUNCTION gdk_screen_get_rgb_visual(BYVAL AS GdkScreen PTR) AS GdkVisual PTR
DECLARE FUNCTION gdk_screen_get_rgba_colormap(BYVAL AS GdkScreen PTR) AS GdkColormap PTR
DECLARE FUNCTION gdk_screen_get_rgba_visual(BYVAL AS GdkScreen PTR) AS GdkVisual PTR
DECLARE FUNCTION gdk_screen_get_root_window(BYVAL AS GdkScreen PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_screen_get_setting(BYVAL AS GdkScreen PTR, BYVAL AS const gchar PTR, BYVAL AS GValue PTR) AS gboolean
DECLARE FUNCTION gdk_screen_get_system_colormap(BYVAL AS GdkScreen PTR) AS GdkColormap PTR
DECLARE FUNCTION gdk_screen_get_system_visual(BYVAL AS GdkScreen PTR) AS GdkVisual PTR
DECLARE FUNCTION gdk_screen_get_toplevel_windows(BYVAL AS GdkScreen PTR) AS GList PTR
DECLARE FUNCTION gdk_screen_get_width(BYVAL AS GdkScreen PTR) AS gint
DECLARE FUNCTION gdk_screen_get_width_mm(BYVAL AS GdkScreen PTR) AS gint
DECLARE FUNCTION gdk_screen_get_window_stack(BYVAL AS GdkScreen PTR) AS GList PTR
DECLARE FUNCTION gdk_screen_is_composited(BYVAL AS GdkScreen PTR) AS gboolean
DECLARE FUNCTION gdk_screen_list_visuals(BYVAL AS GdkScreen PTR) AS GList PTR
DECLARE FUNCTION gdk_screen_make_display_name(BYVAL AS GdkScreen PTR) AS gchar PTR
DECLARE SUB gdk_screen_set_default_colormap(BYVAL AS GdkScreen PTR, BYVAL AS GdkColormap PTR)
DECLARE SUB gdk_screen_set_font_options(BYVAL AS GdkScreen PTR, BYVAL AS const cairo_font_options_t PTR)
DECLARE SUB gdk_screen_set_resolution(BYVAL AS GdkScreen PTR, BYVAL AS gdouble)
TYPE _GdkScreenClass
  AS GObjectClass parent_class
  size_changed AS SUB CDECL(BYVAL AS GdkScreen PTR)
  composited_changed AS SUB CDECL(BYVAL AS GdkScreen PTR)
  monitors_changed AS SUB CDECL(BYVAL AS GdkScreen PTR)
END TYPE
TYPE _GdkSegment
  AS gint x1
  AS gint y1
  AS gint x2
  AS gint y2
END TYPE
TYPE _GdkSpan
  AS gint x
  AS gint y
  AS gint width
END TYPE
TYPE _GdkTimeCoord
  AS guint32 time
  AS gdouble axes(127)
END TYPE
TYPE _GdkTrapezoid
  AS double y1
  AS double x11
  AS double x21
  AS double y2
  AS double x12
  AS double x22
END TYPE
TYPE _GdkVisual
  AS GObject parent_instance
  AS GdkVisualType type
  AS gint depth
  AS GdkByteOrder byte_order
  AS gint colormap_size
  AS gint bits_per_rgb
  AS guint32 red_mask
  AS gint red_shift
  AS gint red_prec
  AS guint32 green_mask
  AS gint green_shift
  AS gint green_prec
  AS guint32 blue_mask
  AS gint blue_shift
  AS gint blue_prec
END TYPE
DECLARE FUNCTION gdk_visual_get_type() AS GType
#DEFINE GDK_TYPE_VISUAL (gdk_visual_get_type())
#DEFINE GDK_VISUAL(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_VISUAL, GdkVisual))
#DEFINE GDK_VISUAL_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_VISUAL, GdkVisualClass))
#DEFINE GDK_IS_VISUAL(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_VISUAL))
#DEFINE GDK_IS_CLASS_VISUAL(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_VISUAL))
#DEFINE GDK_VISUAL_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_VISUAL, GdkVisualClass))
DECLARE FUNCTION gdk_visual_get_best() AS GdkVisual PTR
DECLARE FUNCTION gdk_visual_get_best_depth() AS gint
DECLARE FUNCTION gdk_visual_get_best_type() AS GdkVisualType
DECLARE FUNCTION gdk_visual_get_best_with_both(BYVAL AS gint, BYVAL AS GdkVisualType) AS GdkVisual PTR
DECLARE FUNCTION gdk_visual_get_best_with_depth(BYVAL AS gint) AS GdkVisual PTR
DECLARE FUNCTION gdk_visual_get_best_with_type(BYVAL AS GdkVisualType) AS GdkVisual PTR
DECLARE FUNCTION gdk_visual_get_system() AS GdkVisual PTR
DECLARE FUNCTION gdk_visual_get_bits_per_rgb(BYVAL AS GdkVisual PTR) AS gint
DECLARE SUB gdk_visual_get_blue_pixel_details(BYVAL AS GdkVisual PTR, BYVAL AS guint32 PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE FUNCTION gdk_visual_get_byte_order(BYVAL AS GdkVisual PTR) AS GdkByteOrder
DECLARE FUNCTION gdk_visual_get_colormap_size(BYVAL AS GdkVisual PTR) AS gint
DECLARE FUNCTION gdk_visual_get_depth(BYVAL AS GdkVisual PTR) AS gint
DECLARE SUB gdk_visual_get_green_pixel_details(BYVAL AS GdkVisual PTR, BYVAL AS guint32 PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE SUB gdk_visual_get_red_pixel_details(BYVAL AS GdkVisual PTR, BYVAL AS guint32 PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE FUNCTION gdk_visual_get_screen(BYVAL AS GdkVisual PTR) AS GdkScreen PTR
DECLARE FUNCTION gdk_visual_get_visual_type(BYVAL AS GdkVisual PTR) AS GdkVisualType
TYPE _GdkWindow
  AS GObject parent_instance
END TYPE
DECLARE FUNCTION gdk_window_object_get_type() AS GType
#DEFINE GDK_TYPE_WINDOW_OBJECT (gdk_window_object_get_type())
#DEFINE GDK_WINDOW_OBJECT(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), GDK_TYPE_WINDOW_OBJECT, GdkWindow))
#DEFINE GDK_WINDOW_OBJECT_CLASS(obj) (G_TYPE_CHECK_CLASS_CAST((obj), GDK_TYPE_WINDOW_OBJECT, Gdk))
#DEFINE GDK_IS_WINDOW_OBJECT(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), GDK_TYPE_WINDOW_OBJECT))
#DEFINE GDK_IS_CLASS_WINDOW_OBJECT(obj) (G_TYPE_CHECK_CLASS_TYPE((obj), GDK_TYPE_WINDOW_OBJECT))
#DEFINE GDK_WINDOW_OBJECT_GET_CLASS(obj) (G_TYPE_INSTANCE_GET_CLASS((obj), GDK_TYPE_WINDOW_OBJECT, Gdk))
DECLARE SUB gdk_window_add_filter(BYVAL AS GdkWindow PTR, BYVAL AS GdkFilterFunc, BYVAL AS gpointer)
DECLARE SUB gdk_window_beep(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_begin_move_drag(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS guint32)
DECLARE SUB gdk_window_begin_paint_rect(BYVAL AS GdkWindow PTR, BYVAL AS const GdkRectangle PTR)
DECLARE SUB gdk_window_begin_paint_region(BYVAL AS GdkWindow PTR, BYVAL AS const GdkRegion PTR)
DECLARE SUB gdk_window_begin_resize_drag(BYVAL AS GdkWindow PTR, BYVAL AS GdkWindowEdge, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS guint32)
DECLARE SUB gdk_window_clear(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_clear_area(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_clear_area_e(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_configure_finished(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_coords_from_parent(BYVAL AS GdkWindow PTR, BYVAL AS gdouble, BYVAL AS gdouble, BYVAL AS gdouble PTR, BYVAL AS gdouble PTR)
DECLARE SUB gdk_window_coords_to_parent(BYVAL AS GdkWindow PTR, BYVAL AS gdouble, BYVAL AS gdouble, BYVAL AS gdouble PTR, BYVAL AS gdouble PTR)
DECLARE FUNCTION gdk_window_create_similar_surface(BYVAL AS GdkWindow PTR, BYVAL AS cairo_content_t, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/) AS cairo_surface_t PTR
DECLARE SUB gdk_window_deiconify(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_destroy(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_destroy_notify(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_enable_synchronized_configure(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_end_paint(BYVAL AS GdkWindow PTR)
DECLARE FUNCTION gdk_window_ensure_native(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE SUB gdk_window_flush(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_focus(BYVAL AS GdkWindow PTR, BYVAL AS guint32)
DECLARE SUB gdk_window_freeze_toplevel_updates_libgtk_only(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_freeze_updates(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_fullscreen(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_geometry_changed(BYVAL AS GdkWindow PTR)
DECLARE FUNCTION gdk_window_get_accept_focus(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE FUNCTION gdk_window_get_background_pattern(BYVAL AS GdkWindow PTR) AS cairo_pattern_t PTR
DECLARE FUNCTION gdk_window_get_children(BYVAL AS GdkWindow PTR) AS GList PTR
DECLARE FUNCTION gdk_window_get_composited(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE FUNCTION gdk_window_get_cursor(BYVAL AS GdkWindow PTR) AS GdkCursor PTR
DECLARE FUNCTION gdk_window_get_decorations(BYVAL AS GdkWindow PTR, BYVAL AS GdkWMDecoration PTR) AS gboolean
DECLARE FUNCTION gdk_window_get_deskrelative_origin(BYVAL AS GdkWindow PTR, BYVAL AS gint PTR, BYVAL AS gint PTR) AS gboolean
DECLARE FUNCTION gdk_window_get_display(BYVAL AS GdkWindow PTR) AS GdkDisplay PTR
DECLARE FUNCTION gdk_window_get_effective_parent(BYVAL AS GdkWindow PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_window_get_effective_toplevel(BYVAL AS GdkWindow PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_window_get_events(BYVAL AS GdkWindow PTR) AS GdkEventMask
DECLARE FUNCTION gdk_window_get_focus_on_map(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE SUB gdk_window_get_frame_extents(BYVAL AS GdkWindow PTR, BYVAL AS GdkRectangle PTR)
DECLARE SUB gdk_window_get_geometry(BYVAL AS GdkWindow PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE FUNCTION gdk_window_get_group(BYVAL AS GdkWindow PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_window_get_height(BYVAL AS GdkWindow PTR) AS gint /'int'/
DECLARE SUB gdk_window_get_internal_paint_info(BYVAL AS GdkWindow PTR, BYVAL AS GdkDrawable PTR PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE FUNCTION gdk_window_get_modal_hint(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE FUNCTION gdk_window_get_origin(BYVAL AS GdkWindow PTR, BYVAL AS gint PTR, BYVAL AS gint PTR) AS gint
DECLARE FUNCTION gdk_window_get_parent(BYVAL AS GdkWindow PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_window_get_pointer(BYVAL AS GdkWindow PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS GdkModifierType PTR) AS GdkWindow PTR
DECLARE SUB gdk_window_get_position(BYVAL AS GdkWindow PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE SUB gdk_window_get_root_coords(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE SUB gdk_window_get_root_origin(BYVAL AS GdkWindow PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE FUNCTION gdk_window_get_screen(BYVAL AS GdkWindow PTR) AS GdkScreen PTR
DECLARE FUNCTION gdk_window_get_state(BYVAL AS GdkWindow PTR) AS GdkWindowState
DECLARE FUNCTION gdk_window_get_toplevel(BYVAL AS GdkWindow PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_window_get_type_hint(BYVAL AS GdkWindow PTR) AS GdkWindowTypeHint
DECLARE FUNCTION gdk_window_get_update_area(BYVAL AS GdkWindow PTR) AS GdkRegion PTR
DECLARE SUB gdk_window_get_user_data(BYVAL AS GdkWindow PTR, BYVAL AS gpointer PTR)
DECLARE FUNCTION gdk_window_get_visual(BYVAL AS GdkWindow PTR) AS GdkVisual PTR
DECLARE FUNCTION gdk_window_get_width(BYVAL AS GdkWindow PTR) AS gint /'int'/
DECLARE FUNCTION gdk_window_get_window_type(BYVAL AS GdkWindow PTR) AS GdkWindowType
DECLARE FUNCTION gdk_window_has_native(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE SUB gdk_window_hide(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_iconify(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_input_shape_combine_mask(BYVAL AS GdkWindow PTR, BYVAL AS GdkBitmap PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_input_shape_combine_region(BYVAL AS GdkWindow PTR, BYVAL AS const GdkRegion PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_invalidate_maybe_recurse(BYVAL AS GdkWindow PTR, BYVAL AS const GdkRegion PTR, BYVAL AS gpointer PTR, BYVAL AS gpointer)
DECLARE SUB gdk_window_invalidate_rect(BYVAL AS GdkWindow PTR, BYVAL AS const GdkRectangle PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_invalidate_region(BYVAL AS GdkWindow PTR, BYVAL AS const GdkRegion PTR, BYVAL AS gboolean)
DECLARE FUNCTION gdk_window_is_destroyed(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE FUNCTION gdk_window_is_input_only(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE FUNCTION gdk_window_is_shaped(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE FUNCTION gdk_window_is_viewable(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE FUNCTION gdk_window_is_visible(BYVAL AS GdkWindow PTR) AS gboolean
DECLARE SUB gdk_window_lower(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_maximize(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_merge_child_input_shapes(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_merge_child_shapes(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_move(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_move_region(BYVAL AS GdkWindow PTR, BYVAL AS const GdkRegion PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_move_resize(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE FUNCTION gdk_window_new(BYVAL AS GdkWindow PTR, BYVAL AS GdkWindowAttr PTR, BYVAL AS gint) AS GdkWindow PTR
DECLARE FUNCTION gdk_window_peek_children(BYVAL AS GdkWindow PTR) AS GList PTR
DECLARE SUB gdk_window_process_updates(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_raise(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_redirect_to_drawable(BYVAL AS GdkWindow PTR, BYVAL AS GdkDrawable PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_register_dnd(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_remove_filter(BYVAL AS GdkWindow PTR, BYVAL AS GdkFilterFunc, BYVAL AS gpointer)
DECLARE SUB gdk_window_remove_redirection(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_reparent(BYVAL AS GdkWindow PTR, BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_resize(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_restack(BYVAL AS GdkWindow PTR, BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_scroll(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_set_accept_focus(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_back_pixmap(BYVAL AS GdkWindow PTR, BYVAL AS GdkPixmap PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_background(BYVAL AS GdkWindow PTR, BYVAL AS const GdkColor PTR)
DECLARE SUB gdk_window_set_child_input_shapes(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_set_child_shapes(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_set_composited(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_cursor(BYVAL AS GdkWindow PTR, BYVAL AS GdkCursor PTR)
DECLARE SUB gdk_window_set_decorations(BYVAL AS GdkWindow PTR, BYVAL AS GdkWMDecoration)
DECLARE SUB gdk_window_set_events(BYVAL AS GdkWindow PTR, BYVAL AS GdkEventMask)
DECLARE SUB gdk_window_set_focus_on_map(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_functions(BYVAL AS GdkWindow PTR, BYVAL AS GdkWMFunction)
DECLARE SUB gdk_window_set_geometry_hints(BYVAL AS GdkWindow PTR, BYVAL AS const GdkGeometry PTR, BYVAL AS GdkWindowHints)
DECLARE SUB gdk_window_set_group(BYVAL AS GdkWindow PTR, BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_set_hints(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_set_icon(BYVAL AS GdkWindow PTR, BYVAL AS GdkWindow PTR, BYVAL AS GdkPixmap PTR, BYVAL AS GdkBitmap PTR)
DECLARE SUB gdk_window_set_icon_list(BYVAL AS GdkWindow PTR, BYVAL AS GList PTR)
DECLARE SUB gdk_window_set_icon_name(BYVAL AS GdkWindow PTR, BYVAL AS const gchar PTR)
DECLARE SUB gdk_window_set_keep_above(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_keep_below(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_modal_hint(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_opacity(BYVAL AS GdkWindow PTR, BYVAL AS gdouble)
DECLARE SUB gdk_window_set_override_redirect(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_role(BYVAL AS GdkWindow PTR, BYVAL AS const gchar PTR)
DECLARE SUB gdk_window_set_skip_pager_hint(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_skip_taskbar_hint(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_startup_id(BYVAL AS GdkWindow PTR, BYVAL AS const gchar PTR)
DECLARE FUNCTION gdk_window_set_static_gravities(BYVAL AS GdkWindow PTR, BYVAL AS gboolean) AS gboolean
DECLARE SUB gdk_window_set_title(BYVAL AS GdkWindow PTR, BYVAL AS const gchar PTR)
DECLARE SUB gdk_window_set_transient_for(BYVAL AS GdkWindow PTR, BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_set_type_hint(BYVAL AS GdkWindow PTR, BYVAL AS GdkWindowTypeHint)
DECLARE SUB gdk_window_set_urgency_hint(BYVAL AS GdkWindow PTR, BYVAL AS gboolean)
DECLARE SUB gdk_window_set_user_data(BYVAL AS GdkWindow PTR, BYVAL AS gpointer)
DECLARE SUB gdk_window_shape_combine_mask(BYVAL AS GdkWindow PTR, BYVAL AS GdkBitmap PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_shape_combine_region(BYVAL AS GdkWindow PTR, BYVAL AS const GdkRegion PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_window_show(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_show_unraised(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_stick(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_thaw_toplevel_updates_libgtk_only(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_thaw_updates(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_unfullscreen(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_unmaximize(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_unstick(BYVAL AS GdkWindow PTR)
DECLARE SUB gdk_window_withdraw(BYVAL AS GdkWindow PTR)
TYPE _GdkWindowAttr
  AS gchar PTR title
  AS gint event_mask
  AS gint x
  AS gint y
  AS gint width
  AS gint height
  AS GdkWindowClass wclass
  AS GdkVisual PTR visual
  AS GdkColormap PTR colormap
  AS GdkWindowType window_type
  AS GdkCursor PTR cursor
  AS gchar PTR wmclass_name
  AS gchar PTR wmclass_class
  AS gboolean override_redirect
  AS GdkWindowTypeHint type_hint
END TYPE
TYPE _GdkWindowObjectClass
  AS GdkDrawableClass parent_class
END TYPE
' P_4

DECLARE SUB gdk_add_client_message_filter(BYVAL AS GdkAtom, BYVAL AS GdkFilterFunc, BYVAL AS gpointer)
DECLARE SUB gdk_add_option_entries_libgtk_only(BYVAL AS GOptionGroup PTR)
DECLARE SUB gdk_beep()
DECLARE FUNCTION gdk_cairo_create(BYVAL AS GdkDrawable PTR) AS cairo_t PTR
DECLARE SUB gdk_cairo_rectangle(BYVAL AS cairo_t PTR, BYVAL AS const GdkRectangle PTR)
DECLARE SUB gdk_cairo_region(BYVAL AS cairo_t PTR, BYVAL AS const GdkRegion PTR)
DECLARE SUB gdk_cairo_reset_clip(BYVAL AS cairo_t PTR, BYVAL AS GdkDrawable PTR)
DECLARE SUB gdk_cairo_set_source_color(BYVAL AS cairo_t PTR, BYVAL AS const GdkColor PTR)
DECLARE SUB gdk_cairo_set_source_pixbuf(BYVAL AS cairo_t PTR, BYVAL AS const GdkPixbuf PTR, BYVAL AS double, BYVAL AS double)
DECLARE SUB gdk_cairo_set_source_pixmap(BYVAL AS cairo_t PTR, BYVAL AS GdkPixmap PTR, BYVAL AS double, BYVAL AS double)
DECLARE SUB gdk_cairo_set_source_window(BYVAL AS cairo_t PTR, BYVAL AS GdkWindow PTR, BYVAL AS double, BYVAL AS double)
DECLARE FUNCTION gdk_char_height(BYVAL AS GdkFont PTR, BYVAL AS gint8) AS gint
DECLARE FUNCTION gdk_char_measure(BYVAL AS GdkFont PTR, BYVAL AS gint8) AS gint
DECLARE FUNCTION gdk_char_width(BYVAL AS GdkFont PTR, BYVAL AS gint8) AS gint
DECLARE FUNCTION gdk_char_width_wc(BYVAL AS GdkFont PTR, BYVAL AS GdkWChar) AS gint
DECLARE FUNCTION gdk_colors_alloc(BYVAL AS GdkColormap PTR, BYVAL AS gboolean, BYVAL AS gulong PTR, BYVAL AS gint, BYVAL AS gulong PTR, BYVAL AS gint) AS gint
DECLARE SUB gdk_colors_free(BYVAL AS GdkColormap PTR, BYVAL AS gulong PTR, BYVAL AS gint, BYVAL AS gulong)
DECLARE SUB gdk_colors_store(BYVAL AS GdkColormap PTR, BYVAL AS GdkColor PTR, BYVAL AS gint)
DECLARE FUNCTION gdk_devices_list() AS GList PTR
DECLARE SUB gdk_drag_abort(BYVAL AS GdkDragContext PTR, BYVAL AS guint32)
DECLARE FUNCTION gdk_drag_begin(BYVAL AS GdkWindow PTR, BYVAL AS GList PTR) AS GdkDragContext PTR
DECLARE SUB gdk_drag_drop(BYVAL AS GdkDragContext PTR, BYVAL AS guint32)
DECLARE FUNCTION gdk_drag_drop_succeeded(BYVAL AS GdkDragContext PTR) AS gboolean
DECLARE SUB gdk_drag_find_window(BYVAL AS GdkDragContext PTR, BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkWindow PTR PTR, BYVAL AS GdkDragProtocol PTR)
DECLARE SUB gdk_drag_find_window_for_screen(BYVAL AS GdkDragContext PTR, BYVAL AS GdkWindow PTR, BYVAL AS GdkScreen PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkWindow PTR PTR, BYVAL AS GdkDragProtocol PTR)
DECLARE FUNCTION gdk_drag_get_protocol(BYVAL AS GdkNativeWindow, BYVAL AS GdkDragProtocol PTR) AS GdkNativeWindow
DECLARE FUNCTION gdk_drag_get_protocol_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkNativeWindow, BYVAL AS GdkDragProtocol PTR) AS GdkNativeWindow
DECLARE FUNCTION gdk_drag_get_selection(BYVAL AS GdkDragContext PTR) AS GdkAtom
DECLARE FUNCTION gdk_drag_motion(BYVAL AS GdkDragContext PTR, BYVAL AS GdkWindow PTR, BYVAL AS GdkDragProtocol, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkDragAction, BYVAL AS GdkDragAction, BYVAL AS guint32) AS gboolean
DECLARE SUB gdk_drag_status(BYVAL AS GdkDragContext PTR, BYVAL AS GdkDragAction, BYVAL AS guint32)
DECLARE SUB gdk_draw_arc(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gboolean, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_draw_drawable(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS GdkDrawable PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_draw_glyphs(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS PangoFont PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS PangoGlyphString PTR)
DECLARE SUB gdk_draw_glyphs_transformed(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS const PangoMatrix PTR, BYVAL AS PangoFont PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS PangoGlyphString PTR)
DECLARE SUB gdk_draw_gray_image(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkRgbDither, BYVAL AS const guchar PTR, BYVAL AS gint)
DECLARE SUB gdk_draw_image(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS GdkImage PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_draw_indexed_image(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkRgbDither, BYVAL AS const guchar PTR, BYVAL AS gint, BYVAL AS GdkRgbCmap PTR)
DECLARE SUB gdk_draw_layout(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS PangoLayout PTR)
DECLARE SUB gdk_draw_layout_line(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS PangoLayoutLine PTR)
DECLARE SUB gdk_draw_layout_line_with_colors(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS PangoLayoutLine PTR, BYVAL AS const GdkColor PTR, BYVAL AS const GdkColor PTR)
DECLARE SUB gdk_draw_layout_with_colors(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS PangoLayout PTR, BYVAL AS const GdkColor PTR, BYVAL AS const GdkColor PTR)
DECLARE SUB gdk_draw_line(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_draw_lines(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS const GdkPoint PTR, BYVAL AS gint)
DECLARE SUB gdk_draw_pixbuf(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS const GdkPixbuf PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkRgbDither, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_draw_point(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_draw_points(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS const GdkPoint PTR, BYVAL AS gint)
DECLARE SUB gdk_draw_polygon(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gboolean, BYVAL AS const GdkPoint PTR, BYVAL AS gint)
DECLARE SUB gdk_draw_rectangle(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gboolean, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_draw_rgb_32_image(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkRgbDither, BYVAL AS const guchar PTR, BYVAL AS gint)
DECLARE SUB gdk_draw_rgb_32_image_dithalign(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkRgbDither, BYVAL AS const guchar PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_draw_rgb_image(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkRgbDither, BYVAL AS const guchar PTR, BYVAL AS gint)
DECLARE SUB gdk_draw_rgb_image_dithalign(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint, BYVAL AS GdkRgbDither, BYVAL AS const guchar PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint)
DECLARE SUB gdk_draw_segments(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS const GdkSegment PTR, BYVAL AS gint)
DECLARE SUB gdk_draw_string(BYVAL AS GdkDrawable PTR, BYVAL AS GdkFont PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS const gchar PTR)
DECLARE SUB gdk_draw_text(BYVAL AS GdkDrawable PTR, BYVAL AS GdkFont PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS const gchar PTR, BYVAL AS gint)
DECLARE SUB gdk_draw_text_wc(BYVAL AS GdkDrawable PTR, BYVAL AS GdkFont PTR, BYVAL AS GdkGC PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS const GdkWChar PTR, BYVAL AS gint)
DECLARE SUB gdk_draw_trapezoids(BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS const GdkTrapezoid PTR, BYVAL AS gint)
DECLARE SUB gdk_drop_finish(BYVAL AS GdkDragContext PTR, BYVAL AS gboolean, BYVAL AS guint32)
DECLARE SUB gdk_drop_reply(BYVAL AS GdkDragContext PTR, BYVAL AS gboolean, BYVAL AS guint32)
DECLARE FUNCTION gdk_error_trap_pop() AS gint
DECLARE SUB gdk_error_trap_push()
DECLARE FUNCTION gdk_events_pending() AS gboolean
DECLARE SUB gdk_exit(BYVAL AS gint)
DECLARE SUB gdk_flush()
DECLARE FUNCTION gdk_fontset_load(BYVAL AS const gchar PTR) AS GdkFont PTR
DECLARE FUNCTION gdk_fontset_load_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS const gchar PTR) AS GdkFont PTR
DECLARE SUB gdk_free_compound_text(BYVAL AS guchar PTR)
DECLARE SUB gdk_free_text_list(BYVAL AS gchar PTR PTR)
DECLARE FUNCTION gdk_get_default_root_window() AS GdkWindow PTR
DECLARE FUNCTION gdk_get_display() AS gchar PTR
DECLARE FUNCTION gdk_get_display_arg_name() AS const gchar PTR
DECLARE FUNCTION gdk_get_program_class() AS const char PTR
DECLARE FUNCTION gdk_get_show_events() AS gboolean
DECLARE FUNCTION gdk_get_use_xshm() AS gboolean
DECLARE SUB gdk_init(BYVAL AS gint PTR, BYVAL AS gchar PTR PTR PTR)
DECLARE FUNCTION gdk_init_check(BYVAL AS gint PTR, BYVAL AS gchar PTR PTR PTR) AS gboolean
DECLARE FUNCTION gdk_input_add(BYVAL AS gint, BYVAL AS GdkInputCondition, BYVAL AS GdkInputFunction, BYVAL AS gpointer) AS gint
DECLARE FUNCTION gdk_input_add_full(BYVAL AS gint, BYVAL AS GdkInputCondition, BYVAL AS GdkInputFunction, BYVAL AS gpointer, BYVAL AS GDestroyNotify) AS gint
DECLARE SUB gdk_input_remove(BYVAL AS gint)
DECLARE SUB gdk_input_set_extension_events(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS GdkExtensionMode)
DECLARE FUNCTION gdk_keyboard_grab(BYVAL AS GdkWindow PTR, BYVAL AS gboolean, BYVAL AS guint32) AS GdkGrabStatus
DECLARE SUB gdk_keyboard_ungrab(BYVAL AS guint32)
DECLARE SUB gdk_keyval_convert_case(BYVAL AS guint, BYVAL AS guint PTR, BYVAL AS guint PTR)
DECLARE FUNCTION gdk_keyval_from_name(BYVAL AS const gchar PTR) AS guint
DECLARE FUNCTION gdk_keyval_is_lower(BYVAL AS guint) AS gboolean
DECLARE FUNCTION gdk_keyval_is_upper(BYVAL AS guint) AS gboolean
DECLARE FUNCTION gdk_keyval_name(BYVAL AS guint) AS gchar PTR
DECLARE FUNCTION gdk_keyval_to_lower(BYVAL AS guint) AS guint
DECLARE FUNCTION gdk_keyval_to_unicode(BYVAL AS guint) AS guint32
DECLARE FUNCTION gdk_keyval_to_upper(BYVAL AS guint) AS guint
DECLARE FUNCTION gdk_list_visuals() AS GList PTR
DECLARE FUNCTION gdk_mbstowcs(BYVAL AS GdkWChar PTR, BYVAL AS const gchar PTR, BYVAL AS gint) AS gint
DECLARE SUB gdk_notify_startup_complete()
DECLARE SUB gdk_notify_startup_complete_with_id(BYVAL AS const gchar PTR)
DECLARE FUNCTION gdk_offscreen_window_get_embedder(BYVAL AS GdkWindow PTR) AS GdkWindow PTR
DECLARE FUNCTION gdk_offscreen_window_get_pixmap(BYVAL AS GdkWindow PTR) AS GdkPixmap PTR
DECLARE SUB gdk_offscreen_window_set_embedder(BYVAL AS GdkWindow PTR, BYVAL AS GdkWindow PTR)
DECLARE FUNCTION gdk_pango_context_get() AS PangoContext PTR
DECLARE FUNCTION gdk_pango_context_get_for_screen(BYVAL AS GdkScreen PTR) AS PangoContext PTR
DECLARE SUB gdk_pango_context_set_colormap(BYVAL AS PangoContext PTR, BYVAL AS GdkColormap PTR)
DECLARE FUNCTION gdk_pango_layout_get_clip_region(BYVAL AS PangoLayout PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS const gint PTR, BYVAL AS gint) AS GdkRegion PTR
DECLARE FUNCTION gdk_pango_layout_line_get_clip_region(BYVAL AS PangoLayoutLine PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS const gint PTR, BYVAL AS gint) AS GdkRegion PTR
DECLARE SUB gdk_parse_args(BYVAL AS gint PTR, BYVAL AS gchar PTR PTR PTR)
DECLARE FUNCTION gdk_pixbuf_get_from_drawable(BYVAL AS GdkPixbuf PTR, BYVAL AS GdkDrawable PTR, BYVAL AS GdkColormap PTR, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/) AS GdkPixbuf PTR
DECLARE FUNCTION gdk_pixbuf_get_from_image(BYVAL AS GdkPixbuf PTR, BYVAL AS GdkImage PTR, BYVAL AS GdkColormap PTR, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/) AS GdkPixbuf PTR
DECLARE SUB gdk_pixbuf_render_pixmap_and_mask(BYVAL AS GdkPixbuf PTR, BYVAL AS GdkPixmap PTR PTR, BYVAL AS GdkBitmap PTR PTR, BYVAL AS gint /'int'/)
DECLARE SUB gdk_pixbuf_render_pixmap_and_mask_for_colormap(BYVAL AS GdkPixbuf PTR, BYVAL AS GdkColormap PTR, BYVAL AS GdkPixmap PTR PTR, BYVAL AS GdkBitmap PTR PTR, BYVAL AS gint /'int'/)
DECLARE SUB gdk_pixbuf_render_threshold_alpha(BYVAL AS GdkPixbuf PTR, BYVAL AS GdkBitmap PTR, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/)
DECLARE SUB gdk_pixbuf_render_to_drawable(BYVAL AS GdkPixbuf PTR, BYVAL AS GdkDrawable PTR, BYVAL AS GdkGC PTR, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS GdkRgbDither, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/)
DECLARE SUB gdk_pixbuf_render_to_drawable_alpha(BYVAL AS GdkPixbuf PTR, BYVAL AS GdkDrawable PTR, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/, BYVAL AS GdkPixbufAlphaMode, BYVAL AS gint /'int'/, BYVAL AS GdkRgbDither, BYVAL AS gint /'int'/, BYVAL AS gint /'int'/)
DECLARE FUNCTION gdk_pointer_grab(BYVAL AS GdkWindow PTR, BYVAL AS gboolean, BYVAL AS GdkEventMask, BYVAL AS GdkWindow PTR, BYVAL AS GdkCursor PTR, BYVAL AS guint32) AS GdkGrabStatus
DECLARE FUNCTION gdk_pointer_grab_info_libgtk_only(BYVAL AS GdkDisplay PTR, BYVAL AS GdkWindow PTR PTR, BYVAL AS gboolean PTR) AS gboolean
DECLARE FUNCTION gdk_pointer_is_grabbed() AS gboolean
DECLARE SUB gdk_pointer_ungrab(BYVAL AS guint32)
DECLARE SUB gdk_pre_parse_libgtk_only()
DECLARE SUB gdk_property_change(BYVAL AS GdkWindow PTR, BYVAL AS GdkAtom, BYVAL AS GdkAtom, BYVAL AS gint, BYVAL AS GdkPropMode, BYVAL AS const guchar PTR, BYVAL AS gint)
DECLARE SUB gdk_property_delete(BYVAL AS GdkWindow PTR, BYVAL AS GdkAtom)
DECLARE FUNCTION gdk_property_get(BYVAL AS GdkWindow PTR, BYVAL AS GdkAtom, BYVAL AS GdkAtom, BYVAL AS gulong, BYVAL AS gulong, BYVAL AS gint, BYVAL AS GdkAtom PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS guchar PTR PTR) AS gboolean
DECLARE SUB gdk_query_depths(BYVAL AS gint PTR PTR, BYVAL AS gint PTR)
DECLARE SUB gdk_query_visual_types(BYVAL AS GdkVisualType PTR PTR, BYVAL AS gint PTR)
DECLARE FUNCTION gdk_rgb_colormap_ditherable(BYVAL AS GdkColormap PTR) AS gboolean
DECLARE FUNCTION gdk_rgb_ditherable() AS gboolean
DECLARE SUB gdk_rgb_find_color(BYVAL AS GdkColormap PTR, BYVAL AS GdkColor PTR)
DECLARE SUB gdk_rgb_gc_set_background(BYVAL AS GdkGC PTR, BYVAL AS guint32)
DECLARE SUB gdk_rgb_gc_set_foreground(BYVAL AS GdkGC PTR, BYVAL AS guint32)
DECLARE FUNCTION gdk_rgb_get_colormap() AS GdkColormap PTR
DECLARE FUNCTION gdk_rgb_get_visual() AS GdkVisual PTR
DECLARE SUB gdk_rgb_init()
DECLARE SUB gdk_rgb_set_install(BYVAL AS gboolean)
DECLARE SUB gdk_rgb_set_min_colors(BYVAL AS gint)
DECLARE SUB gdk_rgb_set_verbose(BYVAL AS gboolean)
DECLARE FUNCTION gdk_rgb_xpixel_from_rgb(BYVAL AS guint32) AS gulong
DECLARE SUB gdk_selection_convert(BYVAL AS GdkWindow PTR, BYVAL AS GdkAtom, BYVAL AS GdkAtom, BYVAL AS guint32)
DECLARE FUNCTION gdk_selection_owner_get(BYVAL AS GdkAtom) AS GdkWindow PTR
DECLARE FUNCTION gdk_selection_owner_get_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkAtom) AS GdkWindow PTR
DECLARE FUNCTION gdk_selection_owner_set(BYVAL AS GdkWindow PTR, BYVAL AS GdkAtom, BYVAL AS guint32, BYVAL AS gboolean) AS gboolean
DECLARE FUNCTION gdk_selection_owner_set_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkWindow PTR, BYVAL AS GdkAtom, BYVAL AS guint32, BYVAL AS gboolean) AS gboolean
DECLARE FUNCTION gdk_selection_property_get(BYVAL AS GdkWindow PTR, BYVAL AS guchar PTR PTR, BYVAL AS GdkAtom PTR, BYVAL AS gint PTR) AS gint
DECLARE SUB gdk_selection_send_notify(BYVAL AS GdkNativeWindow, BYVAL AS GdkAtom, BYVAL AS GdkAtom, BYVAL AS GdkAtom, BYVAL AS guint32)
DECLARE SUB gdk_selection_send_notify_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkNativeWindow, BYVAL AS GdkAtom, BYVAL AS GdkAtom, BYVAL AS GdkAtom, BYVAL AS guint32)
DECLARE SUB gdk_set_double_click_time(BYVAL AS guint)
DECLARE FUNCTION gdk_set_locale() AS gchar PTR
DECLARE FUNCTION gdk_set_pointer_hooks(BYVAL AS const GdkPointerHooks PTR) AS GdkPointerHooks PTR
DECLARE SUB gdk_set_program_class(BYVAL AS const char PTR)
DECLARE SUB gdk_set_show_events(BYVAL AS gboolean)
DECLARE SUB gdk_set_sm_client_id(BYVAL AS const gchar PTR)
DECLARE SUB gdk_set_use_xshm(BYVAL AS gboolean)
DECLARE FUNCTION gdk_setting_get(BYVAL AS const gchar PTR, BYVAL AS GValue PTR) AS gboolean
DECLARE FUNCTION gdk_spawn_command_line_on_screen(BYVAL AS GdkScreen PTR, BYVAL AS const gchar PTR, BYVAL AS GError PTR PTR) AS gboolean
DECLARE FUNCTION gdk_spawn_on_screen(BYVAL AS GdkScreen PTR, BYVAL AS const gchar PTR, BYVAL AS gchar PTR PTR, BYVAL AS gchar PTR PTR, BYVAL AS GSpawnFlags, BYVAL AS GSpawnChildSetupFunc, BYVAL AS gpointer, BYVAL AS gint PTR, BYVAL AS GError PTR PTR) AS gboolean
DECLARE FUNCTION gdk_spawn_on_screen_with_pipes(BYVAL AS GdkScreen PTR, BYVAL AS const gchar PTR, BYVAL AS gchar PTR PTR, BYVAL AS gchar PTR PTR, BYVAL AS GSpawnFlags, BYVAL AS GSpawnChildSetupFunc, BYVAL AS gpointer, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS GError PTR PTR) AS gboolean
DECLARE SUB gdk_string_extents(BYVAL AS GdkFont PTR, BYVAL AS const gchar PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE FUNCTION gdk_string_height(BYVAL AS GdkFont PTR, BYVAL AS const gchar PTR) AS gint
DECLARE FUNCTION gdk_string_measure(BYVAL AS GdkFont PTR, BYVAL AS const gchar PTR) AS gint
DECLARE FUNCTION gdk_string_to_compound_text(BYVAL AS const gchar PTR, BYVAL AS GdkAtom PTR, BYVAL AS gint PTR, BYVAL AS guchar PTR PTR, BYVAL AS gint PTR) AS gint
DECLARE FUNCTION gdk_string_to_compound_text_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS const gchar PTR, BYVAL AS GdkAtom PTR, BYVAL AS gint PTR, BYVAL AS guchar PTR PTR, BYVAL AS gint PTR) AS gint
DECLARE FUNCTION gdk_string_width(BYVAL AS GdkFont PTR, BYVAL AS const gchar PTR) AS gint
DECLARE SUB gdk_synthesize_window_state(BYVAL AS GdkWindow PTR, BYVAL AS GdkWindowState, BYVAL AS GdkWindowState)
DECLARE SUB gdk_test_render_sync(BYVAL AS GdkWindow PTR)
DECLARE FUNCTION gdk_test_simulate_button(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS guint, BYVAL AS GdkModifierType, BYVAL AS GdkEventType) AS gboolean
DECLARE FUNCTION gdk_test_simulate_key(BYVAL AS GdkWindow PTR, BYVAL AS gint, BYVAL AS gint, BYVAL AS guint, BYVAL AS GdkModifierType, BYVAL AS GdkEventType) AS gboolean
DECLARE SUB gdk_text_extents(BYVAL AS GdkFont PTR, BYVAL AS const gchar PTR, BYVAL AS gint, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE SUB gdk_text_extents_wc(BYVAL AS GdkFont PTR, BYVAL AS const GdkWChar PTR, BYVAL AS gint, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE FUNCTION gdk_text_height(BYVAL AS GdkFont PTR, BYVAL AS const gchar PTR, BYVAL AS gint) AS gint
DECLARE FUNCTION gdk_text_measure(BYVAL AS GdkFont PTR, BYVAL AS const gchar PTR, BYVAL AS gint) AS gint
DECLARE FUNCTION gdk_text_property_to_text_list(BYVAL AS GdkAtom, BYVAL AS gint, BYVAL AS const guchar PTR, BYVAL AS gint, BYVAL AS gchar PTR PTR PTR) AS gint
DECLARE FUNCTION gdk_text_property_to_text_list_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkAtom, BYVAL AS gint, BYVAL AS const guchar PTR, BYVAL AS gint, BYVAL AS gchar PTR PTR PTR) AS gint
DECLARE FUNCTION gdk_text_property_to_utf8_list(BYVAL AS GdkAtom, BYVAL AS gint, BYVAL AS const guchar PTR, BYVAL AS gint, BYVAL AS gchar PTR PTR PTR) AS gint
DECLARE FUNCTION gdk_text_property_to_utf8_list_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkAtom, BYVAL AS gint, BYVAL AS const guchar PTR, BYVAL AS gint, BYVAL AS gchar PTR PTR PTR) AS gint
DECLARE FUNCTION gdk_text_width(BYVAL AS GdkFont PTR, BYVAL AS const gchar PTR, BYVAL AS gint) AS gint
DECLARE FUNCTION gdk_text_width_wc(BYVAL AS GdkFont PTR, BYVAL AS const GdkWChar PTR, BYVAL AS gint) AS gint
DECLARE FUNCTION gdk_threads_add_idle(BYVAL AS GSourceFunc, BYVAL AS gpointer) AS guint
DECLARE FUNCTION gdk_threads_add_idle_full(BYVAL AS gint, BYVAL AS GSourceFunc, BYVAL AS gpointer, BYVAL AS GDestroyNotify) AS guint
DECLARE FUNCTION gdk_threads_add_timeout(BYVAL AS guint, BYVAL AS GSourceFunc, BYVAL AS gpointer) AS guint
DECLARE FUNCTION gdk_threads_add_timeout_full(BYVAL AS gint, BYVAL AS guint, BYVAL AS GSourceFunc, BYVAL AS gpointer, BYVAL AS GDestroyNotify) AS guint
DECLARE FUNCTION gdk_threads_add_timeout_seconds(BYVAL AS guint, BYVAL AS GSourceFunc, BYVAL AS gpointer) AS guint
DECLARE FUNCTION gdk_threads_add_timeout_seconds_full(BYVAL AS gint, BYVAL AS guint, BYVAL AS GSourceFunc, BYVAL AS gpointer, BYVAL AS GDestroyNotify) AS guint
DECLARE SUB gdk_threads_enter()
DECLARE SUB gdk_threads_init()
DECLARE SUB gdk_threads_leave()
DECLARE SUB gdk_threads_set_lock_functions(BYVAL AS GCallback, BYVAL AS GCallback)
DECLARE FUNCTION gdk_unicode_to_keyval(BYVAL AS guint32) AS guint
DECLARE FUNCTION gdk_utf8_to_compound_text(BYVAL AS const gchar PTR, BYVAL AS GdkAtom PTR, BYVAL AS gint PTR, BYVAL AS guchar PTR PTR, BYVAL AS gint PTR) AS gboolean
DECLARE FUNCTION gdk_utf8_to_compound_text_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS const gchar PTR, BYVAL AS GdkAtom PTR, BYVAL AS gint PTR, BYVAL AS guchar PTR PTR, BYVAL AS gint PTR) AS gboolean
DECLARE FUNCTION gdk_utf8_to_string_target(BYVAL AS const gchar PTR) AS gchar PTR
DECLARE FUNCTION gdk_wcstombs(BYVAL AS const GdkWChar PTR) AS gchar PTR
DECLARE FUNCTION gdk_window_at_pointer(BYVAL AS gint PTR, BYVAL AS gint PTR) AS GdkWindow PTR
DECLARE SUB gdk_window_constrain_size(BYVAL AS GdkGeometry PTR, BYVAL AS guint, BYVAL AS gint, BYVAL AS gint, BYVAL AS gint PTR, BYVAL AS gint PTR)
DECLARE FUNCTION gdk_window_foreign_new(BYVAL AS GdkNativeWindow) AS GdkWindow PTR
DECLARE FUNCTION gdk_window_foreign_new_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkNativeWindow) AS GdkWindow PTR
DECLARE FUNCTION gdk_window_get_toplevels() AS GList PTR
DECLARE FUNCTION gdk_window_lookup(BYVAL AS GdkNativeWindow) AS GdkWindow PTR
DECLARE FUNCTION gdk_window_lookup_for_display(BYVAL AS GdkDisplay PTR, BYVAL AS GdkNativeWindow) AS GdkWindow PTR
DECLARE SUB gdk_window_process_all_updates()
DECLARE SUB gdk_window_set_debug_updates(BYVAL AS gboolean)
END EXTERN

