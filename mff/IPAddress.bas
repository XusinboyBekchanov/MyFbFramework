...
Namespace My.Sys.Forms
	...
	Private Property IPAddress.Text ByRef As WString
			...
			For i As Integer = 1 To 3
				#ifdef __USE_GTK4__
					WAdd(FText, "." & Trim(WStr(Val(*gtk_entry_buffer_get_text(gtk_entry_get_buffer(GTK_ENTRY(Entries(i))))))))
				#else
					WAdd(FText, "." & Trim(WStr(Val(*gtk_entry_get_text(GTK_ENTRY(Entries(i)))))))
				#endif
			Next
	...
	End Operator
		...
		Private Sub IPAddress.Layout_SizeAllocate(widget As GtkWidget Ptr, allocation As GdkRectangle Ptr, user_data As Any Ptr)
			...
			If allocation->width <> ipa->AllocatedWidth OrElse allocation->height <> ipa->AllocatedHeight Then
				...
				If ipa->OnResize Then ipa->OnResize(*ipa->Designer, *ipa, ipa->UnScaleX(allocation->width), ipa->UnScaleY(allocation->height))
			End If
		End Sub
